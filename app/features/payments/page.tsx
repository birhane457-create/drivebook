  ...
  return (
    <div className="light min-h-screen bg-background text-foreground">
      ...
      <main>
        {/* Hero */}
        <FeatureHero
          gradientClasses={heroPresets.emerald.gradient}
          badgeIcon={Shield}
          badgeText="Powered by Stripe · PCI compliant"
          badgeColors={heroPresets.emerald.badgeColors}
          title={
            <>
          ...
          description="Students pay upfront when booking. Lesson costs are deducted automatically. You receive a weekly payout without lifting a finger."
          ctaHref="/teach-with-drivebook"
          ctaText="Start Free Trial →"
          ctaGradient={heroPresets.emerald.ctaGradient}
          ctaShadow={heroPresets.emerald.ctaShadow}
          breadcrumb={{
            parent: 'For Instructors',
            parentHref: '/for-instructors',
            current: 'Payments'
          }}
          blobColor={heroPresets.emerald.blobColor}
        />

        {/* How money flows */}
        <section className="max-w-4xl mx-auto px-4 py-16 md:py-20">
          <h2 className="text-3xl font-bold text-foreground text-center mb-3">How Money Flows</h2>
          <p className="text-foreground/50 text-center text-sm mb-12">From student card to instructor bank account — fully automated.</p>
          <div className="space-y-3 max-w-2xl mx-auto">
            {[
              ...
              <div key={n} className="flex gap-4 p-4 rounded-xl bg-white/[0.04] border border-border hover:border-emerald-500/30 transition-all">
        ...
        <section className="bg-white/[0.02] border-y border-border py-14 px-4">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-bold text-foreground text-center mb-12">Everything in the Payment System</h2>
            <div className="grid md:grid-cols-2 gap-5">
              {[
                ...
                <div key={title} className="flex gap-4 p-5 rounded-xl bg-white/[0.04] border border-border hover:border-emerald-500/30 transition-all">
          ...
          <div className="space-y-2">
            {[
              ...
              <Link key={href} href={href} className="flex items-center justify-between p-3.5 rounded-xl border border-border bg-card/40 hover:bg-white/[0.07] hover:border-white/20 transition-all no-underline group">
                ...
                <ChevronRight className="h-4 w-4 text-foreground/30 group-hover:text-emerald-400 shrink-0 ml-3" />
              </Link>
            ))}
          </div>
        </section>
        ...
        <section className="max-w-3xl mx-auto px-4 pb-16">
          <div className="rounded-2xl bg-gradient-to-br from-emerald-900/60 to-teal-900/60 border border-emerald-500/30 p-10 text-center">
            <h2 className="text-2xl font-bold text-foreground mb-3">Stop Chasing Payments</h2>
            <p className="text-foreground/60 mb-6 text-sm max-w-md mx-auto">DriveBook's payment system is included on all plans. Students pay upfront, you get paid weekly.</p>

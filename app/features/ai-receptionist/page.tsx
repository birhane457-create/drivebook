  ...
  return (
    <div className="light min-h-screen bg-background text-foreground">
      ...
      <main>
        {/* Hero */}
        <FeatureHero
          gradientClasses={heroPresets.pink.gradient}
          badgeIcon={Zap}
          badgeText="Available on PRO and above"
          badgeColors={heroPresets.pink.badgeColors}
          title={
            <>
          ...
          description="Every missed call is a potential booking lost. DriveBook's AI receptionist answers immediately, checks your live calendar, confirms the lesson, and sends the student an SMS — while you're in the car teaching."
          ctaHref="/teach-with-drivebook"
          ctaText="Start Free Trial →"
          ctaGradient={heroPresets.pink.ctaGradient}
          ctaShadow={heroPresets.pink.ctaShadow}
          secondaryLink={{
            href: '/blog/drivebook-ai-phone-receptionist-driving-instructors',
            text: 'How It Works'
          }}
          breadcrumb={{
          ...
          blobColor={heroPresets.pink.blobColor}
        />
        ...
        <section className="max-w-4xl mx-auto px-4 py-16 md:py-20">
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              <h2 className="text-3xl font-bold text-foreground mb-4">You're in the car. The phone rings. You can't answer.</h2>
            ...
            <div className="space-y-3">
              {['A student calls during your 10am lesson', 'AI answers immediately — no hold music', 'AI checks your calendar in real time', 'Offers 3 available slots this week', 'Student confirms a time', 'SMS confirmation sent to student', 'Booking appears in your dashboard'].map((step, i) => (
                <div key={step} className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.04] border border-border">
        ...
        <section className="bg-white/[0.02] border-y border-border py-16 px-4">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-bold text-foreground text-center mb-12">What the AI Receptionist Does</h2>
            <div className="grid md:grid-cols-3 gap-6">
              {[
                ...
                <div key={title} className="bg-white/[0.04] border border-border rounded-2xl p-6 hover:border-white/20 transition-all">
        ...
        {/* Comparison table */}
        <section className="max-w-4xl mx-auto px-4 py-16 md:py-20">
          <h2 className="text-3xl font-bold text-foreground mb-3 text-center">AI Receptionist vs Human Receptionist</h2>
          <p className="text-foreground/50 text-center mb-10 text-sm">For most driving providers, a human receptionist is cost-prohibitive. AI fills the gap.</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-white/[0.04]">
                  ...
                  <th className="text-center p-4 text-foreground/50 font-semibold">Human Receptionist</th>
                </tr>
              </thead>
              <tbody>
                {comparisonRows.map(({ feature, ai, human, note }) => (
                  <tr key={feature} className="border-b border-border/60 hover:bg-white/[0.02] transition-colors">
                    <td className="p-4">
                      <span className="text-foreground font-medium">{feature}</span>
                      {note && <span className="block text-foreground/40 text-xs mt-0.5">{note}</span>}
                    </td>
                    <td className="p-4 text-center">
                      {ai === true ? <CheckCircle className="h-5 w-5 text-emerald-400 mx-auto" /> :
                       ai === false ? <XCircle className="h-5 w-5 text-destructive/60 mx-auto" /> :
                    ...
                    <td className="p-4 text-center">
                      {human === true ? <CheckCircle className="h-5 w-5 text-emerald-400 mx-auto" /> :
                       human === false ? <XCircle className="h-5 w-5 text-destructive/60 mx-auto" /> :
                       human === 'partial' ? <span className="text-yellow-400 text-xs font-semibold">Partial</span> :
                       <span className="text-foreground/50 text-xs">{human}</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="space-y-2">
            {[
              ...
              <Link key={href} href={href} className="flex items-center justify-between p-3.5 rounded-xl border border-border bg-card/40 hover:bg-white/[0.07] hover:border-white/20 transition-all no-underline group">
                ...
                <ChevronRight className="h-4 w-4 text-foreground/30 group-hover:text-pink-400 shrink-0 ml-3" />
              </Link>
            ))}
          </div>
        </section>
        ...
        <section className="max-w-3xl mx-auto px-4 pb-16">
          <div className="rounded-2xl bg-gradient-to-br from-pink-900/60 to-violet-900/60 border border-pink-500/30 p-10 text-center">
            <h2 className="text-2xl font-bold text-foreground mb-3">Stop Missing Calls. Start Growing.</h2>
            <p className="text-foreground/60 mb-6 text-sm max-w-md mx-auto">The AI receptionist is included in every DriveBook PRO plan. Start your free trial today.</p>

    ...
    <header className="border-b border-border bg-background/80 backdrop-blur-xl sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1 text-sm">
          <Link href="/learn-to-drive" className="text-foreground px-3 py-2 rounded-lg bg-secondary no-underline font-semibold">Learn to Drive</Link>
          <Link href="/pda-guide" className="text-foreground/70 hover:text-foreground px-3 py-2 rounded-lg hover:bg-secondary transition-colors no-underline">PDA Guide</Link>
          ...
          <Link href="/book" className="ml-2 bg-gradient-to-r from-violet-600 to-indigo-600 text-foreground px-5 py-2 rounded-xl font-bold text-sm no-underline hover:from-violet-500 hover:to-indigo-500 transition-all">
            Book Now
          </Link>
        </nav>
      </div>
...
function SiteFooter() {
  return (
    <footer className="border-t border-border py-10 mt-16 text-center text-foreground/40 text-sm">
      <p>© {new Date().getFullYear()} DriveBook · <Link href="/privacy" className="hover:text-foreground/60 no-underline transition-colors">Privacy</Link> · <Link href="/terms" className="hover:text-foreground/60 no-underline transition-colors">Terms</Link></p>
    </footer>
  )
}
...
function StepCard({ num, title, desc, href }: { num: string; title: string; desc: string; href?: string }) {
  const inner = (
    <div className="flex gap-4 p-5 rounded-2xl bg-secondary/50 border border-border hover:border-violet-500/40 hover:bg-white/[0.08] transition-all group">
      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-600 to-indigo-600 flex items-center justify-center shrink-0 text-foreground font-bold text-sm shadow-lg shadow-violet-500/20">
        {num}
      </div>
      <div>
        <p className="font-semibold text-foreground group-hover:text-violet-200 transition-colors mb-1">{title}</p>
        <p className="text-foreground/60 text-sm leading-relaxed">{desc}</p>
      </div>
      {href && <ChevronRight className="h-4 w-4 text-foreground/30 group-hover:text-violet-400 shrink-0 self-center ml-auto transition-colors" />}
    </div>
  )
  ...
  return (
    <Link href={href} className="flex items-center justify-between p-4 rounded-xl border border-border bg-card/40 hover:bg-white/[0.07] hover:border-white/20 transition-all no-underline group">
      <span className="flex items-center gap-3 min-w-0">
        ...
        <span className="text-sm font-medium text-foreground group-hover:text-violet-200 transition-colors truncate">{title}</span>
      </span>
      ...
      <main>
        ...
        <section className="relative overflow-hidden bg-gradient-to-br from-violet-900 via-indigo-900 to-slate-950 py-20 md:py-28 px-4 text-center">
          ...
          <div className="relative max-w-3xl mx-auto">
            <nav className="flex items-center justify-center gap-2 text-xs text-white/50 mb-6">
              ...
              <span>/</span>
              <span className="text-white/70">Learn to Drive</span>
            </nav>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <Link href="/pda-guide" className="bg-secondary hover:bg-white/20 text-white px-8 py-3.5 rounded-xl font-semibold no-underline transition-all border border-border">
                PDA Guide →
              </Link>
            </div>
          </div>
        ...
        <section className="max-w-4xl mx-auto px-4 py-16 md:py-20">
          <div className="text-center mb-10">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-3">The Road to Your Licence</h2>
            <p className="text-foreground/60">Five stages every WA learner driver goes through.</p>
          </div>
        ...
        {/* Quick Stats */}
        <section className="bg-card/40 border-y border-border py-10 px-4">
        ...
        <section className="max-w-5xl mx-auto px-4 py-16 md:py-20">
          <div className="grid md:grid-cols-2 gap-10">
            ...
            <div>
              ...
              <h2 className="text-xl font-bold text-foreground mt-10 mb-5 flex items-center gap-2">
        ...
        <section className="max-w-3xl mx-auto px-4 py-12 md:py-16">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={q} className="bg-secondary/50 border border-border rounded-xl p-5 hover:border-foreground/20 transition-all">
                <p className="font-semibold text-foreground mb-2">{q}</p>
                <p className="text-foreground/70 text-sm leading-relaxed">{a}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="max-w-3xl mx-auto px-4 pb-16 text-center">
          <div className="rounded-2xl bg-gradient-to-br from-violet-900/60 to-indigo-900/60 border border-violet-500/30 p-10">
            ...
            <p className="text-white/70 mb-6">Find a verified instructor near you, check availability, and book online in seconds.</p>

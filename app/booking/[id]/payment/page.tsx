  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <button
        ...
        className="w-full bg-primary hover:bg-primary/90 active:bg-blue-800 disabled:bg-gray-300 text-foreground font-semibold py-3.5 rounded-xl text-base transition-colors"
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center space-y-3">
          ...
          <p className="text-muted-foreground/60 text-sm">Loading your booking...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gray-50 py-6 px-4">
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        <div className="text-center pt-2">
          ...
          <p className="text-muted-foreground/60 text-sm mt-1">Pay now to confirm your slot</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="px-5 py-4 space-y-4">
            ...
            <div className="flex items-center gap-3">
              {summary.provider.profileImage ? (
                <img
                  src={summary.provider.profileImage}
                ...
                <div className="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
                  <span className="text-blue-600 font-bold text-lg" aria-hidden="true">
                    {summary.provider.name.charAt(0)}
                  </span>
                </div>
              )}
              <div>
                <p className="font-semibold text-gray-900">{summary.provider.name}</p>
                <p className="text-xs text-muted-foreground/60">Driving Instructor</p>
                {summary.provider.rating && (
                  <p className="text-xs text-amber-500 mt-0.5" aria-label={`Rating: ${summary.provider.rating}`}>
                    ★ {summary.provider.rating.toFixed(1)}
                    {summary.provider.reviews > 0 && (
                      <span className="text-muted-foreground"> ({summary.provider.reviews} reviews)</span>
                    )}
                  </p>
                )}
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">One-time charge — no recurring fees</p>
          </div>
        ...
        <div className="text-center pb-2">
          {cancelConfirming ? (
            ...
            <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-4 text-sm text-left">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={cancelling}
                  className="flex-1 py-2 rounded-lg bg-destructive text-foreground text-xs font-semibold hover:bg-destructive/90 transition-colors disabled:opacity-50"
                >
            ...
            <>
              <p className="text-xs text-muted-foreground mb-1">Need to make changes?</p>
              <button
                onClick={handleCancel}
                disabled={cancelling}
                className="text-xs text-muted-foreground/60 hover:text-red-600 underline transition-colors disabled:opacity-50"
              >
        ...
        {/* Trust footer */}
        <div className="flex items-center justify-center gap-5 text-xs text-muted-foreground pb-4">
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-lg p-8 max-w-sm w-full text-center space-y-4">
        ...
        <button
          onClick={() => go(action)}
          className="w-full bg-primary text-foreground py-3 rounded-xl font-semibold hover:bg-primary/90 transition-colors"
        >
          {action.label}
        </button>
        {secondaryAction && (
          <button
            ...
            className="w-full text-muted-foreground/60 text-sm underline"
          >

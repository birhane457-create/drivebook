...
export default async function BookingDetailsPage({
  ...
  const booking = await prisma.booking.findUnique({
    where: { id: params.id },
    include: {
      provider: { select: { name: true, phone: true, profileImage: true } },
  ...
  const isInstructor = session!.user!.role === 'provider';
  const now = new Date();
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow mb-4">
          <div className="p-4 sm:p-6 border-b border-gray-200">
            ...
            <p className="text-sm text-muted-foreground/60">ID: #{formatBookingId(booking.id)}</p>
          </div>
          ...
          <div className="p-4 sm:p-6 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground/60">{booking.provider.phone}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600 mb-1">Client</p>
                <p className="font-medium text-gray-900">{booking.customer.name}</p>
                <p className="text-sm text-muted-foreground/60">{booking.customer.phone}</p>
              </div>
            </div>
              ...
              <div className={`border rounded-lg p-4 ${booking.checkInTime ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'}`}>
                ...
                {booking.checkInTime ? (
                  <div className="text-sm space-y-1">
                    ...
                    <p className="text-gray-700">
                      By: {booking.checkInBy === 'provider' ? booking.provider.name : booking.customer.name}
                    </p>
              ...
              <div className={`border rounded-lg p-4 ${booking.checkOutTime ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-200'}`}>
                ...
                {booking.checkOutTime ? (
                  <div className="text-sm space-y-1">
                    ...
                    <p className="text-gray-700">
                      By: {booking.checkOutBy === 'provider' ? booking.provider.name : booking.customer.name}
                    </p>

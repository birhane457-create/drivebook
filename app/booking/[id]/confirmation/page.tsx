...
interface BookingSummary {
  ...
  total: number;
  currency: string;
  provider: { name: string; profileImage: string | null; rating: number | null };
}
...
export default function BookingConfirmationPage() {
  ...
  const wizardInstructor = searchParams.get('provider') ?? '';
  ...
  const instructorName = summary?.provider?.name ?? 'your instructor';
            ...
            <div className="px-5 py-4 space-y-3 text-sm">
              ...
              <div className="flex items-center gap-3">
                {summary.provider.profileImage ? (
                  <img src={summary.provider.profileImage} alt={instructorName}
              ...
              <div className="bg-gray-50 rounded-xl p-3 grid grid-cols-2 gap-2 text-xs">
                {summary.date && (
                  <div>
                    <p className="text-muted-foreground uppercase tracking-wide mb-0.5">Date</p>
                ...
                {summary.time && (
                  <div>
                    <p className="text-muted-foreground uppercase tracking-wide mb-0.5">Time</p>
                ...
                {summary.isPackageBooking && summary.packageHours && (
                  <div>
                    <p className="text-muted-foreground uppercase tracking-wide mb-0.5">Package</p>
                    <p className="font-medium text-gray-900">{summary.packageHours}-Hour Package</p>
                  </div>
                )}
                {summary.pickupLocation && (
                  <div className="col-span-2">
                    <p className="text-muted-foreground uppercase tracking-wide mb-0.5">Pickup</p>
              ...
              <div className="text-xs text-muted-foreground flex items-center gap-1">
                <span>Ref:</span>
                <code className="font-mono text-muted-foreground/60">{bookingId.slice(0, 12)}...</code>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="px-5 py-4 space-y-3 text-sm">
              ...
              {(wizardDate || wizardTime || wizardHours) && (
                <div className="bg-gray-50 rounded-xl p-3 grid grid-cols-2 gap-2 text-xs">
                  {wizardDate && (
                    <div>
                      <p className="text-muted-foreground uppercase tracking-wide mb-0.5">First Lesson</p>
                      <p className="font-medium text-gray-900">
                        {new Date(wizardDate + 'T00:00:00').toLocaleDateString('en-AU', {
                          weekday: 'short', day: 'numeric', month: 'short', year: 'numeric',
                        })}
                        {wizardTime && <span className="text-muted-foreground/60"> at {wizardTime}</span>}
                      </p>
                    </div>
                  )}
                  {wizardHours && (
              ...
              <div className="text-xs text-muted-foreground flex items-center gap-1">
                ...
                <code className="font-mono text-muted-foreground/60 select-all">{bookingId}</code>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col gap-3">
          {session?.user ? (
            <Link href="/client-dashboard"
              className="block w-full bg-primary text-foreground py-3.5 rounded-xl font-semibold text-center hover:bg-primary/90 transition-colors">
              Go to My Dashboard
            </Link>
          ) : (
            <>

    ...
    <div className="light min-h-screen bg-background text-foreground">
      ...
      <main className="max-w-4xl mx-auto px-4 py-12">
        ...
        <nav className="flex items-center gap-2 text-xs text-foreground/40 mb-8">
          ...
          <span>/</span>
          <Link href="/for-instructors" className="hover:text-foreground no-underline">For Instructors</Link>
          <span>/</span>
          <span className="text-foreground/60">DriveBook vs Google Calendar</span>
        </nav>

        <div className="mb-12">
          <p className="text-sm font-semibold text-violet-400 uppercase tracking-wider mb-3">Comparison</p>
          <h1 className="text-4xl md:text-5xl font-bold text-foreground mb-5 leading-tight">DriveBook vs Google Calendar<br />for Driving Instructors</h1>
        ...
        <div className="grid md:grid-cols-2 gap-5 mb-14">
          <div className="bg-white/[0.04] border border-border rounded-2xl p-6">
            <p className="text-xs font-semibold text-foreground/40 uppercase tracking-wider mb-3">Google Calendar is good for...</p>
            <ul className="space-y-2">
              {['Tracking your own schedule', 'Sharing availability with people you trust', 'Syncing across devices', 'Setting personal reminders'].map((i: any) => (
                <li key={i} className="flex gap-2 text-sm text-foreground/70"><CheckCircle className="h-4 w-4 text-emerald-400 shrink-0 mt-0.5" />{i}</li>
              ))}
            </ul>
          </div>
          <div className="bg-white/[0.04] border border-border rounded-2xl p-6">
            <p className="text-xs font-semibold text-foreground/40 uppercase tracking-wider mb-3">Google Calendar cannot...</p>
            <ul className="space-y-2">
              {['Let students book lessons directly', 'Collect payment at booking', 'Send automated SMS reminders', 'Track student progress', 'Enforce a cancellation policy', 'Give you a public booking page'].map((i: any) => (
                <li key={i} className="flex gap-2 text-sm text-foreground/70"><XCircle className="h-4 w-4 text-destructive/70 shrink-0 mt-0.5" />{i}</li>
              ))}
            </ul>
          </div>
        </div>
        ...
        <h2 className="text-2xl font-bold text-foreground mb-6">Full Feature Comparison</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-white/[0.04]">
                ...
                <th className="text-center p-4 text-foreground/50 font-semibold">Google Calendar</th>
              </tr>
            </thead>
            <tbody>
              {rows.map(({ feature, db, gc, note }) => {
                const Cell = ({ v }: { v: boolean | string }) =>
                  ...
                  v === false ? <XCircle className="h-5 w-5 text-destructive/60 mx-auto" /> :
                ...
                return (
                  <tr key={feature} className="border-b border-border/60 hover:bg-white/[0.02]">
                    <td className="p-4">
                      ...
                      {note && <span className="block text-foreground/35 text-xs mt-0.5">{note}</span>}
                    </td>
        ...
        <section className="prose prose-invert max-w-none mb-12">
          <h2 className="text-2xl font-bold text-foreground mb-4">The Real Difference</h2>
          ...
          <h3 className="text-xl font-bold text-foreground mt-8 mb-3">Can I use both?</h3>
          <p className="text-foreground/60 leading-relaxed">
        ...
        <div className="rounded-2xl bg-gradient-to-br from-violet-900/60 to-indigo-900/60 border border-violet-500/30 p-10 text-center">
          <h2 className="text-2xl font-bold text-foreground mb-3">Ready for a Real Booking System?</h2>
          <p className="text-foreground/60 mb-6 text-sm max-w-md mx-auto">Start your free DriveBook trial today. It takes under 10 minutes to set up your booking page.</p>
          ...
          <p className="text-foreground/30 text-xs mt-4">No credit card required</p>
        </div>

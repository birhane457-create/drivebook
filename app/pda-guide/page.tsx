    ...
    <header className="border-b border-border bg-background/80 backdrop-blur-xl sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1 text-sm">
          <Link href="/learn-to-drive" className="text-foreground/70 hover:text-foreground px-3 py-2 rounded-lg hover:bg-secondary transition-colors no-underline">Learn to Drive</Link>
          <Link href="/pda-guide" className="text-foreground px-3 py-2 rounded-lg bg-secondary no-underline font-semibold">PDA Guide</Link>
          ...
          <Link href="/driving-lessons" className="text-foreground/70 hover:text-foreground px-3 py-2 rounded-lg hover:bg-secondary transition-colors no-underline">Find Instructors</Link>
...
function ArticleLink({ href, title }: { href: string; title: string }) {
  return (
    <Link href={href} className="flex items-center justify-between p-4 rounded-xl border border-border bg-card/40 hover:bg-white/[0.07] hover:border-white/20 transition-all no-underline group">
      <span className="text-sm font-medium text-foreground group-hover:text-violet-200 transition-colors">{title}</span>
      <ChevronRight className="h-4 w-4 text-foreground/30 group-hover:text-violet-400 shrink-0 ml-3 transition-colors" />
    </Link>
  )
}
  ...
  return (
    <div className="light min-h-screen bg-background text-foreground">
      ...
      <main className="max-w-4xl mx-auto px-4">
        ...
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2 text-xs text-foreground/40 pt-8 mb-8">
          <Link href="/" className="hover:text-foreground no-underline">Home</Link>
          <span>/</span>
          <Link href="/learn-to-drive" className="hover:text-foreground no-underline">Learn to Drive</Link>
          <span>/</span>
          <span className="text-foreground/60">PDA Guide</span>
        </nav>
        ...
        <section className="grid sm:grid-cols-3 gap-4 mb-14">
          <div className="bg-secondary/50 border border-border rounded-2xl p-5 text-center">
            ...
            <p className="text-2xl font-bold text-foreground mb-1">30–45 min</p>
            <p className="text-foreground/50 text-xs">On-road duration</p>
          </div>
          <div className="bg-secondary/50 border border-border rounded-2xl p-5 text-center">
            <AlertTriangle className="h-6 w-6 text-yellow-400 mx-auto mb-2" />
            <p className="text-2xl font-bold text-foreground mb-1">~10 faults</p>
            <p className="text-foreground/50 text-xs">Typical failure threshold</p>
          </div>
          <div className="bg-secondary/50 border border-border rounded-2xl p-5 text-center">
            <CheckCircle className="h-6 w-6 text-emerald-400 mx-auto mb-2" />
            <p className="text-2xl font-bold text-foreground mb-1">3 docs</p>
            <p className="text-foreground/50 text-xs">Permit · Logbook · HPT cert</p>
          </div>
        </section>

        {/* Prerequisites */}
        <section className="mb-14">
          <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-6">Before You Can Book</h2>
          ...
          <p className="text-foreground/40 text-xs mt-4">All requirements must be met before the assessment can proceed. The assessor will verify documents before you drive.</p>
        </section>

        {/* What the PDA Tests */}
        <section className="mb-14">
          <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-6">What the PDA Tests</h2>
          <div className="grid md:grid-cols-2 gap-4">
            {[
              ...
              <div key={cat} className="bg-white/[0.04] border border-border rounded-2xl p-5">
                <h3 className="font-bold text-foreground mb-3 text-sm uppercase tracking-wide text-violet-400">{cat}</h3>
        ...
        {/* Scoring */}
        <section className="mb-14">
          <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-6">How Scoring Works</h2>
          <div className="grid md:grid-cols-2 gap-5">
            <div className="rounded-2xl bg-red-900/20 border border-red-500/30 p-6">
              <div className="flex items-center gap-2 mb-4">
                <XCircle className="h-5 w-5 text-destructive" />
                <h3 className="font-bold text-destructive">Immediate Failure</h3>
              </div>
              <p className="text-foreground/60 text-sm mb-4">Any of these ends the assessment immediately, regardless of your other performance:</p>
              <ul className="space-y-2 text-sm text-foreground/70">
                {['Causing or nearly causing a collision', 'Disobeying a red traffic light', 'Driving dangerously', 'Requiring the assessor to intervene', 'Driving on the wrong side of the road'].map((i: any) => (
                  <li key={i} className="flex gap-2"><XCircle className="h-3.5 w-3.5 text-destructive shrink-0 mt-0.5" />{i}</li>
                ))}
              </ul>
            </div>
            <div className="rounded-2xl bg-yellow-900/20 border border-yellow-500/30 p-6">
              ...
              <p className="text-foreground/60 text-sm mb-4">Minor errors are recorded as fault marks. Accumulating too many — or repeating the same error — results in a fail.</p>
              <ul className="space-y-2 text-sm text-foreground/70">
                {['Insufficient mirror checks', 'Poor speed management', 'Hesitation or stalling', 'Incomplete give way', 'Late or jerky braking', 'Incorrect gear selection (manual)'].map((i: any) => (
        ...
        <section className="mb-14">
          <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-2">The Most Common Failure Causes</h2>
          <p className="text-foreground/50 text-sm mb-6">Based on WA assessment patterns — address these specifically in your preparation.</p>
        ...
        {/* On Test Day */}
        <section className="mb-14">
          <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-6">On Test Day</h2>
          <div className="space-y-3">
            {[
              ...
              <div key={step} className="flex gap-4 p-4 rounded-xl bg-white/[0.04] border border-border">
                <div className="w-7 h-7 rounded-full bg-secondary flex items-center justify-center text-xs font-bold text-foreground/60 shrink-0">{i + 1}</div>
                <div>
                  <p className="font-semibold text-foreground text-sm mb-0.5">{step}</p>
                  <p className="text-foreground/55 text-xs leading-relaxed">{detail}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Related Articles */}
        <section className="mb-14">
          <h2 className="text-2xl font-bold text-foreground mb-6">All PDA-Related Guides</h2>
        ...
        {/* FAQ */}
        <section className="mb-14">
          <h2 className="text-2xl font-bold text-foreground mb-6">PDA FAQ</h2>
          <div className="space-y-3">
            {[
        ...
        <section className="pb-16 text-center">
          <div className="rounded-2xl bg-gradient-to-br from-violet-900/60 to-indigo-900/60 border border-violet-500/30 p-10">
            <h2 className="text-2xl font-bold text-foreground mb-3">Find an Instructor Who Knows Your Test Routes</h2>
            <p className="text-foreground/60 mb-6 text-sm max-w-md mx-auto">Local instructors on DriveBook know the PDA routes from test centres near you and will prepare you specifically for what you'll face on the day.</p>

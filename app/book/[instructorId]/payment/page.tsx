...
function PaymentForm({ setIsRedirecting, slotSecondsLeft }: { setIsRedirecting: (value: boolean) => void; slotSecondsLeft: number | null }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (bookingState.scheduledBookings.length > 0) {
        const slotsToValidate = bookingState.scheduledBookings.map((booking: any) => ({
          date: booking.date,
          time: booking.time,
      ...
      const buildConfirmationUrl = (paymentStatus: string) => {
        ...
        if (bookingState.provider?.displayName || bookingState.provider?.name) confirmParams.set('provider', bookingState.provider.displayName || bookingState.provider.name);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 text-foreground">
      {/* Card Payment Form */}
      <div className="bg-secondary/50 border border-border rounded-2xl p-6 backdrop-blur-sm">
        <h3 className="text-lg font-semibold text-foreground/95 mb-4">Payment Information</h3>
      ...
      {/* Success Message */}
      {success && (
        <div className="bg-green-900/10 border border-green-500/20 rounded-lg p-4 text-foreground">
      ...
      {/* Error Message */}
      {error && (
        <div className="bg-red-900/10 border border-red-500/20 rounded-lg p-4 text-foreground">
          <div className="flex gap-3">
            ...
            <div className="flex-1">
              ...
              {(error.toLowerCase().includes('account') || error.toLowerCase().includes('email') || error.toLowerCase().includes('log in')) && (
                <div className="mt-3 flex flex-wrap gap-2">
                  <a
                    href="/login"
                    className="inline-flex items-center px-4 py-2 rounded-md text-sm font-medium bg-gradient-to-r from-purple-600 to-pink-600 text-foreground hover:from-purple-500 hover:to-pink-500"
                  >
                    Log In
                  </a>
                  <a
                    href="/auth/forgot-password"
                    className="inline-flex items-center px-4 py-2 rounded-md text-sm font-medium bg-secondary/50 text-foreground border border-border hover:bg-secondary"
                  >
                    Reset Password
                  </a>
                </div>
      ...
      <button
        ...
        className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-foreground px-8 py-4 rounded-lg font-semibold hover:from-purple-500 hover:to-pink-500 transition-colors disabled:bg-gray-700 disabled:cursor-not-allowed flex items-center justify-center gap-2"
      >
        {loading ? (
          <>
      ...
      <div className="flex items-center justify-center gap-2 text-sm text-foreground/75">
...
export default function PaymentPage() {
  ...
  useEffect(() => {
    if (!bookingState.provider && !isRedirecting) {
      router.push('/book');
    }
  }, [bookingState.provider, router, isRedirecting]);
  ...
  if (!stripePublishableKey) {
    return (
      <MultiStepBookingLayout currentStep={bookingState.bookingType === 'now' ? 6 : 5}>
        <div className="bg-red-900/10 border border-red-500/20 rounded-lg p-6 text-foreground">
    ...
    <MultiStepBookingLayout currentStep={stepNumber}>
      <div className="space-y-6">
        ...
        {bookingState.accountHolderName && (
          <div className="bg-secondary/50 border border-border rounded-lg p-4 text-foreground">
            <h4 className="font-semibold text-foreground/90 text-sm mb-2">Account Details</h4>
        ...
        {bookingState.scheduledBookings.length > 0 && (
          <div className="bg-green-900/10 border border-green-500/20 rounded-lg p-4 text-foreground">
            <div className="flex items-center justify-between mb-2">
              ...
              {slotSecondsLeft !== null && (
                <span className={`text-xs font-mono font-semibold px-2 py-0.5 rounded-full ${
                  ...
                  ⏱ {Math.floor(slotSecondsLeft / 60)} min {String(slotSecondsLeft % 60).padStart(2, '0')} sec left
                </span>
              )}
            </div>
            <div className="space-y-2">
              {bookingState.scheduledBookings.slice(0, 3).map((booking: any, index: number) => (
            ...
            {bookingState.remainingHours > 0 && (
              <p className="text-xs text-white font-medium bg-green-600 px-3 py-2 rounded-lg mt-2">
        ...
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-foreground/95">Payment Information</h3>

...
export default function ConfirmationPage() {
  ...
  const handleConfirmAndPay = async () => {
    ...
    try {
      ...
      router.push(`/book/${bookingState.provider.id}/payment`);
    } catch (error) {
      setValidationError(error instanceof Error ? error.message : 'Failed to validate booking. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <MultiStepBookingLayout currentStep={bookingState.bookingType === 'now'
      ? (bookingState.provider?.offersTestPackage ? 6 : 5)
      : (bookingState.provider?.offersTestPackage ? 5 : 4)
    }>
      <div className="space-y-8 max-w-2xl">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 border border-blue-100">
          ...
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-full bg-gray-300 flex-shrink-0 flex items-center justify-center text-foreground text-2xl font-bold">
              {(bookingState.provider.displayName || bookingState.provider.name).charAt(0)}
            </div>
            <div className="flex-1">
              <p className="font-semibold text-gray-900">{bookingState.provider.displayName || bookingState.provider.name}</p>
              <p className="text-sm text-gray-600">
                ⭐ {bookingState.provider.averageRating?.toFixed(1) || 'New'} ({bookingState.provider.totalReviews} reviews)
              </p>
              <p className="text-sm text-gray-600">${bookingState.provider.hourlyRate}/hour</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            ...
            {bookingState.includeTestPackage && bookingState.provider.offersTestPackage && (
        ...
        {bookingState.scheduledBookings.length > 0 && (
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              {bookingState.scheduledBookings.map((booking: any, index: number) => (
        ...
        {/* Validation error (replaces alert()) */}
        {validationError && (
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <button
            ...
            disabled={loading}
            className="flex-1 bg-primary text-foreground px-6 py-3 rounded-lg font-semibold hover:bg-primary/90 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {loading ? (
              <>

...
// ── Generic website renderer (Business Operating System) ─────────────────────
// When a business has custom BusinessConfig (non-driving or customised config),
// render using the generic BusinessWebsitePage instead of the driving-specific page.
...
export async function generateMetadata({
  ...
  const metaDisplayName = getDisplayName(instructor as any);
  const title = `Book Driving Lessons with ${metaDisplayName}`;
  const description = instructor.bio
    ? instructor.bio.slice(0, 155)
    : `Book driving lessons with ${metaDisplayName}${instructor.serviceAreas ? ` in ${instructor.serviceAreas}` : ''}. From $${instructor.hourlyRate}/hr. Easy online booking, no account needed.`;
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      type: 'website',
      images: instructor.profileImage ? [{ url: instructor.profileImage, width: 400, height: 400, alt: metaDisplayName }] : [],
    },
    twitter: {
      card: 'summary',
      title,
...
export default async function SubdomainBookingPage({
  ...
  // ── Generic website renderer gate ─────────────────────────────────────────
  // Check if this provider has a custom BusinessConfig with a non-driving
  // template or custom services. If so, use the generic renderer.
  // Driving businesses (templateSlug === 'driving' or no Business record) fall
  // through to the existing driving-specific page below.
  try {
    const rootDomain = process.env.NEXT_PUBLIC_ROOT_DOMAIN ?? 'drivebook.com.au'
    const websiteData = await fetchProviderWebsiteData(params.slug, rootDomain)
    if (websiteData) {
      const { config } = websiteData
      // Use generic renderer when business has custom terminology or non-driving template
      const isCustomised =
        config.terminology.provider !== 'provider' ||
        config.terminology.booking !== 'Lesson' ||
        (config.services.length > 0 && config.services.some(s => !s.name.toLowerCase().includes('lesson')))
      if (isCustomised) {
        return <BusinessWebsitePage {...websiteData} />
      }
    }
  } catch {
    // Non-fatal — fall through to the driving-specific page
  }

  // ── Driving-specific page (existing behaviour) ────────────────────────────
  ...
  // Resolve full provider profile through extension boundary.
  // For driving: merges DrivingProviderProfile fields (vehicles, licences, PDA, etc.)
  // For other industries: returns provider unchanged — no driving fields added.
  if (instructor) {
    const { getProviderProfile } = await import('@/lib/core/getProviderProfile')
    const merged = await getProviderProfile(instructor as any)
    Object.assign(instructor as any, merged)
  }
  ...
  // subdomain page — isPro controls featured badge visibility
  const isPro = instructor.subscriptionTier === 'PRO' || instructor.subscriptionTier === 'STUDIO';
  ...
  const popularPackageHours: number | null =
    packageCounts.length > 0 && ((packageCounts[0]._count as any).id ?? 0) >= 3
      ? Number(packageCounts[0].packageHours)
      : null;
  ...
  const pdaConfigs = instructor.offersTestPackage
    ? await prisma.pDATestConfig.findMany({
        where: { providerId: instructor.id, isActive: true },
        select: {
          testCentres: {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-white shadow-sm sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {!hasBranding && <span className="text-xs text-muted-foreground hidden sm:block">Powered by DriveBook</span>}
            {whatsapp && (
              <a
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center gap-6">
            <div className="relative h-24 w-24 sm:h-32 sm:w-32 rounded-full overflow-hidden border-4 border-white shadow-lg shrink-0">
              {instructor.profileImage ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-4xl font-bold text-foreground" style={{ background: primary }}>
              ...
              <div className="flex items-center gap-1 mt-1">
                {instructor.totalReviews > 0 ? (
                  <>
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} className={`h-4 w-4 ${i < Math.round(instructor.averageRating ?? 5) ? 'fill-white text-foreground' : 'text-foreground/40'}`} />
                    ))}
                  ...
                  <span className="text-sm text-foreground/70 bg-secondary px-2 py-0.5 rounded-full">New instructor</span>
                )}
              </div>
      ...
      {isAcceptingBookings && (
        <div className="bg-white border-b border-gray-100">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-4 text-center">How booking works</p>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-4 order-2 lg:order-1">
            ...
            {nextAvailableSlots.length > 0 && (
              <div className="rounded-xl p-4 border-2 flex items-start gap-3" style={{ backgroundColor: `${primary}10`, borderColor: `${primary}40` }}>
                ...
                <div>
                  <p className="text-xs text-muted-foreground/60 font-medium uppercase tracking-wide mb-1">Next Available</p>
                  <div className="space-y-0.5">
                    {nextAvailableSlots.slice(0, 2).map((slot, i) => (
                      <p key={i} className={`font-semibold ${i === 0 ? 'text-gray-900' : 'text-muted-foreground/60 text-sm font-normal'}`}>{slot}</p>
                    ))}
                  </div>
                </div>
              </div>
            ...
            {instructor.bio?.trim() && (
              <div id="section-about" className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                ...
                {bioIsSubstantial && (
                  <p className="text-sm text-muted-foreground/60 leading-relaxed mt-3 pt-3 border-t border-gray-50">
                    {displayName} is a verified DriveBook instructor
                ...
                {specialties.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-gray-100">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Teaching style</p>
              ...
              <div className="space-y-2">
                ...
                {allowedDurations.length > 0 && (
                  <div className="py-2 border-b border-gray-50">
                    <p className="text-xs text-muted-foreground mb-2 uppercase tracking-wide font-medium">Available lesson lengths</p>
                    <div className="flex flex-wrap gap-1.5">
                      {allowedDurations.map((mins) => {
                        ...
                        return (
                          <div key={mins} className="flex flex-col items-center px-3 py-1.5 rounded-lg border border-gray-200 bg-gray-50 text-center">
                            ...
                            <span className="text-xs text-muted-foreground/60">${cost % 1 === 0 ? cost.toFixed(0) : cost.toFixed(2)}</span>
                          </div>
                        );
                      })}
                    </div>
            ...
            <div id="section-contact" className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-3">
              ...
              {coveredTestCentres.length > 0 && (
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">Test centres covered</p>
                    <div className="flex flex-wrap gap-1">
                ...
                <div className="relative h-40 transition-all duration-300 ease-in-out group-hover:h-56 overflow-hidden">
                  ...
                  {vehicleTypes.length > 0 && (
                    <div className="absolute top-2 left-2 flex gap-1">
                      {vehicleTypes.map((v: string) => (
                        <span key={v} className="text-xs font-semibold bg-black/50 text-foreground px-2 py-0.5 rounded-full backdrop-blur-sm">
                          {v}
                        </span>
                      ))}
                    </div>
            ...
            {!bioIsSubstantial && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                ...
                <div className="space-y-2 text-sm text-gray-600 leading-relaxed">
                  <p>
                    {displayName} offers driving lessons
                    {instructor.serviceAreas
                  ...
                  {yearsExperience ? (
                    <p>
                      With {yearsExperience}+ years of teaching experience, {displayName} can guide students from their first lesson through to PDA preparation, tailoring each session to the student&apos;s current ability and learning pace.
                    </p>
                  ) : null}
                </div>
              </div>
          ...
          <div className="lg:col-span-2 space-y-6 order-1 lg:order-2">
              ...
              <div className="rounded-xl p-4 border-2" style={{ backgroundColor: `${primary}10`, borderColor: `${primary}40` }}>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-0.5">{displayName} services this area. Enter your exact pickup address below.</p>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-sm text-muted-foreground/60">({instructor.totalReviews})</span>
                  </div>
                </div>
                  ...
                  {recentReviews.map((review: any, i: number) => (
                    <div key={i} className="border-b border-gray-50 last:border-0 pb-4 last:pb-0">
                      <div className="flex items-center justify-between mb-1">
                        <p className="font-medium text-gray-900 text-sm">{review.customerName || 'Student'}</p>
                        <div className="flex items-center gap-0.5">
                          {[...Array(5)].map((_, s) => (
                            <Star key={s} className={`h-3.5 w-3.5 ${s < review.customerRating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-200'}`} />
                          ))}
                        </div>
                      </div>
                      {review.customerRating && (
                        <p className="text-sm text-gray-600">{review.customerRating}</p>
                      )}
            ...
            {videoEmbed && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="px-4 py-2.5 border-t border-gray-50">
                  <p className="text-xs text-muted-foreground">🎬 Meet your instructor</p>
                </div>
              </div>
            )}
            ...
            <div id="booking-form" className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              ...
              <p className="text-sm text-muted-foreground/60 mb-6">Takes less than 60 seconds · No app install required</p>
              {isAcceptingBookings ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="font-semibold text-gray-900 mb-1">{displayName} is not currently accepting bookings</p>
                  <p className="text-sm text-muted-foreground/60 mb-5">This instructor's account is temporarily inactive. Please check back later or find another instructor.</p>
                  <a href="/instructors" className="inline-flex items-center gap-2 bg-primary hover:bg-primary/90 text-foreground px-5 py-2.5 rounded-lg font-semibold text-sm transition-colors">
      ...
      <footer className="mt-12 border-t border-gray-200 bg-white py-6">
        <div className="max-w-6xl mx-auto px-4 text-center text-sm text-muted-foreground">
          {hasBranding
            ? `© ${new Date().getFullYear()} ${displayName} · Powered by DriveBook`
            : `© ${new Date().getFullYear()} DriveBook`}
        </div>
      </footer>

      {/* Mobile bottom nav + full-screen booking drawer */}
      <SubdomainClientFeatures primary={primary} instructorName={displayName} isAcceptingBookings={isAcceptingBookings}>
        <div className="space-y-4">
          {isAcceptingBookings ? (
            <>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-muted-foreground/60 mb-4">This instructor's account is temporarily inactive.</p>
              <a href="/instructors" className="inline-flex items-center gap-2 bg-primary text-foreground px-4 py-2 rounded-lg text-sm font-semibold">
                Find Another Instructor
              </a>
            </div>
          )}

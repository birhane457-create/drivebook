...
export default function AdminCreditsPage() {
  ...
  const fetchCreditsStats = async () => {
    try {
      ...
      setFetchError('Failed to load credits data. Please refresh.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <AdminPageLayout title="Credits Management" breadcrumbs={[{ label: 'Admin', href: '/admin' }, { label: 'S' }]}>
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-2">Overview of all credits in the system</p>
        </div>
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
              {/* Total Credits */}
              <div className="bg-card rounded-lg border border-border p-6 border-t-4 border-blue-500">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-muted-foreground font-semibold">Total Credits Paid</p>
              ...
              {/* Total Spent */}
              <div className="bg-card rounded-lg border border-border p-6 border-t-4 border-orange-500">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-muted-foreground font-semibold">Total Spent</p>
              ...
              {/* Credits Remaining */}
              <div className="bg-card rounded-lg border border-border p-6 border-t-4 border-green-500">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-muted-foreground font-semibold">Total Remaining</p>
              ...
              {/* Clients Active */}
              <div className="bg-card rounded-lg border border-border p-6 border-t-4 border-purple-500">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-muted-foreground font-semibold">Clients with Credits</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              <div className="bg-card rounded-lg border border-border p-6">
                <p className="text-sm text-muted-foreground font-semibold">Average Credits/Client</p>
                <p className="text-2xl font-bold text-foreground mt-3">
                  ${stats.averageCreditPerClient.toFixed(2)}
                </p>
                <p className="text-xs text-muted-foreground/60 mt-2">Based on total clients</p>
              </div>

              {/* Average Spent Per Client */}
              <div className="bg-card rounded-lg border border-border p-6">
                <p className="text-sm text-muted-foreground font-semibold">Average Spent/Client</p>
                <p className="text-2xl font-bold text-orange-600 mt-3">
                  ${stats.averageSpentPerClient.toFixed(2)}
                </p>
                <p className="text-xs text-muted-foreground/60 mt-2">Per booking average</p>
              </div>

              {/* Utilization Rate */}
              <div className="bg-card rounded-lg border border-border p-6">
                <p className="text-sm text-muted-foreground font-semibold">Credit Utilization</p>
                <p className="text-2xl font-bold text-primary mt-3">
                  {stats.totalCreditsInSystem > 0
                    ? ((stats.totalSpent / stats.totalCreditsInSystem) * 100).toFixed(1)
                    : '0'}%
                </p>
                <p className="text-xs text-muted-foreground/60 mt-2">Of all credits in system</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              <div className="bg-amber-900/20 rounded-lg shadow p-6 border border-amber-700/50">
                ...
                <p className="text-3xl font-bold text-amber-400">{stats.clientsWithZeroBalance}</p>
                <p className="text-sm text-amber-600 mt-2">
                  Clients who have exhausted their credits
                </p>
                <Link
                  href="/admin/clients?status=zero-balance"
                  className="inline-block mt-4 px-4 py-2 bg-amber-600 text-foreground rounded-lg hover:bg-amber-700 transition text-sm font-semibold"
                >
                  View Clients →
                </Link>
              </div>
              ...
              <div className="bg-red-900/20 rounded-lg shadow p-6 border border-red-700/50">
                ...
                <p className="text-3xl font-bold text-destructive">{stats.clientsWithNegativeBalance}</p>
                <p className="text-sm text-destructive mt-2">
                  Clients with dispute or refund issues
                </p>
                <Link
                  href="/admin/clients?status=negative"
                  className="inline-block mt-4 px-4 py-2 bg-destructive text-foreground rounded-lg hover:bg-destructive/90 transition text-sm font-semibold"
                >
                  Resolve Issues →
                </Link>
              </div>
            ...
            <div className="mt-8 flex gap-4 flex-wrap">
              <Link
                href="/admin/clients"
                className="px-6 py-3 bg-primary text-foreground rounded-lg hover:bg-primary/90 transition font-semibold"
              >
                Manage All Clients
              </Link>
              <button
                onClick={fetchCreditsStats}
                className="px-6 py-3 bg-secondary text-foreground rounded-lg hover:bg-secondary transition font-semibold"
              >
                Refresh Data
              </button>
            </div>

...
function ConfigRow({ label, value, masked }: { label: string; value?: string; masked?: boolean }) {
  ...
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-border last:border-0">
      ...
      <span className={`text-sm font-mono ${value ? 'text-foreground' : 'text-muted-foreground/60'}`}>{display}</span>
    </div>
  );
}
...
export default async function AdminSettingsPage() {
  ...
  const tiers = [
    { name: 'Basic', color: 'text-foreground', border: 'border-border', monthly: basicMonthly, annual: basicAnnual, commission: basicCommission, trial: basicTrial },
    { name: 'Pro', color: 'text-primary', border: 'border-blue-700/50 bg-blue-900/20', monthly: proMonthly, annual: proAnnual, commission: proCommission, trial: proTrial },
    { name: 'Premium', color: 'text-purple-700', border: 'border-violet-700/50 bg-violet-900/20', monthly: premiumMonthly, annual: premiumAnnual, commission: premiumCommission, trial: premiumTrial },
  ];

  return (
    <div className="min-h-screen bg-background text-foreground">
      <AdminPageLayout title="Platform Settings" breadcrumbs={[{ label: 'Admin', href: '/admin' }, { label: 'S' }]}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-5">

        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Platform Settings</h1>
            <p className="text-sm text-muted-foreground/60 mt-1">Booking rules, notification channels, integrations</p>
          </div>
          <Link href="/admin/pricing" className="flex items-center gap-1.5 text-sm bg-primary text-foreground px-4 py-2 rounded-lg hover:bg-primary/90 transition-colors">
        ...
        <div className="bg-card rounded-xl border border-border">
          <div className="px-6 py-4 border-b border-border flex items-center justify-between">
            <h2 className="font-semibold text-foreground">Subscription Tiers</h2>
            <span className="text-xs text-muted-foreground/60">Prices from env · commissions editable on Pricing page</span>
          </div>
          <div className="p-6">
            <div className="grid sm:grid-cols-3 gap-4">
              {tiers.map(t => (
                <div key={t.name} className={`border-2 ${t.border} rounded-xl p-4`}>
                  ...
                  <p className="text-2xl font-bold text-foreground">${t.monthly}<span className="text-sm font-normal text-muted-foreground/60">/mo</span></p>
                  <p className="text-xs text-muted-foreground/60 mb-3">${t.annual}/yr · {t.trial}-day trial</p>
            ...
            <div className="grid sm:grid-cols-2 gap-x-8">
              {[
                ...
                <div key={label} className="flex items-center gap-2.5 py-2.5 border-b border-border last:border-0">
                  ...
                  <span className={`ml-auto text-xs font-medium ${ok ? 'text-emerald-400' : 'text-destructive'}`}>
        ...
        {/* Email Config */}
        <div className="bg-card rounded-xl border border-border">
          <div className="px-6 py-4 border-b border-border"><h2 className="font-semibold text-foreground">Email</h2></div>
        ...
        {/* SMS Config */}
        <div className="bg-card rounded-xl border border-border">
          <div className="px-6 py-4 border-b border-border"><h2 className="font-semibold text-foreground">SMS (Twilio)</h2></div>
          <div className="px-6 py-2">
        ...
        {/* Platform */}
        <div className="bg-card rounded-xl border border-border">
          <div className="px-6 py-4 border-b border-border"><h2 className="font-semibold text-foreground">Platform</h2></div>
          <div className="px-6 py-2">

...
const TARGET_TYPE_OPTIONS = ['PAYOUT', 'TRANSACTION', 'provider', 'BOOKING'];
...
function entityLink(targetType: string, targetId: string): string | null {
  ...
  if (targetType === 'provider')  return `/admin/instructors/${targetId}`;
...
function LogRow({ log }: { log: AuditLog }) {
  ...
  const statusColor = log.success
    ...
    : 'bg-red-900/40 text-destructive';
  ...
  return (
    <>
      <tr
        className={`hover:bg-secondary cursor-pointer transition-colors ${isCritical ? 'border-l-2 border-red-400' : ''}`}
        onClick={() => setExpanded(v => !v)}
      >
        {/* Time */}
        <td className="px-4 py-3 text-sm text-muted-foreground/60 whitespace-nowrap" title={exactTime(log.createdAt)}>
        ...
        <td className="px-4 py-3 text-sm text-muted-foreground">
          <div className="flex items-center gap-1.5">
            <span className="px-1.5 py-0.5 bg-secondary text-muted-foreground rounded text-xs font-mono">
              {log.targetType}
            </span>
            <span className="font-mono text-xs text-muted-foreground/60 truncate max-w-[120px]" title={log.targetId}>
        ...
        {/* Actor */}
        <td className="px-4 py-3 text-sm text-muted-foreground/60 font-mono text-xs truncate max-w-[120px]" title={log.actorId}>
          {log.actorId === 'SYSTEM' ? (
        ...
        <tr className="bg-background border-t border-border">
          <td colSpan={6} className="px-4 py-3">
            <div className="text-xs space-y-1.5">
              <p className="text-muted-foreground/60 font-medium mb-2">
                {exactTime(log.createdAt)} · Actor: {log.actorId} ({log.actorRole})
              </p>
              {log.errorMessage && (
                <p className="text-destructive font-medium">Error: {log.errorMessage}</p>
              )}
              ...
              {log.metadata?.resolutionGroupId && (
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-muted-foreground/60 text-xs">split resolution group</span>
                </div>
              )}
              {log.metadata && Object.keys(log.metadata).length > 0 ? (
                <div className="bg-card border border-border rounded-lg p-3 font-mono text-xs text-foreground overflow-x-auto">
                  {Object.entries(log.metadata).map(([k, v]) => (
                    <div key={k} className="flex gap-3">
                      <span className="text-muted-foreground/60 shrink-0 w-40">{k}</span>
                      <span className="text-foreground">{JSON.stringify(v)}</span>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-muted-foreground/60 italic">No metadata</p>
              )}
            </div>
          </td>
        </tr>
...
const QUICK_FILTERS: { label: string; emoji: string; params: Record<string, string> }[] = [
  ...
  { label: 'Instructors',   emoji: '👤', params: { targetType: 'provider' } },
...
export default function AuditLogPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <AdminPageLayout title="Audit Log" breadcrumbs={[{ label: 'Admin', href: '/admin' }, { label: 'G' }]}>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-muted-foreground/60 mt-1">Full history of financial and admin actions</p>
          </div>
          <button
            onClick={() => fetchLogs()}
            className="p-2 text-muted-foreground/60 hover:text-muted-foreground rounded-lg hover:bg-secondary"
            title="Refresh"
          >
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {QUICK_FILTERS.map(qf => (
            <button
              ...
              disabled={filtering}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm border border-border rounded-full bg-card hover:bg-secondary hover:border-border transition-colors text-muted-foreground disabled:opacity-60"
            >
              <span>{qf.emoji}</span>
              {qf.label}
            </button>
          ...
          {filtering && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-muted-foreground/60">
        ...
        {/* Filters */}
        <div className="bg-card rounded-xl border border-border border border-border p-4 mb-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {/* Target type */}
            <div>
              <label className="block text-xs font-medium text-muted-foreground/60 mb-1">Target type</label>
              <select
                value={targetType}
            ...
            {/* Action */}
            <div>
              <label className="block text-xs font-medium text-muted-foreground/60 mb-1">Action</label>
              <select
                value={action}
            ...
            {/* From */}
            <div>
              <label className="block text-xs font-medium text-muted-foreground/60 mb-1">From</label>
              <input
                type="datetime-local"
                value={from}
            ...
            {/* To */}
            <div>
              <label className="block text-xs font-medium text-muted-foreground/60 mb-1">To</label>
              <input
                type="datetime-local"
                value={to}
          ...
          <div className="flex items-center gap-3 mt-3">
            <button
              ...
              className="px-4 py-2 bg-background text-foreground text-sm rounded-lg hover:bg-card"
            >
              Apply filters
            </button>
            <button
              onClick={clearFilters}
              className="px-4 py-2 text-sm text-muted-foreground/60 hover:text-foreground"
            >
              Clear
            </button>
            {logs.length > 0 && (
              <span className="text-xs text-muted-foreground/60 ml-auto">{logs.length} entries loaded</span>
            )}
          </div>
        </div>

        {/* Table */}
        <div className="bg-card rounded-xl border border-border border border-border overflow-hidden">
          {loading ? (
            <div className="px-4 py-12 text-center text-muted-foreground/60 text-sm">Loading...</div>
          ) : logs.length === 0 ? (
            <div className="px-4 py-12 text-center text-muted-foreground/60 text-sm">
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-border bg-secondary">
                      <th className="px-4 py-3 text-xs font-semibold text-muted-foreground/60 uppercase tracking-wide">Time</th>
                      <th className="px-4 py-3 text-xs font-semibold text-muted-foreground/60 uppercase tracking-wide">Action</th>
                      <th className="px-4 py-3 text-xs font-semibold text-muted-foreground/60 uppercase tracking-wide">Target</th>
                      <th className="px-4 py-3 text-xs font-semibold text-muted-foreground/60 uppercase tracking-wide">Actor</th>
                      <th className="px-4 py-3 text-xs font-semibold text-muted-foreground/60 uppercase tracking-wide">Status</th>
              ...
              {nextCursor && (
                <div className="px-4 py-4 border-t border-border text-center">
                  <button
                    ...
                    className="px-5 py-2 text-sm text-muted-foreground border border-border rounded-lg hover:bg-secondary disabled:opacity-50"
                  >

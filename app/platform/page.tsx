    ...
    <header className="border-b border-border bg-background/80 backdrop-blur-xl sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1 text-sm">
          ...
          <Link href="/platform" className="text-foreground px-3 py-2 rounded-lg bg-secondary no-underline font-semibold">Platform</Link>
          ...
          <Link href="/teach-with-drivebook" className="ml-2 bg-gradient-to-r from-pink-600 to-violet-600 text-foreground px-5 py-2 rounded-xl font-bold text-sm no-underline hover:from-pink-500 hover:to-violet-500 transition-all">
            Get Started
          </Link>
        </nav>
      </div>
  ...
  return (
    <footer className="border-t border-border py-10 mt-16 text-center text-foreground/40 text-sm">
      <p>
        ...
        <Link href="/privacy" className="hover:text-foreground/60 no-underline transition-colors">Privacy</Link> ·{' '}
        <Link href="/terms" className="hover:text-foreground/60 no-underline transition-colors">Terms</Link>
      </p>
    </footer>
  )
}
...
function PlatformSection({
  ...
  return (
    <section id={id} className="scroll-mt-20 py-14 border-b border-border/60 last:border-0">
...
function ArticleLink({ href, title }: { href: string; title: string }) {
  return (
    <Link href={href} className="flex items-center justify-between p-3.5 rounded-xl border border-border bg-card/40 hover:bg-white/[0.07] hover:border-white/20 transition-all no-underline group mt-2">
      <span className="text-sm text-foreground/70 group-hover:text-foreground transition-colors">{title}</span>
...
export default function PlatformPage() {
  ...
  return (
    <div className="light min-h-screen bg-background text-foreground">
      ...
      <main>
        {/* Hero */}
        <section className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-violet-950 to-slate-950 py-16 md:py-24 px-4 text-center border-b border-border">
          ...
          <div className="relative max-w-3xl mx-auto">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {nav.map(({ id, label }) => (
                <a key={id} href={`#${id}`} className="text-xs px-3 py-1.5 rounded-full bg-secondary/50 border border-border text-white/70 hover:text-white hover:bg-secondary transition-all no-underline">
                  {label}
                </a>
              ))}
            </div>
        ...
        <div className="max-w-4xl mx-auto px-4 py-10">
          ...
          <PlatformSection id="booking" icon={Calendar} accent="bg-gradient-to-br from-violet-600 to-indigo-600" title="Booking" subtitle="How students find you and book lessons">
            <div className="space-y-0 divide-y divide-white/[0.06] rounded-2xl bg-white/[0.04] border border-border px-5 mb-6">
          ...
          <section className="py-14">
            <div className="flex items-start gap-4 mb-8">
              ...
              <div>
                <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-1">Subscription Plans</h2>
                <p className="text-foreground/50 text-sm">Four tiers for every stage of your business</p>
              </div>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              {[
                ...
                { name: 'PREMIUM', for: 'Larger schools & groups', features: ['Everything in STUDIO', 'Unlimited instructors', 'Role-based access', 'Advanced reporting', 'Dedicated support'] },
              ].map(plan => (
                <div key={plan.name} className="bg-white/[0.04] border border-border rounded-2xl p-5 hover:border-violet-500/30 transition-all">
                  <p className="text-xs font-bold text-violet-400 uppercase tracking-wider mb-1">{plan.name}</p>
                  <p className="text-foreground/50 text-xs mb-4 leading-snug">{plan.for}</p>
                  <ul className="space-y-1.5">
                    {plan.features.map(f => (
                      <li key={f} className="flex gap-2 text-xs text-foreground/70">
          ...
          <section className="py-8 border-t border-border/60">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="font-bold text-foreground mb-2">Data Security & Privacy</h3>
          ...
          <section className="py-8 border-t border-border/60">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="font-bold text-foreground mb-2">Notifications</h3>
          ...
          <section className="py-8 border-t border-border/60">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="font-bold text-foreground mb-2">Further Reading</h3>
                <p className="text-foreground/60 text-sm mb-4">Deep-dive guides on specific platform areas.</p>
                <div className="grid sm:grid-cols-2 gap-2">
          ...
          <section className="py-10">
            <div className="rounded-2xl bg-gradient-to-br from-pink-900/60 to-violet-900/60 border border-pink-500/30 p-10 text-center">
              <h2 className="text-2xl font-bold text-foreground mb-3">Start Your Free Trial</h2>
              <p className="text-foreground/60 mb-6 text-sm max-w-md mx-auto">

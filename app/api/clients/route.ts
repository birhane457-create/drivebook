...
export async function POST(req: NextRequest) {
  try {
    ...
    // Prevent duplicate client records for the same provider/email pair.
    // Check both: customers with bookings from this provider, and customers
    // added directly (preferredProviderId) who may not have bookings yet.
    const existingClient = await prisma.customer.findFirst({
      where: {
        email: normalizedEmail,
        OR: [
          { preferredProviderId: session!.user!.providerId },
...
export async function GET(req: NextRequest) {
  try {
    ...
    // A customer belongs to this provider if:
    //   a) they have a booking with this provider (established relationship), OR
    //   b) they were manually added by this provider (preferredProviderId set at creation,

...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!customerId && !bookingId) {
      return NextResponse.json({ error: 'customerId or bookingId required' }, { status: 400 })
    }
    ...
    const booking = await prisma.booking.findFirst({
      ...
      select: {
        ...
        provider: { select: { name: true, businessName: true, subscriptionTier: true, accountType: true } },
      },
    })
    ...
    const instructorDisplayName = getDisplayName(booking.provider)
    const hoursRemaining = Number(booking.packageHoursRemaining ?? 0)
    ...
    await prisma.auditLog.create({
      data: {
        ...
        metadata: {
          ...
          hoursRemaining,
          customerPhone: client.phone.slice(-4).padStart(client.phone.length, '*'), // mask for log

...
// GET â€” list expenses for the authenticated instructor
export async function GET(req: NextRequest) {
  try {
    ...
    const instr = await prisma.provider.findUnique({ where: { id: session!.user!.providerId }, select: { timezone: true, state: true } });
    ...
    const expenses = await (prisma as any).providerExpense.findMany({
      where: {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const instr2 = await prisma.provider.findUnique({ where: { id: session!.user!.providerId }, select: { timezone: true, state: true } });
    ...
    const expense = await (prisma as any).providerExpense.create({
      data: {
        providerId: session!.user!.providerId,
        date: utcDate,

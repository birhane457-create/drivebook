...
export async function GET(req: NextRequest) {
  try {
    ...
    const totalNet = transactions.reduce((sum: number, t: any) => sum + t.providerPayout, 0);
...
${day}
  ...
  Individual Lessons:
${data.transactions.map((t: any) => `    • ${t.customerName} at ${t.time}${t.isFromPackage ? ' (Package)' : ''}

...
export async function PUT(req: NextRequest) {
  try {
    ...
    // Driving-specific fields go to extension table only (D6)
    const drivingUpdate: Record<string, unknown> = {}
    if (data.vehicleTypes !== undefined) drivingUpdate.vehicleTypes = Array.isArray(data.vehicleTypes) ? data.vehicleTypes.join(',') : data.vehicleTypes
    ...
    if (Object.keys(updateData).length === 0 && Object.keys(drivingUpdate).length === 0) {
    ...
    if (Object.keys(drivingUpdate).length > 0) {
      await upsertDrivingProfile(providerId, drivingUpdate as any)
    }

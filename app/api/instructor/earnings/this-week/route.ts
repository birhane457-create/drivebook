  ...
  try {
    ...
    const weeklyBookings = await prisma.booking.findMany({
      where: {
        ...
        // Platform bookings only (not offline) as any[]
        source: { not: 'offline' }
      },
      select: {
        id: true,
        price: true,
        providerPayout: true,   // FIX BUG-1: use net payout, not gross price
    ...
    // FIX BUG-1: sum providerPayout (net after commission), not booking.price (gross)
    // Falls back to price × (1 - commissionRate) for bookings missing providerPayout,
    // and ultimately to price × 0.85 (BASIC rate) for very old bookings.
    const completedCount = weeklyBookings.length
    const totalEarned = weeklyBookings.reduce((sum: any, b: any) => {
      if (b.providerPayout && b.providerPayout > 0) return sum + b.providerPayout
    ...
    return NextResponse.json({
      ...
      // FIX DATA-3: return providerPayout per booking, not gross price
      bookings: weeklyBookings.map((b: any) => {
        const net = b.providerPayout && b.providerPayout > 0
          ? b.providerPayout

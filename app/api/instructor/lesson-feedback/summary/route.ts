...
export async function GET(req: NextRequest) {
  try {
    ...
    const bookingsWithFeedback = await (prisma).booking.findMany({
      where: {
        providerId,
        ...(({ feedbackGivenAt: { not: null } }) as any),
      } as any,
      select: {
        id: true,
        startTime: true,
    ...
    const scoresWithValue = bookingsWithFeedback.filter((b: any) => b.performanceScore !== null);
    const averageScore =
      scoresWithValue.length > 0
        ? Math.round(
            scoresWithValue.reduce((sum: any, b: any) => sum + ((b).performanceScore ?? 0), 0) /
    ...
    bookingsWithFeedback.forEach((b: any) => {
    ...
    const highScoreBookings = bookingsWithFeedback.filter(
      (b: any) => ((b as any).performanceScore ?? 0) >= 85
    );
    const strengthCategories: Record<string, number> = {};
    highScoreBookings.forEach((b: any) => {
    ...
    const recentFeedback = bookingsWithFeedback.slice(0, 20).map((b: any) => ({
      id: b.id,
      bookingId: b.id,

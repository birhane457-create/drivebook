  ...
  try {
    ...
    if (!providerId) {
      return NextResponse.json(
        { error: 'providerId parameter is required' },
        { status: 400 }
      );
    }

    // Fetch instructor (core fields) and platform settings in parallel
    const [instructorBase, pricing] = await Promise.all([
    ...
    // Merge test package fields from extension table

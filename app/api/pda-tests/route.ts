...
export async function POST(req: NextRequest) {
  try {
    ...
    const instructorBase = await prisma.provider.findUnique({
      where: { id: session!.user!.providerId },
      select: { id: true, approvalStatus: true },
    });
    if (!instructorBase || instructorBase.approvalStatus !== 'APPROVED') {
    ...
    const instructorCore = await prisma.provider.findUnique({
      where: { id: session!.user!.providerId },
      select: { id: true },
    });
    if (!instructorCore) return NextResponse.json({ error: 'Instructor not found' }, { status: 404 });
    const instructor = await mergeDrivingProfile(instructorCore.id, instructorCore as any) as any;

    // Verify client belongs to instructor
    const client = await prisma.customer.findFirst({
      where: { id: data.customerId, providerId: session!.user!.providerId },
    });

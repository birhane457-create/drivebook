...
function buildPlanTableRows(): string {
  const tiers: SubscriptionTier[] = ['BASIC', 'PRO', 'STUDIO', 'PREMIUM'];
  return tiers
    .map((tier, i) => {
...
function buildPlanList(): string {
  const tiers: SubscriptionTier[] = ['PRO', 'STUDIO', 'PREMIUM'];
  return tiers
    .map((tier) => {
      const plan = SUBSCRIPTION_PLANS[tier];
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const sub of upcomingExpiries) {
      if (!sub.provider.user?.email || !sub.trialEndsAt) continue;
      ...
      try {
        ...
        console.error(`Failed to send 7-day trial warning to ${sub.provider.name}:`, err);
      }
    }
    ...
    for (const sub of threeDayExpiries) {
      ...
      try {
        ...
        console.error(`Failed to send 3-day trial reminder to ${sub.provider.name}:`, err);
      }
    }
    ...
    for (const sub of recentlyExpired) {
      ...
      try {
        ...
        console.error(`Failed to send expiry notification to ${sub.provider.name}:`, err);
      }
    }

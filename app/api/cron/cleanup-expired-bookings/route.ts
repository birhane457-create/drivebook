...
export async function GET(req: NextRequest) {
  try {
    ...
    // P0-8 FIX: Removed createdBy: 'customer' filter — instructor-created PENDING
    ...
    const noShowBookings = await prisma.booking.findMany({
      ...
      select: {
        id: true,
        provider: { select: { userId: true, name: true } },
        customer: { select: { userId: true } },
        customerName: true,
      },
    });
      ...
      if (b.provider?.userId) {
        try {
          ...
          await sendAlert({
            ...
            entityId: b.id,
            metadata: { bookingId: b.id, instructorUserId: b.provider.userId },
          });
        } catch (alertErr) {

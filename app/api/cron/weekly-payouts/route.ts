...
export async function GET(req: NextRequest) {
  ...
  const DISPUTE_BUFFER_HOURS = (Number(settings?.lateCancellationWindowHours ?? 24)) * 2;
  ...
  try {
    ...
    const providerIds = eligibleTxs.map((t) => t.providerId);
    ...
    const totalEligible = providerIds.length;
    const batchedInstructorIds = providerIds.slice(0, BATCH_SIZE);
    ...
    for (const { providerId } of eligibleTxs.filter(t => batchedInstructorIds.includes(t.providerId))) {
      const inst = instructorMap.get(providerId) as any;

      // ── Skip checks ───────────────────────────────────────────────────────

      // Dispute hold
      if (inst?.payoutHold) {
        results.push({ providerId, status: 'SKIPPED', reason: 'payout_hold_open_dispute' });
        continue;
      }

      // ABN on file but not verified — block payout (47% withholding rule)
      if (inst?.abn && !inst.abnVerified) {
        results.push({ providerId, status: 'SKIPPED', reason: `abn_not_verified (status: ${inst.abnStatus ?? 'PENDING'})` });
        continue;
      }

      // Bank transfer / manual — skip from automatic run, admin handles these
      if (inst?.payoutMethod !== 'stripe_connect') {
        results.push({ providerId, status: 'SKIPPED', reason: `manual_payout_method (${inst?.payoutMethod ?? 'unknown'})` });
        continue;
      }

      // Stripe Connect not set up
      if (!inst?.stripeAccountId) {
        results.push({ providerId, status: 'SKIPPED', reason: 'stripe_connect_not_started' });
      ...
      try {
        ...
        const result = await executeInstructorPayout(providerId, 'SYSTEM_CRON');
        results.push({
          providerId,
        ...
        results.push({ providerId, status: 'FAILED', reason });
        console.error(`[WEEKLY PAYOUTS] Failed for instructor ${providerId}:`, reason);
      }
    }

    // ── Summary ───────────────────────────────────────────────────────────
    const paid    = results.filter((r: any) => r.status === 'PAID').length;
    const pending = results.filter((r: any) => r.status === 'PENDING_TRANSFER').length;
    const failed  = results.filter((r: any) => r.status === 'FAILED').length;
    const skipped = results.filter((r: any) => r.status === 'SKIPPED').length;

    const totalPaid = results
      .filter((r: any) => r.status === 'PAID')
      .reduce((s: any, r: any) => s + ((r as any).netAmount ?? 0), 0);
    ...
    if (failed > 0) {
      const failedList = results
        ...
        .map((r: any) => `${r.providerId}: ${r.reason}`)
        .join('\n');

...
const FIELD_LABELS: Record<string, { label: string; tier: string }> = {
  ...
  studioCommissionRate:   { label: 'Studio tier commission rate',  tier: 'STUDIO' },
  businessCommissionRate: { label: 'Premium tier commission rate', tier: 'PREMIUM' },  // DB column name (legacy) — maps to PREMIUM subscriptionTier
};
...
async function notifyInstructors(
  ...
  // Find all instructors on the affected tier (subscriptionTier, not accountType)
  const tierFilter = fieldInfo.tier === 'BASIC'
    ? { subscriptionTier: 'BASIC' }
    : fieldInfo.tier === 'PRO'
    ? { subscriptionTier: { in: ['PRO'] } }
    : fieldInfo.tier === 'STUDIO'
    ? { subscriptionTier: { in: ['STUDIO'] } }
    : { subscriptionTier: { in: ['PREMIUM', 'BUSINESS'] } }; // PREMIUM tier (BUSINESS = legacy tier value if any old records exist)

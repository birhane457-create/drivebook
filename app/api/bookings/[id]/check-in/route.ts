...
export async function POST(
  ...
  try {
    ...
    const bookingWhere = (userRole === 'provider' && providerId)
      ? { id: bookingId, providerId }
      : { id: bookingId };

    const booking = await prisma.booking.findFirst({
      where: bookingWhere,
      include: {
        provider: { select: { name: true, phone: true, timezone: true, state: true } },
    ...
    const instructorTimezone = booking?.provider
    ...
    const isInstructor = userRole === 'provider';
    ...
    if (isClient && booking.customer.userId !== userId) {
      return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
    }

    // Determine who is checking in
    const checkInBy = isInstructor ? 'provider' : 'customer';
    ...
    const otherPartyPhone = isInstructor ? booking.customer.phone : booking.provider.phone;
    const checkedInName = isInstructor ? booking.provider.name : booking.customer.name;

...
export async function POST(
  ...
  try {
    ...
    const isInstructor = user.provider?.id === booking.providerId;
      ...
      await prisma.auditLog.create({
        data: {
          ...
          metadata: {
            ...
            confirmedBy: isAdmin ? 'admin' : 'provider',
    ...
    try {
      if (notifChannels.sms && booking.customer?.phone && booking.startTime) {
    ...
    const bookingTz = booking.provider?.timezone
      ? resolveTimezone(booking.provider.timezone)
      : timezoneFromState((booking.provider as any)?.state ?? '')

    // Send in-app notification to instructor
    try {
      if (notifChannels.inApp && booking.provider?.userId && booking.startTime) {
    ...
    try {
      if (notifChannels.inApp && booking.customer?.userId && booking.startTime) {
        await notifyClientBookingConfirmed(
          booking.customer.userId,
          getDisplayName(booking.provider),
          bookingId,
          new Date(booking.startTime),

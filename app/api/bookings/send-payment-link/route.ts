...
export async function POST(req: NextRequest) {
  try {
    ...
    const { customerId, topUpAmount, lessonPrice, shortfall, platformFeeRate, lessonDate } = await req.json()

    if (!customerId || !topUpAmount) {
      return NextResponse.json({ error: 'Missing required fields' }, { status: 400 })
    }

    // Verify client belongs to this instructor
    const client = await prisma.customer.findFirst({
      where: { id: customerId, bookings: { some: { providerId: session!.user!.providerId } } }
    })

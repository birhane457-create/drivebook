...
const combinedBookingSchema = z.object({
  customerId: z.string().min(1, 'Client ID required'),
  providerId: z.string().min(1, 'Instructor ID required'),
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (data.pdaTest) {
      ...
      if (config.providerId !== data.providerId) {
        return NextResponse.json(
      ...
      let pdaPrice = Number(config.price)
      if (config.discountPercent) {
        pdaPrice = Number(config.price) * (1 - Number(config.discountPercent) / 100)
        totalDiscount += Number(config.price) - pdaPrice
      }

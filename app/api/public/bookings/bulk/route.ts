...
}).refine(d => d.providerId || d.instructorQuery, {
  message: 'Either providerId or instructorQuery must be provided',
});
...
export async function POST(req: NextRequest) {
  try {
    ...
    const identifier = getRateLimitIdentifier(undefined, ip, `bulk-booking:${data.accountHolderEmail}:${data.providerId || data.instructorQuery}`);
    ...
    if (idempotencyKey) {
      ...
      if (existing) {
        ...
        let resolvedInstructorId = data.providerId;
    ...
    if (!resolvedInstructorId) {
      return NextResponse.json(
        { error: 'providerId or instructorQuery is required' },
        { status: 400 }
      );
    }
    ...
    const customerName = data.registrationType === 'myself' 
      ? data.accountHolderName 
      : data.learnerName || data.accountHolderName;
    
    const customerPhone = data.registrationType === 'myself'
    ...
    const existingClient = await (prisma.customer.findFirst({
      where: { email: data.accountHolderEmail, bookings: { some: { providerId: resolvedInstructorId } } }
    }) as any);
    if (existingClient) {
      ...
      const newClient = await prisma.customer.create({
        data: {
          preferredProviderId: resolvedInstructorId,
          userId: userId,
          name: customerName,
          email: data.accountHolderEmail,
          phone: customerPhone,
      ...
      try {
        ...
        const session = await stripe.checkout.sessions.create({
          ...
          metadata: {
            ...
            expectedTotal: String(verifiedTotal), // AUDIT FIX #1: Validate amount in webhook
          },
      ...
      booking = await prisma.$transaction(async (tx) => {
        ...
        if (newBooking.isPackageBooking && newBooking.price === newBooking.packageTotalPaid) {
          logger.error('🚨 Package price corruption detected', {
            ...
            expectedPrice: firstLessonPrice,
          });
    ...
    if (data.includeTestPackage) {
      try {
        ...
        const instructor_with_pda = await (prisma.provider.findUnique({
          where: { id: resolvedInstructorId },
          select: {
            pdaConfigs: {
              where: { isActive: true },
              ...
              <div class="container">
                ...
                <div class="content">
                  ...
                  ${data.registrationType === 'myself' 
                    ? `<p>You have successfully booked a driving lesson with <strong>${getDisplayName(instructor)}</strong>.</p>`
                    : `<p><strong>${getDisplayName(instructor)}</strong> has booked a driving lesson for you.</p>`
                  }
    ...
    if (isShortNotice) {
      try {
        const instructorUser = instructor.userId
          ...
          : null;
        if (instructorUser) {
          await notifyShortNoticeBookingRequest(instructorUser.id, customerName, booking.id, startTime ?? new Date());
        }
        if (userId) {
      ...
      try {
        const instructorUser = instructor.userId
          ...
          : null;
        if (instructorUser) {
          await notifyBookingRequest(instructorUser.id, customerName, booking.id, startTime ?? new Date());
        }
        if (userId) {

...
export async function POST(
  ...
  try {
    ...
    const lateWindow = Number(pricingSettings?.lateCancellationWindowHours ?? 24)
    ...
    if (hoursUntilBooking >= fullRefundWindow) {
      ...
      refundPercentage = 50
      refundAmount = parseFloat((Number(booking.price) * 0.5).toFixed(2))
    }
    ...
    const cancelTz = booking.provider
      ? booking.provider.timezone
        ? (await import('@/lib/utils/timezone')).resolveTimezone(booking.provider.timezone)
        : (await import('@/lib/utils/timezone')).timezoneFromState(booking.provider.state)

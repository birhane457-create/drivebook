...
export async function GET() {
  try {
    ...
    // Auth boundary fix: do not use session!.user.customerId for data access.
    // customerId in the JWT is always clients[0] — wrong for users who have booked
    ...
    const recentBookings = await (prisma as any).booking.findMany({
      where: {
    ...
    const bookingsWithFeedback = recentBookings.filter(
      (b: any) => (b?.lessonFeedback as number[]).length > 0
    ).slice(0, 5);
    ...
    for (const booking of bookingsWithFeedback) {
      for (const code of ((booking as any).lessonFeedback as number[])) {
    ...
    const content = allContent.filter((item) =>
      (item.pdaCodes as number[]).some((c: any) => uniqueCodes.includes(c))
    );

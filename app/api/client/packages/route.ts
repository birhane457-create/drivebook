  ...
  try {
    ...
    const packages = await Promise.all(packageBookings.map(async (packageBooking) => {
      ...
      return {
        ...
        bookings: childBookings.map((b: any) => ({
          id: b.id,
          date: b.startTime,
          instructor: packageBooking.provider.name,
    ...
    // Calculate summary
    const summary = {
      total: packages.reduce((sum: any, p: any) => sum + p.hoursTotal, 0),
      used: packages.reduce((sum: any, p: any) => sum + p.hoursUsed, 0),
      remaining: packages.reduce((sum: any, p: any) => sum + p.hoursRemaining, 0),

...
async function buildReport(): Promise<WeeklyReport> {
  ...
  const [
    ...
    prisma.customer.count({ where: { createdAt: { gte: weekStart } } }).catch(() => 0),
    prisma.provider.count({ where: { createdAt: { gte: weekStart } }  as any }).catch(() => 0),
    prisma.customer.count().catch(() => 0),
    prisma.provider.count().catch(() => 0),
    prisma.provider.count({ where: { approvalStatus: 'APPROVED', isActive: true } }).catch(() => 0),
    prisma.provider.count({ where: { approvalStatus: 'PENDING' } }).catch(() => 0),
  ...
  if (topInstructorRows.length > 0) {
    ...
    const inst = await (prisma as any).provider.findUnique({
      where: { id: top.providerId },
      select: { name: true },
    }).catch(() => null)
...
export async function POST(req: NextRequest) {
  ...
  const totalIssues = Object.values(report.openIssues).reduce((s: any, v: any) => s + v, 0)
    ...
    <div style="background:#1e293b;border:1px solid #334155;border-radius:16px;padding:20px;margin-bottom:16px;">
      ...
      <table style="width:100%;border-collapse:collapse;">
        <tr>
          ...
          <td style="padding:4px 0;font-size:13px;font-weight:600;color:#f1f5f9;text-align:right;">${report.providers.active}</td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td style="padding:4px 0;font-size:13px;font-weight:600;color:${report.providers.pendingApproval > 0 ? '#fbbf24' : '#f1f5f9'};text-align:right;">${report.providers.pendingApproval}</td>
        </tr>
      </table>
    </div>

...
export async function GET(
  ...
  try {
    ...
    const invoiceTz = transaction.booking?.provider
      ? transaction.booking.provider.timezone
        ? resolveTimezone(transaction.booking.provider.timezone)
        : timezoneFromState(transaction.booking.provider.state)
      : DEFAULT_TIMEZONE;
...
Instructor: ${transaction.booking?.provider?.name || 'N/A'}
Instructor ID: ${transaction.providerId}

${transaction.booking ? `Client: ${transaction.booking.customer?.name || 'N/A'}
Client Email: ${transaction.booking.customer?.email || 'N/A'}
Client Phone: ${transaction.booking.customer?.phone || 'N/A'}` : ''}
...
Instructor Payout:      $${transaction.providerPayout.toFixed(2)} AUD

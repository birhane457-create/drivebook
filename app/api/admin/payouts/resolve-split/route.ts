...
export async function POST(req: NextRequest) {
  try {
    ...
    if (payoutAmount > txn.providerPayout + 0.001) {
      return NextResponse.json(
        { error: `payoutAmount (${payoutAmount}) exceeds instructor payout (${txn.providerPayout})` },
        { status: 422 },
      );
    }

    const wallet = txn.booking?.customer?.wallet;
    ...
    await (prisma as any).$transaction(async (tx: any) => {
      ...
      await tx.transaction.update({
        where: { id: transactionId },
        data: {
          // SETTLED = eligible for payout run; providerPayout capped to approved amount
          status: 'SETTLED',
      ...
      await tx.auditLog.create({
        data: {
          ...
          metadata: {
            ...
            originalInstructorPayout: txn.providerPayout,
            reason,

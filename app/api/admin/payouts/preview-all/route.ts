...
export async function GET(req: NextRequest) {
  try {
    ...
    pendingTransactions.forEach((transaction: any) => {
      ...
      if (!inst) return;
      const providerId = inst.id;
      
      if (!instructorMap.has(providerId)) {
        instructorMap.set(providerId, {
          id: providerId,
          name: inst.name,
          phone: inst.phone,
      ...
      const instructor = instructorMap.get(providerId)!;
      instructor.amount += transaction.providerPayout;
    ...
    const totalAmount = instructors.reduce((sum: any, i: any) => sum + i.amount, 0);

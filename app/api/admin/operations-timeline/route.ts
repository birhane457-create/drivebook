...
export async function GET(req: NextRequest) {
  ...
  if (typeFilter.includes('booking')) {
    ...
    for (const row of bookingRows) {
      ...
      const customerName = (row as any).customer?.name ?? (row as any).customerName ?? 'Unknown student'
      const instructorName = (row as any).provider?.name ?? 'Unknown instructor'
      events.push({
        id: `booking-${row.id}`,
        type: cfg.type,
        severity: cfg.severity,
        title: cfg.title,
        detail: `${customerName} with ${instructorName}${(row as any).price ? ` · $${(row as any).price.toFixed(0)}` : ''}`,
  ...
  if (typeFilter.includes('payout')) {
    ...
    for (const row of payoutRows) {
      ...
      const instructorName = row.provider?.name ?? 'Unknown'
  ...
  if (typeFilter.includes('dispute')) {
    ...
    for (const row of disputeRows) {
      ...
      events.push({
        ...
        detail: `${amount} · ${((row as any).reason ?? 'unknown').replace(/_/g, ' ')}`,
        actorName: null,
        link: '/admin/disputes',
        timestamp: row.updatedAt.toISOString(),
        source: 'dispute',

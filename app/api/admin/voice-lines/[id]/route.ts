...
const actionSchema = z.object({
  ...
  providerId: z.string().optional(), // required for assign
})
 ...
 *
 * assign:     assign this number to an instructor (providerId required)
...
export async function PATCH(
  ...
  const { action, providerId } = parsed.data
  ...
  try {
    switch (action) {
      case 'assign': {
        if (!providerId) {
          return NextResponse.json({ error: 'providerId is required for assign' }, { status: 400 })
        }
  ...
  const updated = await prisma.twilioPhoneNumber.findUnique({
    where: { id: numberId },
    include: {
      provider: { select: { id: true, name: true, subscriptionTier: true } },
    },
  })

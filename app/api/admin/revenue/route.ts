...
export async function GET(req: NextRequest) {
  try {
     ...
     * so we filter on transaction status only â€” not booking status.
    ...
    const [
      ...
      prisma.transaction.aggregate({ where: commissionWhere, _sum: { platformFee: true, amount: true, providerPayout: true }, _count: { id: true } }),
      prisma.transaction.aggregate({ where: commissionWhereAllTime, _sum: { platformFee: true, amount: true, providerPayout: true }, _count: { id: true } }),
      ...
      prisma.transaction.aggregate({ where: { status: 'PENDING', type: 'BOOKING_PAYMENT' }, _sum: { providerPayout: true } }),
      prisma.transaction.aggregate({ where: { status: 'COMPLETED', type: 'BOOKING_PAYMENT' }, _sum: { providerPayout: true } }),
    ...
    for (const t of eligibleTxns) {
      ...
      const e = instrAgg.get(t.providerId) || { payout: 0, fee: 0, gross: 0, count: 0 };
      e.payout += Number(t.providerPayout) || 0;
      e.fee += Number(t.platformFee) || 0;
      e.gross += Number(t.amount) || 0;
      e.count += 1;
      instrAgg.set(t.providerId, e);
    }

    const providerIds = Array.from(instrAgg.keys());
    const instructors = await prisma.provider.findMany({
      where: { id: { in: providerIds } },
      select: { id: true, name: true },
    });
    const instrMap = new Map(instructors.map((i: any) => [i.id, i.name]));
    ...
    // Old approach: 6 Ã— ~50ms = ~300ms serial. New: ~50ms total (one round-trip per query, all concurrent).
    const monthRanges = Array.from({ length: 6 }, (_, idx) => {
      const i = 5 - idx; // i = 5,4,3,2,1,0 â†’ oldest to most recent
      return {
        i,
    ...
    const monthAggs = await Promise.all(
      monthRanges.map(({ mStart, mEnd }) =>
        (prisma as any).transaction.aggregate({
          ...
          _sum: { platformFee: true, amount: true, providerPayout: true },
          _count: { id: true },
        })
      )
    );
    const revenueByMonth = monthRanges.map(({ mStart }, idx) => {
      ...
      return {
        ...
        providerPayout: agg._sum.providerPayout || 0,
    ...
    const recentTransactions = await (prisma as any).transaction.findMany({
      ...
      include: {
        booking: {
          select: {
            customerName: true, startTime: true, status: true,
    ...
    const refundedTransactions = await (prisma as any).transaction.findMany({
      ...
      include: {
        booking: {
          select: {
            customerName: true, startTime: true,
            provider: { select: { id: true, name: true } },
          },
        },
      },
    ...
    return NextResponse.json({
      ...
      rangeInstructorPayout: commissionAgg._sum.providerPayout || 0,
      ...
      totalInstructorPayouts: commissionAllTime._sum.providerPayout || 0,
      ...
      // Payouts
      pendingPayouts: pendingAgg._sum.providerPayout || 0,
      completedPayouts: completedPayoutsAgg._sum.providerPayout || 0,
      ...
      // Platform ledger â€” real-time balance

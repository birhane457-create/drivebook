...
export async function GET() {
  try {
    ...
    const deny = await requirePermission(session, PERM.OPERATIONS_EXTENSIONS_VIEW);
    if (deny) return deny;
...
export async function POST(req: NextRequest) {
  try {
    const session = await getServerSession(authOptions);
    const deny = await requirePermission(session, PERM.OPERATIONS_EXTENSIONS_MANAGE);
    if (deny) return deny;

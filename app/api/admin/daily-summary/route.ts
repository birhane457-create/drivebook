...
import { SignalSeverity } from '@/lib/types/signal'
  ...
  try {
    const [
      ...
      prisma.booking.findMany({
        ...
        select: { id: true, price: true, createdAt: true, customer: { select: { name: true } }, provider: { select: { name: true } } },
        take: 20,
      }).catch(() => []),
      ...
      prisma.payout.findMany({
        ...
        select: { id: true, netAmount: true, createdAt: true, providerId: true },
        take: 10,
      }).catch(() => []),
    ...
    if (stripeIncomplete.length > 0) {
      ...
      const blockedAgg = await prisma.transaction.aggregate({
        where: {
          providerId: { in: incompleteIds },
          createdAt: { gte: last30Days },
          status: { in: ['COMPLETED', 'SETTLED'] },
        },
        _sum: { providerPayout: true },
      }).catch(() => ({ _sum: { providerPayout: 0 } }))
      stripeBlockedAmount = Number(blockedAgg._sum?.providerPayout ?? 0)
    }

    // ── Top performers ────────────────────────────────────────────────────────
    const topInstructorIds = topInstructors.map((t: any) => t.providerId).filter(Boolean)
    ...
    const topPerformers = topInstructors.map((t: any) => ({
      name: topInstructorDetails.find((i: any) => i.id === t.providerId)?.name ?? 'Unknown',
    ...
    type AttentionItem = {
      type: string
      severity: SignalSeverity  // ← Now enforces canonical enum
      message: string
      link: string
      count?: number

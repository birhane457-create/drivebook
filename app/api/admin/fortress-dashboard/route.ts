...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalRevenue = weeklyBookings.reduce((s: any, b: any) => s + b.price, 0);
    const totalPlatformFees = weeklyBookings.reduce((s: any, b: any) => s + b.platformFee, 0);
    const completedCount = weeklyBookings.filter((b: any) => b.status === 'COMPLETED').length;
    ...
    const totalRefunds = Math.abs(Number(refundEntries._sum?.amount ?? 0));
    ...
    const frozenPayouts = await prisma.provider.count({
      where: { payoutHold: true }  as any,
    });

    // ── Instructor health ─────────────────────────────────────────────────────
    const totalInstructors = await prisma.provider.count({ where: { isActive: true } });
    const pendingApproval = await prisma.provider.count({ where: { approvalStatus: 'PENDING' } });
    const activeSubscriptions = await prisma.provider.count({
      where: { subscriptionStatus: { in: ['ACTIVE', 'TRIAL'] } },
    });
    const expiringDocuments = await prisma.provider.count({
      where: {
        OR: [
    ...
    const bookingByStatus = Object.fromEntries(
      bookingStats.map((b: any) => [b.status, b._count])
    );
    ...
    return NextResponse.json({
      ...
      financial: {
        ...
        disputes: {
          ...
          lostAmountThisWeek: Math.round((Number(lostDisputes._sum?.amount ?? 0)) * 100) / 100,
        },
      },
      ...
      payouts: {
        ...
        amountPaidThisMonth: Math.round((Number(payoutsThisMonth._sum?.netAmount ?? 0)) * 100) / 100,
        failedPayouts,
      ...
      overallStatus: {
        ...
        providers: pendingApproval === 0 && expiringDocuments === 0 ? 'HEALTHY' : 'WARNING',
      },
    });
  } catch (error) {

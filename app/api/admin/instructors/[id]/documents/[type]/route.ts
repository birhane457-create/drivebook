...
export async function GET(
  ...
  try {
    ...
    const { id: providerId, type: docType } = params;

    if (!FIELD_TO_SUBFOLDER[docType]) {
      return NextResponse.json({ error: 'Unknown document type' }, { status: 400 });
    }

    // Fetch the stored value — from extension table (falls back to Instructor col)
    const { getDrivingProfile } = await import('@/lib/extensions/driving/providerProfile');
    const profile = await getDrivingProfile(providerId);

    if (!profile) {
      return NextResponse.json({ error: 'Instructor not found' }, { status: 404 });
    }

    const storedValue: string | null = (profile as any)[docType];
    if (!storedValue) {

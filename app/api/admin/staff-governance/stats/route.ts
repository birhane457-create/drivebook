...
export async function GET() {
  try {
    ...
    const pendingApprovals = await prisma.provider.count({
    ...
    const totalRevenueAmt = Number(totalRevenue._sum.platformFee ?? 0);
    const totalRefundsAmt = Number(totalRefunds._sum.amount ?? 0);
    ...
    const expiredDocs = await prisma.provider.count({
      where: {
        approvalStatus: 'APPROVED',
        OR: [
          { /* licenseExpiry moved to DrivingProviderProfile */ },
          { /* insuranceExpiry moved to DrivingProviderProfile */ },
          { /* policeCheckExpiry moved to DrivingProviderProfile */ },
          { /* wwcCheckExpiry moved to DrivingProviderProfile */ },
        ],
      }  as any,
    });

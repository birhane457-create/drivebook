...
function csvResponse(filename: string, header: string[], rows: unknown[][]): NextResponse {
  const lines = [header.join(','), ...rows.map((r: any) => row(r))].join('\n');
...
export async function GET(req: NextRequest) {
  ...
  if (type === 'bookings') {
    ...
    const bookings = await prisma.booking.findMany({
      ...
      select: {
        ...
        pickupAddress: true,
        bookingType: true,
        provider: { select: { name: true, suburb: true, state: true } },
  ...
  if (type === 'revenue') {
    ...
    const header = [
      ...
      'Booking ID', 'Booking Status', 'provider', 'Suburb', 'State',
    ];
    ...
    const rows = transactions.map((t: any) => [
      ...
      t.booking?.provider?.suburb ?? '',
      t.booking?.provider?.state ?? '',
    ]);

    // Summary row at top
    const totalGross   = transactions.reduce((s: number, t: any) => s + ((t as any).amount ?? 0), 0);
    const totalFee     = transactions.reduce((s: number, t: any) => s + ((t as any).platformFee ?? 0), 0);
    const totalPayout  = transactions.reduce((s: number, t: any) => s + ((t as any).providerPayout ?? 0), 0);
  ...
  if (type === 'instructors') {
    ...
    const rows = (instructors as any[]).map((i: any) => [
      i.id,
      i.name,
      i.user?.email ?? '',
      i.phone,

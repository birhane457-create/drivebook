...
export async function GET(req: NextRequest) {
  ...
  const deny = await requirePermission(session, PERM.USERS_PROVIDERS_VIEW)
  if (deny) return deny
  ...
  if (instructors.length === 0) {
    return NextResponse.json({ providers: [], generatedAt: now.toISOString() })
  }
  ...
  const [bookingsThisWeek, bookingsLastWeek] = await Promise.all([
    prisma.booking.groupBy({
      by: ['providerId'],
      where: { providerId: { in: ids }, createdAt: { gte: last7 }, deletedAt: null } as any,
      _count: { id: true },
    }).catch(() => []),
    prisma.booking.groupBy({
      by: ['providerId'],
      where: { providerId: { in: ids }, createdAt: { gte: prev7, lt: last7 }, deletedAt: null } as any,
      _count: { id: true },
    }).catch(() => []),
  ])
  ...
  type BookingCount = { providerId: string; status: string; _count: { id: number } }

  const bookingMap: Record<string, Record<string, number>> = {}
  for (const row of bookings30d as BookingCount[]) {
    if (!bookingMap[row.providerId]) bookingMap[row.providerId] = {}
    bookingMap[row.providerId][row.status] = row._count.id
  }

  const thisWeekMap: Record<string, number> = {}
  for (const row of bookingsThisWeek as { providerId: string; _count: { id: number } }[]) {
    thisWeekMap[row.providerId] = row._count.id
  }

  const lastWeekMap: Record<string, number> = {}
  for (const row of bookingsLastWeek as { providerId: string; _count: { id: number } }[]) {
    lastWeekMap[row.providerId] = row._count.id
  }

  const disputeMap: Record<string, number> = {}
  for (const row of openDisputes as { providerId: string; _count: { id: number } }[]) {
    if (row.providerId) disputeMap[row.providerId] = row._count.id
  }
  ...
  const results = instructors.map((instructor: any) => {
    const counts = bookingMap[instructor.id] ?? {}
    const completed = counts['COMPLETED'] ?? 0
    const cancelled = counts['CANCELLED'] ?? 0
    const noShow = counts['NO_SHOW'] ?? 0
    const total = Object.values(counts).reduce((s: any, n: any) => s + n, 0)

        ...
        <div className="flex border-b border-border">
          <button
            ...
            className={`flex-1 px-6 py-4 font-semibold transition ${
              activeTab === 'pending'
                ? 'border-b-2 border-blue-500 text-primary bg-blue-900/20'
            ...
            pendingReviews.length > 0 ? (
              <div className="space-y-4">
                {pendingReviews.map((pending) => (
                  <div key={pending.id} className="border border-border rounded-lg p-4 hover:shadow-md transition">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-semibold text-foreground">{pending.instructorName}</h3>
                        <p className="text-sm text-muted-foreground/60 flex items-center gap-1 mt-1">
                      ...
                      <button 
                        ...
                        className="px-4 py-2 bg-primary text-foreground rounded-lg hover:bg-primary/90 transition font-semibold"
                      >
                        Write Review
                      </button>
                    </div>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-muted-foreground/60 mt-2">Complete a lesson to leave a review</p>
              </div>
            )
          ) : (
            reviews.length > 0 ? (
                ...
                {reviews.map((review) => (
                  <div key={review.id} className="border border-border rounded-lg p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h3 className="font-semibold text-foreground">{review.instructorName}</h3>
                        <div className="flex items-center gap-1 mt-1">
                          {[...Array(5)].map((_, i) => (
                            <Star
                              key={i}
                              className={`w-4 h-4 ${
                                i < review.rating ? 'text-yellow-400 fill-yellow-400' : 'text-foreground'
                              }`}
                            />
                          ))}
                        </div>
                    ...
                    <p className="text-sm text-muted-foreground/60 mt-2 flex items-center gap-1">
                      <Calendar className="w-4 h-4" />
                      {review.bookingDate
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-muted-foreground/60 mt-2">Your reviews will appear here after you submit them</p>
              </div>
            )
          )}
        </div>

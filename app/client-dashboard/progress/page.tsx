...
const ASSESSMENT_BADGE: Record<string, { label: string; icon: string; bg: string; text: string }> = {
  ...
  MOCK:     { label: 'Mock Assessment',   icon: '🎯', bg: 'bg-blue-900/40',    text: 'text-primary' },
...
export default function ClientProgressPage() {
    ...
    return (
      <div className="min-h-screen bg-background text-foreground pb-24">
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <p className="text-muted-foreground mb-8">Track your learning journey based on instructor feedback</p>
          <div className="bg-card/60 rounded-2xl border border-border p-12 text-center">
            <TrendingUp className="h-16 w-16 text-muted-foreground mx-auto mb-4" />
            <h3 className="text-lg font-semibold mb-2">No feedback yet</h3>
            <p className="text-muted-foreground text-sm mb-6">Complete lessons and your instructor will add coaching notes here.</p>
            <Link href="/client-dashboard/bookings" className="inline-flex items-center gap-2 px-5 py-2.5 bg-secondary rounded-lg text-sm font-semibold hover:bg-secondary/70 border border-border transition">
  ...
  return (
    <div className="min-h-screen bg-background text-foreground pb-24">
      <div className="max-w-4xl mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-2">Your Progress</h1>
        <p className="text-muted-foreground mb-6">Your learning journey with instructor coaching notes and assessments</p>

        {/* Stats row */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          {[
            { label: 'Total Lessons', value: data.totalLessons, icon: <BookOpen className="h-4 w-4 text-primary" /> },
            { label: 'With Feedback', value: data.lessonsWithFeedback, icon: <Star className="h-4 w-4 text-yellow-400" /> },
            { label: 'Mock Tests', value: mockCount, icon: <Target className="h-4 w-4 text-primary" /> },
            {
            ...
            <div key={label} className="bg-card/60 rounded-2xl border border-border p-4">
        ...
        {latestMock && (
          <div className={`mb-6 rounded-2xl border p-5 ${
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-muted-foreground mb-0.5">Latest Mock Assessment</p>
                <p className="text-sm text-foreground">{new Date(latestMock.date).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' })} · {latestMock.provider}</p>
              </div>
              <div className="text-right">
                {latestMock.performanceScore !== null && (
                  <p className={`text-3xl font-bold ${
                    latestMock.performanceScore >= 80 ? 'text-emerald-400' :
                    latestMock.performanceScore >= 65 ? 'text-yellow-400' : 'text-destructive'
                  }`}>{latestMock.performanceScore}%</p>
                )}
                {latestMock.passed !== null && (
                  <p className={`text-sm font-bold mt-0.5 ${latestMock.passed ? 'text-emerald-400' : 'text-amber-400'}`}>
        ...
        {data.progressChart.length > 0 && (
          <div className="bg-card/60 rounded-2xl border border-border p-5 mb-8">
            ...
            <div className="space-y-2.5">
              {data.progressChart.map(item => (
                <div key={item.lesson} className="flex items-center gap-3">
                  <span className="text-xs text-muted-foreground/60 w-10 flex-shrink-0">{item.date}</span>
                  {item.score !== null ? (
                    <>
                      ...
                      <span className={`text-sm font-bold w-12 text-right ${
                        item.score >= 80 ? 'text-emerald-400' : item.score >= 65 ? 'text-yellow-400' : 'text-destructive'
                      }`}>{item.score}%</span>
                      {item.passed !== null && (
              ...
              <div className="bg-card/60 rounded-2xl border border-border p-5">
                ...
                <div className="space-y-1.5">
                  {data.strengths.map((s, i) => (
                    <div key={i} className="flex items-center gap-2 p-2 bg-green-900/20 border border-green-700/30 rounded-lg text-sm text-foreground">
              ...
              <div className="bg-card/60 rounded-2xl border border-border p-5">
                ...
                <div className="space-y-1.5">
                  {data.focusAreas.map((a, i) => (
                    <div key={i} className="flex items-center gap-2 p-2 bg-amber-900/20 border border-amber-700/30 rounded-lg text-sm text-foreground">
        ...
        <div>
          ...
          <div className="space-y-3">
            {data.recentFeedback.map(item => {
              ...
              return (
                <div key={item.id} className="bg-card/60 rounded-2xl border border-border overflow-hidden">
                  <button
                    onClick={() => hasDetail ? setExpandedId(isExpanded ? null : item.id) : undefined}
                    className={`w-full flex items-start gap-3 p-4 text-left ${hasDetail ? 'hover:bg-secondary/40 transition-colors' : ''}`}
                  >
                    {/* Date column */}
                      ...
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        {item.assessmentType !== 'COACHING' && item.performanceScore !== null && (
                          <span className={`text-sm font-bold ${
                            item.performanceScore >= 80 ? 'text-emerald-400' :
                            item.performanceScore >= 65 ? 'text-yellow-400' : 'text-destructive'
                          }`}>{item.performanceScore}%</span>
                        )}
                        {item.passed !== null && (
                          <span className={`text-xs font-semibold ${item.passed ? 'text-emerald-400' : 'text-amber-400'}`}>
                    ...
                    {hasDetail && (
                      isExpanded
                        ? <ChevronUp className="h-4 w-4 text-muted-foreground/60 flex-shrink-0 mt-1" />
                        : <ChevronDown className="h-4 w-4 text-muted-foreground/60 flex-shrink-0 mt-1" />
                    )}
                  </button>
                  ...
                  {isExpanded && hasDetail && (
                    <div className="px-4 pb-4 space-y-2 border-t border-white/5 pt-3">
                      {item.feedback.length > 0 && (
                        <div>
                          <p className="text-xs text-muted-foreground/60 mb-1">Areas to work on:</p>
                      ...
                      {item.notes && (
                        <div>
                          <p className="text-xs text-muted-foreground/60 mb-1">Instructor notes:</p>
                          <p className="text-xs text-foreground leading-relaxed whitespace-pre-line">{item.notes}</p>
                        </div>
                      )}
                    </div>
                  )}
        ...
        {/* Footer note */}
        <div className="mt-8 p-4 bg-card/40 border border-white/5 rounded-2xl">
          <p className="text-xs text-muted-foreground/60 leading-relaxed">
            <span className="font-semibold text-muted-foreground">🟢 Coaching lessons</span> show topics covered and coaching notes — no score, just progress.
            {' '}<span className="font-semibold text-muted-foreground">🎯 Mock assessments</span> show a formal score like the real PDA test.

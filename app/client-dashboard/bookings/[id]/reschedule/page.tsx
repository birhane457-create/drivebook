...
interface BookingInfo {
  ...
  provider: { id: string; name: string; hourlyRate: number };
}
...
export default function ClientReschedulePage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/availability/slots?providerId=${booking.provider.id}&date=${selectedDate}&duration=${durationMins}&excludeBookingId=${bookingId}&bypassDurationCheck=true`)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-muted-foreground/60 text-sm">Redirecting to your booking...</p>
        </div>
      </div>
    );
  }
  ...
  const newPrice = selectedTime
    ? (booking.provider.hourlyRate * durationMins / 60)
    : null;
      ...
      <header className="bg-white/95 border-b border-gray-200 sticky top-0 z-10 backdrop-blur-sm">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground/60">{booking.provider.name}</p>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
        ...
        {selectedDate && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            ...
            {slotsLoading ? (
              ...
              <p className="text-sm text-muted-foreground/60 text-center py-6">No available slots on this day.</p>
            ) : (
                  ...
                  <button
                    ...
                    className={`py-2.5 rounded-lg text-sm font-medium border transition ${
                      selectedTime === slot.time
                        ? 'bg-primary text-foreground border-blue-600'
        ...
        {selectedDate && selectedTime && (
          <button
            ...
            disabled={submitting}
            className="w-full flex items-center justify-center gap-2 py-3.5 bg-primary text-foreground font-semibold rounded-xl hover:bg-primary/90 transition disabled:opacity-60"
          >
            {submitting

    ...
    <div className="min-h-screen bg-background text-foreground pb-24">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">Find answers or get in touch with support</p>
        </div>
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm bg-card text-foreground placeholder-slate-500"
          />
        </div>
        ...
        {!search && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
            <Link href="/client-dashboard/book-lesson" className="flex flex-col items-center gap-2 p-4 bg-card rounded-xl border border-border hover:border-blue-500/60 hover:bg-blue-900/20 transition text-center">
              <BookOpen className="w-6 h-6 text-primary" />
              <span className="text-xs font-medium text-foreground">Book a Lesson</span>
            </Link>
            <Link href="/client-dashboard/wallet" className="flex flex-col items-center gap-2 p-4 bg-card rounded-xl border border-border hover:border-blue-500/60 hover:bg-blue-900/20 transition text-center">
              <Wallet className="w-6 h-6 text-primary" />
              <span className="text-xs font-medium text-foreground">My Wallet</span>
            </Link>
            <Link href="/client-dashboard/bookings" className="flex flex-col items-center gap-2 p-4 bg-card rounded-xl border border-border hover:border-blue-500/60 hover:bg-blue-900/20 transition text-center">
              <Calendar className="w-6 h-6 text-primary" />
              <span className="text-xs font-medium text-foreground">My Bookings</span>
            </Link>
            <Link href="/client-dashboard/progress" className="flex flex-col items-center gap-2 p-4 bg-card rounded-xl border border-border hover:border-blue-500/60 hover:bg-blue-900/20 transition text-center">
              <TrendingUp className="w-6 h-6 text-primary" />
              <span className="text-xs font-medium text-foreground">My Progress</span>
            </Link>
          </div>
        )}
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium transition ${
                !activeCategory ? 'bg-primary text-foreground' : 'bg-secondary text-foreground hover:bg-secondary/70'
              }`}
            >
              All
            </button>
            {faqs.map(cat => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-medium transition ${
                  activeCategory === cat.category ? 'bg-primary text-foreground' : 'bg-secondary text-foreground hover:bg-secondary/70'
                }`}
              >
                {cat.category}
              </button>
        ...
        <div className="space-y-4 mb-10">
          {displayed.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm mt-1 text-muted-foreground/60">Try different keywords or contact support below.</p>
            </div>
          ) : (
            displayed.map(cat => {
              const Icon = cat.icon;
              return (
                <div key={cat.category} className="bg-card rounded-xl border border-border overflow-hidden">
                  <div className="flex items-center gap-2 px-5 py-3 bg-secondary/60 border-b border-border">
        ...
        <div className="bg-blue-900/20 border border-blue-700/40 rounded-xl p-6">
          <h2 className="text-lg font-bold text-foreground mb-1">Still need help?</h2>
          <p className="text-sm text-muted-foreground mb-5">Our support team is available Mon–Fri, 9 AM – 6 PM.</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <a href="mailto:support@drivebook.com.au"
              className="flex items-center gap-3 p-4 bg-card rounded-xl border border-border hover:border-blue-500/60 transition">
              <Mail className="w-5 h-5 text-primary shrink-0" />
              <span className="flex flex-col">
                <span className="text-xs font-semibold text-foreground">Email</span>
                <span className="text-xs text-muted-foreground/60">support@drivebook.com.au</span>
              </span>
            </a>
            <a href="tel:1800DRIVEBOOK"
              ...
              <span className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground/60">1800 DRIVEBOOK</span>
              </span>
            </a>
            <a href="#"
              className="flex items-center gap-3 p-4 bg-card rounded-xl border border-border hover:border-blue-500/60 transition">
              ...
              <span className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground/60">Available 9 AM – 6 PM</span>
              </span>
            </a>
          </div>
        </div>

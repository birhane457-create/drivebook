...
export async function generateMetadata({
  ...
  const pageUrl = `${BASE_URL}/driving-lessons/${params.state}/${params.suburb}`;
  const ogImage = `${BASE_URL}/og-driving-lessons.jpg`; // TODO: Create this image
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{
        ...
        width: 1200,
        height: 630,
        alt: `Driving lessons in ${suburbData.displayName}, ${stateData.code}`,
      }],
    },
    twitter: {
...
export default async function SuburbDrivingLessonsPage({
  ...
  const nearby = hasLocal ? [] : await prisma.provider.findMany({
    where: {
      approvalStatus: 'APPROVED',
      isActive: true,
      state: stateData.code,
    ...
    select: {
      id: true, name: true, businessName: true, bio: true, profileImage: true,
      hourlyRate: true, averageRating: true, totalReviews: true, serviceAreas: true, customSlug: true,
    },
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-white">
        {/* Header */}
        <div className="bg-blue-700 text-foreground py-16 px-4">
          <div className="max-w-4xl mx-auto">
            <nav className="text-blue-200 text-sm mb-4 flex flex-wrap gap-1 items-center">
              <Link href="/" className="hover:text-foreground">Home</Link>
              <span>›</span>
              <Link href={`/driving-lessons/${params.state}`} className="hover:text-foreground">
                {stateData.displayName}
              </Link>
              <span>›</span>
              <span className="text-foreground">{suburbData.displayName}</span>
            </nav>
          ...
          {displayInstructors.length > 0 ? (
            <>
              ...
              <div className="space-y-4">
                {displayInstructors.map((inst: any) => {
                  const slug = inst.customSlug || inst.id;
                    ...
                    >
                      {inst.profileImage ? (
                        ...
                        <div className="w-[72px] h-[72px] rounded-full bg-blue-100 flex items-center justify-center text-blue-600 text-xl font-bold shrink-0">
                          {getDisplayName(inst)[0]}
                        </div>
                      )}
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-baseline gap-2 mb-1">
                          <h3 className="font-semibold text-gray-900">{getDisplayName(inst)}</h3>
                        ...
                        {inst.bio && (
                          <p className="text-sm text-muted-foreground/60 line-clamp-2">{inst.bio}</p>
                        )}
                      </div>
              ...
              <div className="mt-8 text-center">
                <Link
                  href={`/book?location=${encodeURIComponent(suburbData.displayName)}`}
                  className="bg-primary text-foreground font-semibold px-8 py-3 rounded-lg hover:bg-primary/90 transition-colors inline-block"
                >
            ...
            <div className="text-center py-16">
              ...
              <Link
                href="/book"
                className="bg-primary text-foreground font-semibold px-6 py-3 rounded-lg hover:bg-primary/90 transition-colors inline-block"
              >

...
export default async function StateDrivingLessonsPage({
  ...
  const instructorCounts = await prisma.provider.groupBy({
    by: ['suburb'],
    where: {
  ...
  const countMap = new Map<string, number>(
    instructorCounts.map((r: any) => [r.suburb!.toLowerCase(), r._count.id])
  );

  // Total instructors in this state
  const totalInstructors = instructorCounts.reduce((sum: any, r: any) => sum + r._count.id, 0);
        ...
        <div className="bg-blue-700 text-foreground py-16 px-4">
          <div className="max-w-4xl mx-auto">
            <nav className="text-blue-200 text-sm mb-4">
              ...
              {' › '}
              <span className="text-foreground">{stateData.displayName}</span>
            </nav>
                ...
                <Link
                  ...
                  className={`group border rounded-xl px-4 py-3 transition-colors text-sm font-medium ${
                    suburb.count > 0
                      ...
                      : 'border-gray-200 bg-gray-50 text-muted-foreground cursor-default pointer-events-none'
                  }`}
                >

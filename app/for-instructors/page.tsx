    ...
    <header className="border-b border-border bg-background/80 backdrop-blur-xl sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1 text-sm">
          ...
          <Link href="/for-instructors" className="text-foreground px-3 py-2 rounded-lg bg-secondary no-underline font-semibold">For Instructors</Link>
...
function ArticleLink({ href, title }: { href: string; title: string }) {
  return (
    <Link href={href} className="flex items-center justify-between p-3.5 rounded-xl border border-border bg-card/40 hover:bg-white/[0.07] hover:border-white/20 transition-all no-underline group">
      <span className="text-sm text-foreground/80 group-hover:text-foreground transition-colors">{title}</span>
      <ChevronRight className="h-4 w-4 text-foreground/30 group-hover:text-pink-400 shrink-0 ml-3 transition-colors" />
    </Link>
  )
}
...
function TopicCard({ icon: Icon, title, colour, articles }: { icon: React.ElementType; title: string; colour: string; articles: { href: string; title: string }[] }) {
  return (
    <div className="bg-white/[0.04] border border-border rounded-2xl p-6 hover:border-white/20 transition-all">
  ...
  return (
    <div className="light min-h-screen bg-background text-foreground">
      ...
      <main>
        ...
        <section className="relative overflow-hidden bg-gradient-to-br from-pink-900 via-violet-900 to-slate-950 py-20 md:py-28 px-4 text-center">
          ...
          <div className="relative max-w-3xl mx-auto">
            <nav className="flex items-center justify-center gap-2 text-xs text-white/50 mb-6">
              ...
              <span>/</span>
              <span className="text-white/70">For Instructors</span>
            </nav>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <Link href="/platform" className="bg-secondary hover:bg-white/20 text-white px-8 py-3.5 rounded-xl font-semibold no-underline transition-all border border-border">
                Platform Overview →
              </Link>
            </div>
          </div>
        ...
        <section className="bg-card/40 border-y border-border py-8 px-4 overflow-x-auto">
          <div className="max-w-5xl mx-auto flex items-center justify-around gap-6 min-w-max md:min-w-0">
            {[
              ...
              <Link key={label} href={href} className="flex flex-col items-center gap-2 text-center no-underline group">
                <span className="w-10 h-10 rounded-xl bg-secondary group-hover:bg-pink-500/20 group-hover:border-pink-500/30 border border-transparent flex items-center justify-center transition-all">
                  <Icon className="h-4 w-4 text-pink-400" />
                </span>
                <span className="text-foreground/60 group-hover:text-foreground text-xs whitespace-nowrap transition-colors">{label}</span>
              </Link>
            ))}
          </div>
        </section>
        ...
        <section className="max-w-5xl mx-auto px-4 py-16 md:py-20">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-3">Everything You Need to Grow</h2>
            <p className="text-foreground/50">Browse by topic or read from top to bottom — every guide is practical and built for Australian driving instructors.</p>
          </div>
        ...
        <section className="max-w-5xl mx-auto px-4 pb-16">
          <div className="rounded-2xl bg-gradient-to-r from-pink-900/40 to-violet-900/40 border border-pink-500/20 p-8 md:p-10 flex flex-col md:flex-row items-center gap-8">
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Link href="/platform" className="bg-secondary hover:bg-white/20 text-white px-6 py-3 rounded-xl font-semibold no-underline transition-all border border-border text-sm">
                  See the Platform →
                </Link>
              </div>
            </div>
          ...
          <div className="mt-6 pt-6 border-t border-border">
            <p className="text-white/50 text-xs mb-3">Comparing options?</p>
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <Link key={href} href={href} className="text-xs px-3 py-1.5 rounded-full border border-border bg-card/40 text-foreground/50 hover:text-white hover:bg-secondary no-underline transition-all">
                  DriveBook {label} →
                </Link>
              ))}
            </div>

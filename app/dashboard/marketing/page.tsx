...
function resolveFlyer(instructor: InstructorProfile): {
  ...
  if (tier === 'PREMIUM' || accountType === 'BUSINESS') {
    // Premium/Business: business name is the whole brand. Personal name secondary.
    return {
...
function Flyer({ instructor, url, style }: {
  ...
  if (style === 'dark') {
    return (
      <div
        id="flyer-print-area"
        className="bg-card text-foreground rounded-2xl overflow-hidden shadow-2xl w-[320px] mx-auto print:shadow-none"
      ...
      >
        ...
        <div className="px-6 pt-6 pb-4 text-center">
          ...
          {subline && (
            <p className="text-sm text-primary mt-1">{subline}</p>
          )}
          ...
          <p className="text-sm text-foreground font-medium mt-2">
            ...
            {transmissions.length > 0 && (
              <> · <span className="text-primary">{transmissions.join(' + ')} Lessons</span></>
            )}
          </p>

          {/* Suburb */}
          {suburb && (
            <p className="text-xs text-muted-foreground/60 mt-1">{suburb}</p>
          )}
        </div>

        {/* QR code — centred, prominent */}
        <div className="flex justify-center py-4 bg-secondary/60">
        ...
        <div className="px-6 py-5 text-center space-y-1">
          <p className="text-lg font-bold text-primary tracking-wide">Book Online 24/7</p>
          <p className="text-2xl font-extrabold text-foreground">${instructor.hourlyRate}<span className="text-sm font-normal text-muted-foreground">/hr</span></p>
          <p className="text-[10px] text-muted-foreground break-all mt-1">{url}</p>
        </div>

        <div className="bg-background py-2 text-center">
          <p className="text-[10px] text-muted-foreground tracking-wide">drivebook.com.au</p>
        </div>
      </div>
    );
  }
  ...
  if (style === 'light') {
    return (
      <div
        id="flyer-print-area"
        className="bg-card text-foreground rounded-2xl overflow-hidden shadow-xl w-[320px] mx-auto border border-border print:shadow-none print:border print:border-border"
      >
        {/* Top colour strip */}
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 px-6 pt-6 pb-5 text-foreground text-center">
          {instructor.profileImage && (
            <img
              src={instructor.profileImage}
              alt={headline}
          ...
          {instructor.averageRating && instructor.totalReviews > 0 && (
            <div className="mt-2 flex items-center justify-center gap-0.5">
              {[1,2,3,4,5].map((i: any) => (
                <Star key={i} className={`w-4 h-4 ${i <= Math.round(instructor.averageRating!) ? 'fill-amber-300 text-amber-300' : 'fill-white/20 text-foreground/20'}`} />
              ))}
        ...
        <div className="px-6 py-4 text-center">
          <p className="text-base font-bold text-primary">Book Online 24/7</p>
          <p className="text-3xl font-extrabold text-foreground mt-1">${instructor.hourlyRate}<span className="text-sm font-normal text-muted-foreground">/hr</span></p>
          <p className="text-[10px] text-muted-foreground break-all mt-2">{url}</p>
        </div>

        <div className="bg-primary py-2 text-center">
          <p className="text-[10px] text-foreground/80 tracking-wide">drivebook.com.au</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      id="flyer-print-area"
      className="bg-card text-foreground rounded-2xl overflow-hidden shadow-sm w-[320px] mx-auto border-2 border-border print:shadow-none"
    >
      {/* Name block */}
      <div className="px-6 pt-5 pb-3 text-center border-b-2 border-border">
        <h1 className="text-2xl font-extrabold leading-tight tracking-tight text-foreground">{headline}</h1>
        {subline && <p className="text-sm text-muted-foreground mt-0.5">{subline}</p>}

        {instructor.averageRating && instructor.totalReviews > 0 && (
          <div className="mt-1.5 flex items-center justify-center gap-0.5">
            {[1,2,3,4,5].map((i: any) => (
              <Star key={i} className={`w-3.5 h-3.5 ${i <= Math.round(instructor.averageRating!) ? 'fill-gray-800 text-foreground' : 'fill-gray-200 text-gray-200'}`} />
            ))}
          </div>
        )}
        ...
        {suburb && <p className="text-xs text-muted-foreground/60 mt-0.5">{suburb}</p>}
      </div>
      ...
      {/* CTA + price */}
      <div className="px-6 py-4 text-center border-t-2 border-border">
        <p className="text-base font-bold text-foreground">Book Online 24/7</p>
        <p className="text-3xl font-extrabold text-foreground mt-1">${instructor.hourlyRate}<span className="text-sm font-normal text-muted-foreground/60">/hr</span></p>
        <p className="text-[10px] text-muted-foreground break-all mt-2">{url}</p>
      </div>
    </div>
  );
...
function PlatformFlyer({ style }: { style: 'dark' | 'light' | 'minimal' }) {
    ...
    return (
      ...
      >
        ...
        <div className="px-5 pb-4 space-y-1.5">
          {PLATFORM_BENEFITS.map(({ icon, text }) => (
            <div key={text} className="flex items-center gap-2.5 bg-secondary/60 rounded-lg px-3 py-2">
        ...
        <div className="flex justify-center py-4 bg-secondary/40">
          <div className="bg-card rounded-xl p-3">
        ...
        <div className="px-6 py-4 text-center">
          ...
          <p className="text-[10px] text-muted-foreground mt-1 break-all">{PLATFORM_URL}</p>
        </div>
  ...
  if (style === 'light') {
    return (
      <div
        ...
        className="bg-card text-foreground rounded-2xl overflow-hidden shadow-xl w-[320px] mx-auto border border-border print:shadow-none print:border print:border-border"
      >
        <div className="bg-gradient-to-br from-emerald-600 to-teal-700 px-6 pt-6 pb-5 text-foreground text-center">
          <div className="w-14 h-14 rounded-2xl bg-card/20 flex items-center justify-center mx-auto mb-3">
            <span className="text-2xl">🚗</span>
          </div>
  ...
  return (
    ...
    >
      <div className="px-6 pt-5 pb-3 text-center border-b-2 border-border">
        <h1 className="text-2xl font-extrabold leading-tight">Grow Your Driving Business</h1>
        <p className="text-sm font-bold text-muted-foreground mt-0.5">with DriveBook</p>
        <p className="text-xs text-muted-foreground/60 mt-1">Less admin. More teaching.</p>
      </div>
      ...
      <div className="flex justify-center py-3 border-t border-b border-border">
        <QRCodeSVG value={PLATFORM_URL} size={145} bgColor="#ffffff" fgColor="#111111" level="M" includeMargin={false} />
      </div>

      <div className="px-6 py-4 text-center">
        <p className="text-base font-bold text-foreground">Scan to Join Free</p>
        <p className="text-[10px] text-muted-foreground mt-1 break-all">{PLATFORM_URL}</p>
      </div>
    </div>
  );
...
export default function MarketingPage() {
  ...
  const [printTarget, setPrintTarget] = useState<'provider' | 'platform'>('provider');
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground">Marketing Flyer</h1>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-card rounded-2xl border border-border p-5">
            <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-2">Flyer identity</p>
            <p className="text-lg font-bold text-foreground">{headline}</p>
            {subline && <p className="text-sm text-primary mt-0.5">{subline}</p>}
            <p className="text-xs text-muted-foreground/60 mt-2">
              {(instructor.subscriptionTier === 'PREMIUM') || instructor.accountType === 'BUSINESS'
                ? 'Premium tier — business name is the brand'
              ...
              {' · '}
              <a href="/dashboard/branding" className="text-sky-500 hover:text-primary underline">
                Change in Branding →
              </a>
            </p>
          </div>
          ...
          {/* Booking URL */}
          <div className="bg-card rounded-2xl border border-border p-5">
            <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-2">QR code destination</p>
            <div className="flex items-center gap-2">
              <p className="flex-1 text-sm text-primary font-mono break-all bg-background rounded-lg px-3 py-2 border border-border">
                {bookingUrl}
              </p>
              <button
                onClick={handleCopyUrl}
                className="shrink-0 p-2 rounded-lg bg-secondary hover:bg-secondary/70 transition-colors"
                title="Copy URL"
              >
              ...
              {([
                ...
                <button
                  ...
                  className={`px-3 py-2.5 rounded-xl text-sm font-medium border-2 transition-all text-left ${
                    style === key
                      ? 'border-sky-500 bg-sky-950/40 text-primary'
          ...
          {/* Print instructions */}
          <div className="bg-card rounded-2xl border border-border p-5">
            <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-3">How to print</p>
            <ol className="space-y-2 text-sm text-foreground">
              <li className="flex items-start gap-2">
                <span className="shrink-0 w-5 h-5 rounded-full bg-primary text-foreground text-[10px] font-bold flex items-center justify-center mt-0.5">1</span>
                Click <strong className="text-foreground">Print Flyer</strong> below
              </li>
              <li className="flex items-start gap-2">
                <span className="shrink-0 w-5 h-5 rounded-full bg-primary text-foreground text-[10px] font-bold flex items-center justify-center mt-0.5">2</span>
                Choose <strong className="text-foreground">Save as PDF</strong> or your printer
              </li>
              <li className="flex items-start gap-2">
                <span className="shrink-0 w-5 h-5 rounded-full bg-primary text-foreground text-[10px] font-bold flex items-center justify-center mt-0.5">3</span>
                Print A5 card stock — laminate for longevity
              </li>
              <li className="flex items-start gap-2">
                <span className="shrink-0 w-5 h-5 rounded-full bg-primary text-foreground text-[10px] font-bold flex items-center justify-center mt-0.5">4</span>
            ...
            <button
              onClick={() => {
                setPrintTarget('provider');
                setTimeout(() => window.print(), 100);
              }}
              className="mt-4 w-full flex items-center justify-center gap-2 px-5 py-3 bg-primary hover:bg-sky-500 text-foreground rounded-xl font-semibold text-sm transition-colors"
            >
        ...
        {/* Right: live preview */}
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-4 text-center">Preview</p>
      ...
      <div className="mt-12 pt-10 border-t border-border">
        <div className="mb-6">
          <h2 className="text-xl font-bold text-foreground">Recruit Instructors for DriveBook</h2>
          <p className="text-sm text-muted-foreground mt-1">
            Print this flyer and leave it at driving school offices, instructor associations, or hand it to colleagues.
            QR code goes to <span className="text-primary font-mono">drivebook.com.au/teach-with-drivebook</span>.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-5">
            ...
            <div className="bg-card rounded-2xl border border-border p-5">
              <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-3">How to use</p>
              <ol className="space-y-2 text-sm text-foreground">
                <li className="flex items-start gap-2">
                  <span className="shrink-0 w-5 h-5 rounded-full bg-emerald-600 text-foreground text-[10px] font-bold flex items-center justify-center mt-0.5">1</span>
                  Click <strong className="text-foreground">Print Flyer</strong> and save as PDF
                </li>
                <li className="flex items-start gap-2">
                  <span className="shrink-0 w-5 h-5 rounded-full bg-emerald-600 text-foreground text-[10px] font-bold flex items-center justify-center mt-0.5">2</span>
                  Leave at driving schools, instructor offices, or NDIS providers
                </li>
                <li className="flex items-start gap-2">
                  <span className="shrink-0 w-5 h-5 rounded-full bg-emerald-600 text-foreground text-[10px] font-bold flex items-center justify-center mt-0.5">3</span>
              ...
              <button
                ...
                className="mt-4 w-full flex items-center justify-center gap-2 px-5 py-3 bg-emerald-600 hover:bg-emerald-500 text-foreground rounded-xl font-semibold text-sm transition-colors"
              >
                <Printer className="w-4 h-4" />
      ...
      <div id="print-page" style={{ visibility: 'hidden', position: 'absolute', top: 0, left: 0, zIndex: -1, pointerEvents: 'none' }}>
        {printTarget === 'provider' && instructor ? (

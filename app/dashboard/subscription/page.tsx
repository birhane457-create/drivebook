...
export default async function SubscriptionPage() {
  ...
  return (
          <div className="py-8 bg-slate-900/60 border border-slate-800 rounded-3xl shadow-sm">
        <div className="mb-8">
        ...
        {instructor.subscriptionStatus === 'TRIAL' && (
          <div className={`mb-8 rounded-lg p-6 ${trialExpired ? 'bg-red-50 border-2 border-red-200' : daysLeftInTrial <= 3 ? 'bg-amber-50 border-2 border-amber-300' : 'bg-blue-50 border-2 border-blue-200'}`}>
            <div className="flex items-start">
              <div className="flex-shrink-0">
                {trialExpired ? (
                  ...
                  <svg className={`h-6 w-6 ${daysLeftInTrial <= 3 ? 'text-amber-600' : 'text-blue-600'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                )}
              </div>
              <div className="ml-3 flex-1">
                <h3 className={`text-sm font-medium ${trialExpired ? 'text-red-800' : daysLeftInTrial <= 3 ? 'text-amber-800' : 'text-blue-800'}`}>
                  {trialExpired
                    ? 'Trial Expired'
                    : daysLeftInTrial <= 3
                ...
                <div className={`mt-2 text-sm ${trialExpired ? 'text-red-700' : daysLeftInTrial <= 3 ? 'text-amber-700' : 'text-blue-700'}`}>
                  {trialExpired ? (

...
export default function SettingsPage() {
  const { customer, customers, customersLowercase, isDriving } = useBusinessConfig()
  ...
  return (
        ...
        <h1 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6 text-foreground">Settings</h1>

        {/* Business Configuration Link */}
        <div className="mb-6 bg-gradient-to-r from-blue-900/40 to-purple-900/40 border border-blue-700/50 rounded-xl p-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-semibold text-foreground mb-1">Business Configuration</h3>
              <p className="text-sm text-foreground">
                Customize terminology and enable/disable platform capabilities
              </p>
            </div>
            <a
              href="/dashboard/settings/business"
              className="px-4 py-2 bg-primary hover:bg-primary/90 text-foreground rounded-lg font-medium transition-colors text-sm whitespace-nowrap"
            >
              Configure →
            </a>
          </div>
        ...
        {loading ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
              ...
              <div className="mt-4 bg-sky-900/20 border border-sky-700/40 rounded-xl px-4 py-3 text-sm text-primary space-y-1">
                <p className="font-semibold text-sky-200">💡 How package discounts work</p>
                <p>
                  The platform offers bulk-lesson discounts (e.g. 6, 10, or 15 hours) to {customersLowercase} as a platform incentive.
                  These discounts are <strong>funded by the platform</strong> — not by you.
                </p>
                <p>
                  Your payout is always calculated on your <strong>full hourly rate</strong>, regardless of any package discount the {customer.toLowerCase()} received.
                  You earn the same whether a {customer.toLowerCase()} books 1 hour or 15.
                </p>
              </div>
            </div>
            ...
            <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Add the suburbs you serve. {customers} searching those suburbs will find you first.
              ...
              <div className="space-y-5">
                {/* Base address */}
                <div>
                  <label className="block text-sm font-medium mb-2 text-foreground">Base Suburb (your home base)</label>
                  <SuburbAutocomplete
                  ...
                  />
                  <p className="text-xs text-muted-foreground mt-1">Suburb name shown publicly. Precise address kept private.</p>
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 text-foreground">
                    ...
                    {formData.serviceAreas.length > 0 && (
                      <span className="ml-2 text-xs font-normal text-primary">{formData.serviceAreas.length} suburb{formData.serviceAreas.length !== 1 ? 's' : ''}</span>
                    )}
                  </label>
                  <ServiceAreaPicker
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 text-foreground">
                    Fallback Radius (km)
                    <span className="ml-2 text-xs font-normal text-muted-foreground/60">— used only if no suburbs are listed above</span>
                  </label>
                  <input
                    type="number"
                    ...
                    min="1"
                    max="100"
                    className="w-32 px-3 py-2 border border-border bg-background text-foreground rounded-lg focus:ring-2 focus:ring-primary/50"
                  />
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 text-foreground">
                    Your Timezone
                    <span className="ml-2 text-xs font-normal text-muted-foreground/60">— used for scheduling, availability, and notifications</span>
                  </label>
                  <select
                    value={formData.timezone}
                    onChange={(e) => setFormData(prev => ({ ...prev, timezone: e.target.value }))}
                    className="w-full max-w-xs px-3 py-2 border border-border bg-background text-foreground rounded-lg focus:ring-2 focus:ring-primary/50"
                  >
                    {AU_TIMEZONES.map(tz => (
            ...
            {/* Professional Credentials — Driving Only */}
            {isDriving && (
              <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
                <h2 className="text-xl font-bold mb-1 flex items-center gap-2 text-foreground">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium mb-2 text-foreground">Instructor Licence Number</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2 text-foreground">Insurance Policy Number</label>
                    <input
                      type="text"
                  ...
                  {(!formData.licenseNumber || !formData.insuranceNumber) && (
                    <div className="bg-amber-900/20 border border-amber-700/50 rounded-lg p-3 text-sm text-amber-300">
                      ⚠️ Complete your credentials — required for approval and {customer.toLowerCase()} trust.
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
              ...
              {bookingPrefsExpanded && (
                <div className="space-y-6 mt-4">
                  {/* Allowed Durations */}
                  <div>
                    <label className="block text-sm font-medium mb-3">{isDriving ? 'Lesson' : 'Booking'} Durations You Offer</label>
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                setFormData(prev => ({
                                  ...prev,
                                  allowedDurations: [...prev.allowedDurations, duration.value].sort((a: any, b: any) => a - b)
                                }))
                              } else {
                                setFormData(prev => ({
                                  ...prev,
                    ...
                    <p className="text-xs text-muted-foreground mt-2">Select at least one duration. {customers} can only book these lengths.</p>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-2">
                      Time for rest, paperwork, and preparation between {customersLowercase} (always applied)
                    </p>
                  </div>
                      ...
                      <div className="ml-6 space-y-2">
                        <div className="flex items-center gap-3">
                          ...
                          <input
                            ...
                            min="5"
                            max="60"
                            className="w-20 px-3 py-2 border border-border bg-background text-foreground rounded-lg focus:ring-2 focus:ring-primary/50"
                          />
                          <span className="text-sm text-foreground">minutes</span>
                        </div>
                        <p className="text-xs text-muted-foreground">
                          Additional time on top of buffer for traveling to next {customer.toLowerCase()}'s location
                        </p>
                      </div>
                    )}
                  ...
                  <div className="bg-background border border-border rounded-lg p-4">
                    <h4 className="text-sm font-semibold text-foreground mb-2">📅 Schedule Example:</h4>
                    <div className="text-xs text-muted-foreground space-y-1">
                      <p>
                        <strong>{isDriving ? 'Lesson' : 'Booking'}:</strong> 1 hour (9:00-10:00)
                      </p>
                      <p>
              ...
              {!bookingPrefsExpanded && (
                <p className="text-sm text-muted-foreground mt-2">Click to expand and edit booking preferences</p>
              )}
            </div>
            ...
            <div className="bg-background rounded-2xl shadow-sm border border-border p-6">
              ...
              {workingHoursExpanded && (
                <>
                  ...
                  <div className="space-y-4">
                    {days.map((day) => (
                      <div key={day} className="border border-border bg-card rounded-lg p-4">
                        ...
                        <div className="space-y-2">
                          {(formData.workingHours[day as keyof typeof formData.workingHours] || []).length === 0 ? (
                            <div className="text-sm text-muted-foreground italic">Not working this day</div>
                          ) : (
                            formData.workingHours[day as keyof typeof formData.workingHours].map((slot, index) => (
                              <div key={index} className="flex items-center gap-2">
                                <input
                                  ...
                                  className="flex-1 px-3 py-2 border border-border bg-background text-foreground rounded-lg focus:ring-2 focus:ring-primary/50"
                                />
                                ...
                                <button
                                  ...
                                  className="text-destructive hover:text-red-500 p-2"
              ...
              {!workingHoursExpanded && (
                <p className="text-sm text-muted-foreground mt-2">Click to expand and edit your working hours</p>
              )}
            </div>

            {/* PDA Test Configurations Section — Driving Only */}
            {isDriving && (
              <div className="bg-background rounded-2xl shadow-sm border border-border p-6">
                <div className="flex items-center justify-between">
                  <h2 
                    className="text-xl font-bold flex items-center gap-2 text-foreground flex-1 cursor-pointer"
                ...
                <div className="flex items-center gap-2">
                  {pdaConfigsExpanded && (
                    <button
                      ...
                      className="bg-primary text-foreground px-4 py-2 rounded-lg hover:bg-primary/90 flex items-center gap-2 text-sm"
                    >
                      <Plus className="h-4 w-4" />
                      Add PDA Config
                    </button>
                ...
                <>
                  ...
                  {formData.pdaConfigs.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-sm">Create PDA test packages to offer {customersLowercase}</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      {formData.pdaConfigs.map((config) => {
                        ...
                        return (
                          <div key={config.id}>
                            ...
                            {!isExpanded && (
                              <div className="border border-border bg-card rounded-lg p-3 flex items-center justify-between hover:bg-secondary/50 transition">
                                  ...
                                  <div className="flex items-center gap-4 flex-wrap">
                                    {isComplete ? (
                                      <>
                                        ...
                                        <span className="text-xs text-muted-foreground">·</span>
                                        <span className="text-xs text-foreground font-medium">${config.price.toFixed(2)}</span>
                                      ...
                                      <span className="text-sm text-muted-foreground italic">(Unnamed - click to edit)</span>
                                    )}
                                  </div>
                                </div>
                                <button
                                  ...
                                  className="ml-4 text-primary hover:text-primary flex items-center gap-1 whitespace-nowrap text-sm"
                                >
                                  View/Edit
                              ...
                              <div className="border border-border bg-card rounded-lg p-4 space-y-3">
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <button
                                    ...
                                    className="text-destructive hover:text-red-500 p-1"
                                ...
                                {pdaDeleteConfirmId === config.id && (
                                  <div className="flex items-center gap-3 rounded-xl bg-red-950/40 border border-red-700/50 px-4 py-3 mt-2">
                                    ...
                                    <button
                                      ...
                                      className="shrink-0 bg-destructive hover:bg-destructive/90 text-foreground text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors"
                                    >
                                      Delete
                                    </button>
                                    <button
                                      ...
                                      className="shrink-0 bg-secondary/70 hover:bg-slate-600 text-foreground text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors"
                                    >
                                      Cancel
                                    </button>
                                  </div>
                                ...
                                <div className="space-y-3">
                                  <div>
                                    <label className="block text-sm font-medium mb-1 text-foreground">Configuration Name</label>
                                    <input
                                      type="text"
                                      value={config.name}
                                  ...
                                  <div className="grid sm:grid-cols-3 gap-3">
                                    <div>
                                      <label className="block text-sm font-medium mb-1 text-foreground">Duration (min)</label>
                                      <input
                                        type="number"
                                        ...
                                        className="w-full px-3 py-2 border border-border bg-background text-foreground rounded-lg focus:ring-2 focus:ring-primary/50"
                                      />
                                      <p className="text-xs text-muted-foreground mt-1">{durationStr}</p>
                                    </div>
                                    ...
                                    <div>
                                      ...
                                      <input
                                        ...
                                        className="w-full px-3 py-2 border border-border bg-background text-foreground rounded-lg focus:ring-2 focus:ring-primary/50"
                                      />
                                      {config.discountPercent && <p className="text-xs text-emerald-400 mt-1">${(config.price * (1 - config.discountPercent / 100)).toFixed(2)}</p>}
                                    </div>
                                  </div>

                                  <div>
                                    <label className="block text-sm font-medium mb-2 text-foreground">Test Centres</label>
                                    {loadingCentres ? (
                                      <p className="text-sm text-muted-foreground">Loading...</p>
                                    ) : testCentres.length === 0 ? (
                                      <p className="text-sm text-muted-foreground">No test centres available</p>
                                    ) : (
                                      <div className="space-y-2">
                                        <div className="space-y-2 max-h-48 overflow-y-auto bg-background rounded border border-border p-3">
                                          {testCentres.map((centre) => (
                                            <label key={centre.id} className="flex items-start gap-2 cursor-pointer hover:bg-secondary/50 p-2 rounded transition">
                                              <input
                                                type="checkbox"
                                                ...
                                                onChange={(e) => {
                                                  updatePDAConfig(config.id, {
                                                    testCentreIds: e.target.checked 
                                                      ? [...((config as any).testCentreIds || []), centre.id]
                                                      : ((config as any).testCentreIds || []).filter((id: string) => id !== centre.id)
                                                  })
                                                }}
                                                className="w-4 h-4 text-primary rounded mt-0.5 flex-shrink-0"
                                              />
                                              <div className="flex-1 min-w-0">
                                        ...
                                        {config.testCentreIds && config.testCentreIds.length > 0 && (
                                          <div className="bg-background border border-primary/25 rounded p-2">
                                            <p className="text-xs text-primary font-medium mb-1">✓ Selected Centres:</p>
                                            <div className="flex flex-wrap gap-1">
                                  ...
                                  <div>
                                    <label className="block text-sm font-medium mb-2 text-foreground">What's Included</label>
                                    <div className="space-y-2">
                                      {[
                                        { key: 'pickup' as const, label: `Pickup from ${customer.toLowerCase()} location` },
                                  ...
                                  <div>
                                    ...
                                    <textarea
                                      ...
                                      placeholder={`Any special details ${customersLowercase} should know`}
                                      rows={2}
                                      className="w-full px-3 py-2 border border-border bg-background text-foreground rounded-lg focus:ring-2 focus:ring-primary/50"
                                    />
                                  </div>
                  ...
                  <button
                    ...
                    className="mt-4 w-full bg-primary text-foreground px-4 py-2 rounded-lg hover:bg-primary/90 flex items-center justify-center gap-2 text-sm"
                  >
                    <Plus className="h-4 w-4" />
                    Add PDA Config
                  </button>
                  
                  <div className="mt-4 bg-background border border-blue-700 rounded-lg p-3">
                    <p className="text-sm text-primary">
                      <strong>💡 Tip:</strong> Create multiple configs with different durations and test centres. {customers} see all available options when booking.
                    </p>
                  </div>
                </>
              )}
              ...
              {!pdaConfigsExpanded && (
                <p className="text-sm text-muted-foreground mt-2">Click to expand and manage PDA configurations</p>
              )}
            </div>
            )}
            ...
            <div className={`rounded-2xl shadow-sm border p-6 ${!formData.acceptingBookings ? 'bg-background border-amber-700' : 'bg-background border-border'}`}>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    {formData.acceptingBookings
                      ? `${customers} can currently book with you.`
                ...
                <button
                  ...
                  className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-primary/50 focus:ring-offset-2
                    ${formData.acceptingBookings ? 'bg-green-500' : 'bg-secondary/70'}`}
            ...
            <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
              ...
              <p className="text-xs text-muted-foreground/60 mb-4">Platform standard policy applied on all bookings.</p>
              <div className="space-y-2.5">
                <div className="flex items-center justify-between py-2 border-b border-border"><span className="text-sm text-foreground">✅ 48+ hours notice</span><span className="text-sm font-medium text-foreground">Full refund (100%)</span></div>
                <div className="flex items-center justify-between py-2 border-b border-border"><span className="text-sm text-foreground">⚠️ 24–48 hours notice</span><span className="text-sm font-medium text-foreground">50% refund</span></div>
                <div className="flex items-center justify-between py-2"><span className="text-sm text-foreground">❌ Under 24 hours</span><span className="text-sm font-medium text-foreground">No refund</span></div>
              </div>
              <p className="text-xs text-muted-foreground/60 mt-4">{customers} see this under &quot;Before You Book&quot; on your booking page.</p>
            </div>
            ...
            <button
              type="submit"
              disabled={saving}
              className="w-full bg-primary text-foreground py-3 rounded-lg font-semibold hover:bg-primary/90 disabled:opacity-50 flex items-center justify-center gap-2"
            >

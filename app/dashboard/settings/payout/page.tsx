...
export default function PayoutSettingsPage() {
  ...
  const inputBase = 'w-full border border-border bg-background text-foreground rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary/50 placeholder-slate-500';

  return (
    <div className="max-w-2xl mx-auto py-8 px-1">
      <h1 className="text-2xl font-bold text-foreground mb-1">Payout &amp; Tax Settings</h1>
      <p className="text-sm text-muted-foreground mb-6">How you receive payments and your Australian tax details.</p>

      {toast && (
        <div className={`fixed top-5 right-5 z-50 flex items-center gap-3 px-4 py-3 rounded-xl shadow-lg text-foreground text-sm font-medium ${toast.type === 'success' ? 'bg-emerald-600' : 'bg-destructive'}`}>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        {/* Payout Method */}
        <div className="bg-card rounded-2xl border border-border p-1 space-y-4">
          ...
          <div className="grid gap-3">
            {([
              ...
              <label key={opt.value} className={`flex items-start gap-3 p-3 rounded-lg border-2 cursor-pointer transition-colors ${s.payoutMethod === opt.value ? 'border-blue-500 bg-blue-900/20' : 'border-border hover:border-border'}`}>
          ...
          {s.payoutMethod === 'stripe_connect' && (
            <div className="space-y-3">
              {s.stripeAccountId ? (
                    ...
                    <div className="space-y-3">
                      {fullyOnboarded ? (
                        <div className="flex items-start gap-3 p-4 rounded-lg bg-green-900/30 border border-green-700/50">
                          ...
                          <div>
                            <p className="text-sm font-semibold text-emerald-400">Stripe account ready — automatic payouts active</p>
                            <p className="text-xs text-emerald-400 mt-1">Your earnings are paid automatically every Tuesday at 2:00 AM. Lessons must be completed at least 48 hours before the payout run.</p>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="bg-background rounded-lg border border-border p-4 space-y-3">
                            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Onboarding progress</p>
                            {[
                              ...
                              <div key={step.label} className="flex items-center gap-3">
                                {step.done ? <CheckCircle className="h-4 w-4 text-emerald-400 flex-shrink-0" /> : <div className="h-4 w-4 rounded-full border-2 border-border flex-shrink-0" />}
                                <span className={`text-sm ${step.done ? 'text-foreground' : 'text-muted-foreground/60'}`}>{step.label}</span>
                              </div>
                            ))}
                          </div>
                          <button type="button" onClick={handleStripeConnect} disabled={connectingStripe}
                            className="flex items-center gap-2 bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-foreground px-5 py-2.5 rounded-lg font-medium text-sm transition-colors">
                ...
                <div className="space-y-3">
                  <div className="flex items-start gap-2 text-sm p-3 rounded-lg bg-sky-900/20 text-primary border border-sky-700/40">
                    <Info className="h-4 w-4 flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="font-medium">Connect your bank account via Stripe</p>
                      <p className="text-xs text-primary mt-1">You&apos;ll be taken to Stripe&apos;s secure page to enter your bank details directly. DriveBook never sees your account number — Stripe verifies it for you. Once set up, earnings are paid automatically every Tuesday.</p>
                    </div>
                  </div>
                  <button type="button" onClick={handleStripeConnect} disabled={connectingStripe}
                    className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-foreground px-5 py-2.5 rounded-lg font-medium text-sm transition-colors">
                    {connectingStripe ? <><Loader2 className="h-4 w-4 animate-spin" /> Connecting...</> : <><ExternalLink className="h-4 w-4" /> Connect with Stripe →</>}
                  </button>
                  <p className="text-xs text-muted-foreground">Need help?{' '}<a href="https://stripe.com/au/connect" target="_blank" rel="noreferrer" className="text-primary underline">Learn about Stripe Connect</a></p>
                </div>
              )}
            </div>
          )}
        ...
        {s.payoutMethod === 'bank_transfer' && (
          <div className="bg-card rounded-2xl border border-border p-6 space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <Field label="BSB" hint="Format: XXX-XXX">
                ...
                {s.bankBsb && isValidBSB(s.bankBsb) && (() => {
                  ...
                  return bank ? <p className="text-xs text-emerald-400 mt-1 flex items-center gap-1"><CheckCircle className="h-3 w-3" /> {bank}</p> : <p className="text-xs text-muted-foreground mt-1">BSB format valid</p>;
                })()}
                {s.bankBsb && !isValidBSB(s.bankBsb) && <p className="text-xs text-destructive mt-1">Invalid BSB format</p>}
              </Field>
              <Field label="Account Number">
                ...
                {s.bankAccount && !isValidBankAccount(s.bankAccount) && <p className="text-xs text-destructive mt-1">Must be 6–10 digits</p>}
              </Field>
            </div>
            ...
            <div className="flex items-start gap-2 text-xs text-muted-foreground bg-background border border-border rounded-lg p-3">
        ...
        <div className="bg-card rounded-2xl border border-border p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-1">ABN (Australian Business Number)</label>
            <div className="mb-3">
              ...
              <p className="text-xs text-muted-foreground mt-1">Must match the name on your ABR registration. Used to verify ABN ownership.</p>
            </div>
            <div className="relative">
              ...
              <div className="absolute right-3 top-2.5">
                ...
                {!verifying && s.abn.length === 11 && s.abnVerified && <CheckCircle className="h-4 w-4 text-emerald-400" />}
                {!verifying && s.abn.length === 11 && !abnChecksumValid && <XCircle className="h-4 w-4 text-destructive" />}
              </div>
            </div>
            <p className="text-xs text-muted-foreground mt-1">11 digits — no spaces. Verified automatically via the Australian Business Register.</p>

            {s.abn.length === 11 && !abnChecksumValid && (
              <div className="flex items-center gap-2 text-sm text-destructive bg-red-900/30 border border-red-700/50 rounded-lg p-3 mt-2">
            ...
            {verifyResult && !verifying && (
              <>
                {verifyResult.valid && (verifyResult.abnStatus === 'ACTIVE' || verifyResult.abnStatus === 'UNVERIFIED') && (
                  <div className="flex items-start gap-2 text-sm text-emerald-400 bg-green-900/30 border border-green-700/50 rounded-lg p-3 mt-2">
                    ...
                    <div>
                      ...
                      {verifyResult.entityName && <p className="text-emerald-400">{verifyResult.entityName}</p>}
                      ...
                      {verifyResult.nameMatchStatus === 'NO_MATCH' && <p className="text-destructive text-xs mt-1">Name does not match ABR records — admin review required.</p>}
                      <p className="text-xs text-emerald-400 mt-0.5">Save settings to apply.</p>
                    </div>
                  </div>
                )}
                {verifyResult.valid && verifyResult.abnStatus !== 'ACTIVE' && verifyResult.abnStatus !== 'UNVERIFIED' && (
                ...
                {!verifyResult.valid && verifyResult.error && (
                  <div className="flex items-start gap-2 text-sm text-destructive bg-red-900/30 border border-red-700/50 rounded-lg p-3 mt-2">
            ...
            {showVerifiedBadge && (
              <div className="flex items-center gap-2 text-sm text-emerald-400 bg-green-900/30 border border-green-700/50 rounded-lg p-3 mt-2">
                <CheckCircle className="h-4 w-4 flex-shrink-0" />
                <div>
                  <span className="font-medium">ABN verified — {formatABN(s.abn)}</span>
                  {s.abnEntityName && <span className="text-emerald-400"> · {s.abnEntityName}</span>}
                </div>
              </div>
            )}
          ...
          <div className="flex items-center gap-3">
            <button type="button" onClick={() => set({ gstRegistered: !s.gstRegistered })}
              className={`relative inline-flex h-6 w-11 rounded-full transition-colors ${s.gstRegistered ? 'bg-primary' : 'bg-secondary/70'}`}>
            ...
            <div>
              <span className="text-sm text-foreground">I am registered for GST</span>
              {s.gstRegistered && !s.abnVerified && <p className="text-xs text-muted-foreground mt-0.5">GST registration is noted — withholding still applies until your ABN is verified.</p>}
            </div>
          </div>
          ...
          {(() => {
            ...
            return (
              <div className={`flex items-start gap-2 text-sm p-3 rounded-lg ${verified && effectiveRate === 0 ? 'bg-green-900/30 text-emerald-400 border border-green-700/50' : 'bg-amber-900/30 text-amber-300 border border-amber-700/50'}`}>
                <Info className="h-4 w-4 flex-shrink-0 mt-0.5" />
        ...
        <div className="flex items-center gap-3">
          ...
          <p className="text-xs text-muted-foreground">Changes apply to future payouts only.</p>
        </div>
      </form>
    </div>
  );

...
function matchesSearch(b: ScheduleBooking, q: string): boolean {
  ...
  return (
    ((b as any).customerName ?? '').toLowerCase().includes(lq) ||
    ((b as any).customerPhone ?? '').replace(/\s/g, '').includes(q.replace(/\s/g, '')) ||
    ((b as any).pickupAddress ?? '').toLowerCase().includes(lq) ||
...
function BookingCard({ booking, compact = false, tz = DEFAULT_TIMEZONE }: { booking: ScheduleBooking; compact?: boolean; tz?: string }) {
  const { customer } = useBusinessConfig()
  const cfg    = getStatusConfig(booking.status);
  const suburb = extractSuburb(booking.pickupAddress);
  return (
    <Link
      href={`/dashboard/bookings/${booking.id}`}
      className={`block rounded-xl border-l-4 ${cfg.border} bg-secondary/60 hover:bg-secondary transition-colors no-underline`}
    >
...
function WeekView({ bookings, weekStart, search, statusFilter, bufferSettings }: {
  ...
  const filtered = bookings
    .filter((b: any) => matchesSearch(b, search))
    .filter((b: any) => !statusFilter || b.status === statusFilter);
        ...
        {weekDays.map((day, i) => {
          ...
          return (
            <div key={i} className={`flex-1 px-1 py-2 text-center text-xs font-medium border-l border-white/5 ${
              isToday ? 'bg-sky-950/30 text-primary' : 'text-muted-foreground'
            }`}>
              {formatDay(day, tz)}
            </div>
          );
        ...
        <div className="relative flex" style={{ height: `${VISIBLE_HOURS * HOUR_HEIGHT}px` }}>
          <div className="w-12 shrink-0 border-r border-white/5">
            {Array.from({ length: VISIBLE_HOURS }, (_, i) => {
              ...
              return (
                <div key={h} className="flex items-start justify-end pr-2 text-[10px] text-muted-foreground select-none"
          ...
          {weekDays.map((day, di) => {
            const isToday = isSameLocalDay(today, day, tz);
            const dayBookings = filtered.filter((b: any) => isSameLocalDay(b.startTime, day, tz));
            return (
...
function AgendaView({ bookings, range, search, statusFilter, tz = DEFAULT_TIMEZONE }: {
  ...
  const groups = days.map(day => ({
    day,
    bookings: filtered
      .filter((b: any) => isSameLocalDay(b.startTime, day, tz))
      .sort((a: any, b: any) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime()),
  })).filter(g => g.bookings.length > 0);

  const totalShown = groups.reduce((s: any, g: any) => s + g.bookings.length, 0);

  if (groups.length === 0) {
    return (
      <div className="rounded-2xl border border-border bg-card/60 px-6 py-16 text-center">
        {search || statusFilter ? (
          <>
            <p className="text-foreground font-medium mb-1">No lessons match your filters.</p>
            <p className="text-sm text-muted-foreground/60 mb-4">Try clearing the search or changing the status filter.</p>
          </>
        ) : range === 'past' ? (
          <>
            <p className="text-foreground font-medium mb-1">No completed lessons in the past 30 days.</p>
            <p className="text-sm text-muted-foreground/60 mb-4">Your lesson history will appear here once you have completed bookings.</p>
          </>
        ) : range === 'today' ? (
          <>
            <p className="text-foreground font-medium mb-1">Nothing scheduled today.</p>
            <p className="text-sm text-muted-foreground/60 mb-4">Enjoy the day off — or fill it with a new booking.</p>
          </>
        ) : range === 'week' ? (
          <>
            <p className="text-foreground font-medium mb-1">No lessons this week.</p>
            <p className="text-sm text-muted-foreground/60 mb-4">A quiet week — or a good time to plan ahead.</p>
          </>
        ) : (
          <>
            <p className="text-foreground font-medium mb-1">No lessons scheduled this month.</p>
            <p className="text-sm text-muted-foreground/60 mb-4">Create a booking to fill your schedule.</p>
          </>
        )}
        {!search && !statusFilter && (
          <Link
            href="/dashboard/bookings/new"
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-primary hover:bg-sky-500 text-foreground text-sm font-semibold transition-colors no-underline"
          >
            + New Booking
          </Link>
        )}
  ...
  return (
    <div>
      <p className="text-xs text-muted-foreground/60 mb-4">
        ...
        {statusFilter ? ` · ${getStatusConfig(statusFilter as any).label}` : ''}
      </p>
          ...
          return (
            <div key={day.toISOString()}>
              <div className="flex items-center gap-3 mb-2">
                <div className={`h-px flex-1 ${isToday ? 'bg-sky-500/30' : 'bg-secondary'}`} />
              ...
              <div className="space-y-2">
                {dayBookings.map((b: any) => <BookingCard key={b.id} booking={b} tz={tz} />)}
              </div>
            </div>
          );
        })}
...
export default function SchedulePage() {
  const { customer, customerLowercase } = useBusinessConfig()
  ...
  const todayForWorkspace: TodayBooking[] = bookings
    .filter((b: any) => isSameLocalDay(b.startTime, new Date(), tz))
    .map((b: any) => ({ ...b, endTime: b.endTime ?? null }));

  const instructorName = (session?.user as any)?.name ?? '';
  const presentStatuses = Array.from(new Set(bookings.map((b: any) => b.status)));
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Your lessons workspace</p>
        </div>
        <div className="flex items-center gap-2">
            ...
            {([
              ...
              <button key={key} onClick={() => setView(key)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                  view === key ? 'bg-primary text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground hover:bg-secondary/50'
                }`}
              >
          ...
          <Link
            href="/dashboard/bookings/new"
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-semibold bg-primary hover:bg-sky-500 text-foreground transition-colors no-underline"
          >
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          <div className="relative flex-1 min-w-[180px] max-w-xs">
            ...
            <input
              ...
              placeholder={`Search ${customerLowercase}, phone, location…`}
              className="w-full bg-card/80 border border-border rounded-xl pl-8 pr-8 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50"
            />
            {search && (
              <button onClick={() => setSearch('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground/60 hover:text-foreground">
          ...
          <select
            ...
            className="bg-card/80 border border-border rounded-xl px-3 py-2 text-sm text-foreground focus:outline-none focus:border-primary/50"
          >
            <option value="">All statuses</option>
            {presentStatuses.map(s => (
              <option key={s} value={s}>{getStatusConfig(s as any).label}</option>
            ))}
          </select>
          {hasActiveFilters && (
            <button
      ...
      {view === 'week' && (
        <div className="flex items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-2">
            <button onClick={prevWeek} className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-secondary/50 transition-colors">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <span className="text-sm font-medium text-foreground min-w-[140px] text-center">{weekLabel}</span>
            <button onClick={nextWeek} className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-secondary/50 transition-colors">
              <ChevronRight className="w-4 h-4" />
            </button>
          </div>
          <button onClick={goToday} className="px-3 py-1.5 rounded-lg text-xs font-medium border border-border text-foreground hover:text-foreground hover:bg-secondary/50 transition-colors">
            Today
          </button>
        </div>
      )}
      ...
      {/* Agenda range filter */}
      {view === 'agenda' && (
        <div className="flex items-center gap-1 mb-4 bg-card/60 border border-border rounded-xl p-1 w-fit">
          {([
            ...
            <button key={key} onClick={() => setAgendaRange(key)}
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                agendaRange === key ? 'bg-primary text-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-secondary/50'
              }`}
            >
              {label}
            </button>
      ...
      {loading && (
        <div className="space-y-4 animate-pulse">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="rounded-2xl border border-border bg-card/80 px-4 py-3 h-20">
                <div className="h-2.5 bg-secondary/70 rounded w-16 mb-3" />
                <div className="h-6 bg-secondary/70 rounded w-10" />
              </div>
            ))}
          </div>
          {/* Timeline skeleton */}
          <div className="rounded-2xl border border-border bg-card/80 overflow-hidden">
            <div className="px-5 py-4 border-b border-border">
              <div className="h-3.5 bg-secondary/70 rounded w-32" />
            </div>
            {[...Array(3)].map((_, i) => (
              <div key={i} className="flex items-center gap-4 px-5 py-4 border-b border-white/5 last:border-0">
                <div className="w-14 h-4 bg-secondary/70 rounded shrink-0" />
                <div className="w-0.5 self-stretch bg-secondary/70 rounded my-2" />
                <div className="flex-1 space-y-2">
                  <div className="h-3.5 bg-secondary/70 rounded w-36" />
                  <div className="h-3 bg-secondary rounded w-24" />
                </div>
                <div className="h-5 w-20 bg-secondary/70 rounded-full shrink-0" />
              </div>
            ))}
          </div>
        </div>

    ...
    <>
      ...
      <div className="min-h-screen bg-background py-8 px-4">
        ...
        <div className="no-print max-w-2xl mx-auto mb-8 text-center">
          <h1 className="text-2xl font-bold text-foreground mb-1">Receipt & Invoice Gallery</h1>
            ...
            {TABS.map(tab => (
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-semibold border transition-all whitespace-nowrap ${
                  active === tab.id
                    ? 'bg-secondary/70 border-slate-500 text-foreground shadow'
              ...
              >
                ...
                />
                <span className="font-bold text-muted-foreground/60">{tab.id}</span>
                {tab.label}
                <span className={`px-1.5 py-0.5 rounded text-xs font-normal ${
                  tab.audience === 'provider'
        ...
        <div className="no-print max-w-3xl mx-auto mb-4 flex items-center gap-3 bg-card rounded-xl border border-border px-4 py-3">
          <span
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground/60 mt-0.5">Triggered: {activeTab.trigger}</p>
          </div>
        </div>
            ...
            {CANCEL_VARIANTS.map(v => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                  cancelVariant === v.key
                    ? 'bg-red-900/40 border-red-700 text-destructive'
        ...
        <div className="no-print max-w-3xl mx-auto mt-10 mb-4">
          <p className="text-xs text-muted-foreground/60 uppercase tracking-wide mb-3">All receipt types at a glance</p>
            ...
            {TABS.map(tab => (
              <button
                ...
                className={`text-left px-3 py-2.5 rounded-xl border transition-all ${
                  active === tab.id
                    ? 'border-slate-500 bg-secondary'
                    : 'border-border bg-card hover:bg-secondary'
                }`}
              >
                <div className="flex items-center gap-1.5 mb-0.5">
                  <span className="h-2 w-2 rounded-full" style={{ background: tab.color }} />
                  <span className="text-xs font-bold text-muted-foreground">{tab.id}</span>
                </div>
                <p className="text-xs font-semibold text-foreground">{tab.label}</p>
                <p className="text-xs text-muted-foreground/60 mt-0.5">{tab.trigger}</p>
              </button>
            ))}
          </div>
        </div>

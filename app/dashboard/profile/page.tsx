...
export default function ProfilePage() {
  ...
  const { isDriving } = useBusinessConfig()
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({
          ...
          phone: data.phone || '',
          bio: data.bio || '',
          // Car/vehicle fields only for driving
          carMake: isDriving ? (data.carMake || '') : '',
          carModel: isDriving ? (data.carModel || '') : '',
          carYear: isDriving ? (data.carYear || '') : '',
          vehicleTypes: isDriving ? vehicleTypesArray : [],
        ...
        setCarImage(isDriving ? (data.carImage || '') : '')
      }
    } catch (error) {
    ...
    try {
      const res = await fetch('/api/instructor/profile', {
        ...
        body: JSON.stringify({
          ...formData,
          profileImage,
          // Car fields only for driving providers
          ...(isDriving ? {
            carImage,
      ...
      // Also save vehicleTypes through settings API (driving only)
      let settingsRes = { ok: true }  // default success for non-driving
      if (isDriving) {
        settingsRes = await fetch('/api/instructor/settings', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <DashboardPageLayout
      title="Profile"
      description="Your public instructor profile and account details"
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <section className="bg-card rounded-2xl border border-border p-5">
          <h2 className="text-base font-bold text-foreground mb-4">Photos</h2>
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                ...
                <input
                  ...
                  className="text-xs text-muted-foreground file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-secondary/70 file:text-foreground hover:file:bg-slate-600"
                />
              </div>
            </div>
            {/* Car photo */}
            <div>
              <p className="text-xs font-medium text-muted-foreground mb-2">Car photo</p>
              <div className="flex items-center gap-4">
                {carImage ? (
        ...
        {/* ── Basic Info ── */}
        <section className="bg-card rounded-2xl border border-border p-5">
          <h2 className="text-base font-bold text-foreground mb-4">Basic Information</h2>
          <div className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-border bg-background text-foreground rounded-lg focus:ring-2 focus:ring-violet-500 text-sm placeholder-slate-400"
                  required
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-foreground mb-1">Phone <span className="text-destructive">*</span></label>
                <input
                  type="tel"
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="block text-xs font-medium text-foreground">Bio</label>
                <button
                  type="button"
                  onClick={generateBio}
              ...
              <textarea
                ...
                className="w-full px-3 py-2 border border-border bg-background text-foreground rounded-lg focus:ring-2 focus:ring-violet-500 text-sm placeholder-slate-400 resize-none"
              />
              ...
              {(() => {
                ...
                return (
                  <div className="flex items-center justify-between mt-1">
                    ...
                    <span className={`text-[11px] font-medium tabular-nums ${met ? 'text-emerald-400' : words > 0 ? 'text-amber-400' : 'text-muted-foreground'}`}>
                      {words}/75 words
                    </span>
                  </div>
                );
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. 5"
                min="0" max="50"
                className="w-32 px-3 py-2 border border-border bg-background text-foreground rounded-lg focus:ring-2 focus:ring-violet-500 text-sm placeholder-slate-400"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                readOnly
                className="w-full px-3 py-2 border border-border bg-card text-muted-foreground rounded-lg text-sm cursor-not-allowed"
              />
        ...
        <section className="bg-card rounded-2xl border border-border overflow-hidden">
          ...
          >
            <h2 className="text-base font-bold text-foreground">Teaching Vehicle</h2>
            <ChevronDown className={`h-4 w-4 text-muted-foreground transition-transform ${vehicleOpen ? 'rotate-180' : ''}`} />
          </button>
        ...
        <section className="bg-card rounded-2xl border border-border overflow-hidden">
          ...
          <div className="px-5 pb-5">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-foreground mb-1">Base Address</label>
                <input
                  type="text"
                  ...
                  className="w-full px-3 py-2 border border-border bg-background text-foreground rounded-lg focus:ring-2 focus:ring-violet-500 text-sm placeholder-slate-400"
                />
                <p className="text-[11px] text-muted-foreground/60 mt-1">Only the suburb name is shown publicly</p>
              </div>

              <div>
                <label className="block text-xs font-medium text-foreground mb-2">Service Postcodes</label>
                <div className="flex gap-2 mb-3">
                  <input
                    ...
                    className="flex-1 px-3 py-2 border border-border bg-background text-foreground rounded-lg focus:ring-2 focus:ring-violet-500 text-sm placeholder-slate-400"
                  />
                  <button
                    type="button"
                    onClick={addServiceArea}
                    className="bg-violet-600 hover:bg-violet-700 text-foreground px-4 py-2 rounded-lg flex items-center gap-1.5 text-sm font-medium"
                  >
                ...
                <div className="flex flex-wrap gap-2">
                  {serviceAreas.map((area) => (
                    <div key={area.id} className="bg-secondary text-foreground px-3 py-1.5 rounded-lg flex items-center gap-2 text-sm">
                      <span className="font-medium">{area.postcode}</span>
                      {area.suburb && <span className="text-muted-foreground text-xs">({area.suburb})</span>}
                      <button
                        type="button"
                  ...
                  {serviceAreas.length === 0 && (
                    <p className="text-muted-foreground/60 text-xs">No postcodes added yet</p>
                  )}
                </div>
              </div>
            </div>
        ...
        <section className="bg-card rounded-2xl border border-border overflow-hidden">
          <button
            ...
            className="w-full flex items-center justify-between p-5 text-left hover:bg-secondary/50 transition-colors"
          >
            <h2 className="text-base font-bold text-foreground">Languages</h2>
            <ChevronDown className={`h-4 w-4 text-muted-foreground transition-transform ${langOpen ? 'rotate-180' : ''}`} />
          </button>

          {langOpen && (
          <div className="px-5 pb-5">
            <p className="text-[11px] text-muted-foreground/60 mb-3">Shown on your public profile — students filter by language</p>
            <div className="flex flex-wrap gap-2 mb-3">
              {formData.languages.map((lang) => (
                <span key={lang} className="bg-secondary text-foreground px-3 py-1 rounded-full text-sm flex items-center gap-1.5">
                  {lang}
                  <button
                    type="button"
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="bg-secondary/70 hover:bg-slate-600 text-foreground px-3 py-2 rounded-lg text-sm"
              >
                Add
              </button>
            </div>
        ...
        <section className="bg-card rounded-2xl border border-border overflow-hidden">
          ...
          >
            ...
            <ChevronDown className={`h-4 w-4 text-muted-foreground transition-transform ${videoOpen ? 'rotate-180' : ''}`} />
          </button>
                ...
                if (ytMatch) {
                  return (
                    <div className="rounded-lg overflow-hidden border border-border mt-2">
                      ...
                      <p className="text-[11px] text-muted-foreground/60 px-3 py-2">Preview of what students will see</p>
                    </div>
                  )
                }
                if (viMatch) {
        ...
        <section className="bg-card rounded-2xl border border-border overflow-hidden">
          ...
          >
            ...
            <ChevronDown className={`h-4 w-4 text-muted-foreground transition-transform ${specialtiesOpen ? 'rotate-180' : ''}`} />
          </button>
          ...
          {specialtiesOpen && (
            <div className="px-5 pb-5 space-y-3">
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${
                        selected
                          ...
                          : 'bg-secondary border-border text-foreground hover:border-slate-500'
                      }`}
                    >
                      {selected ? '✓ ' : ''}{tag}
                    </button>
              ...
              {formData.specialties.length > 0 && (
                <p className="text-[11px] text-muted-foreground/60">{formData.specialties.length} selected</p>
              )}
            </div>
          )}
        </section>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-violet-600 hover:bg-violet-700 disabled:opacity-50 text-foreground py-3 rounded-xl font-semibold flex items-center justify-center gap-2 transition-colors"
        >
      ...
      <div className="grid grid-cols-3 gap-3 mt-6">
        {[
          ...
          <Link
            key={href}
            href={href}
            className="flex flex-col gap-1 p-3 rounded-xl bg-secondary/60 border border-border hover:border-border hover:bg-secondary transition-all no-underline group"
          >

...
const resultColor = (r: string) =>
  r === 'PASS' ? 'bg-green-900/40 text-emerald-400' :
  r === 'FAIL' ? 'bg-red-900/40 text-destructive' :
  'bg-yellow-900/40 text-yellow-300'

const resultIcon = (r: string) =>
  r === 'PASS' ? <CheckCircle className="h-5 w-5 text-emerald-400" /> :
  r === 'FAIL' ? <XCircle className="h-5 w-5 text-destructive" /> :
...
export default function PDATestsPage() {
  ...
  const [providerId, setInstructorId] = useState<string>('')
  ...
  useEffect(() => {
    if (!form.configId || !form.testCentreId || !form.testDate || !providerId) {
      setSlots([])
      return
    }
    ...
    const fetchSlots = async () => {
      ...
      try {
        const res = await fetch(
          `/api/availability/pda-tests?providerId=${providerId}&configId=${form.configId}&testCentreId=${form.testCentreId}&date=${form.testDate}`
        )
        if (!res.ok) {
    ...
    fetchSlots()
  }, [form.configId, form.testCentreId, form.testDate, providerId])
  ...
  const handleSchedule = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setForm({ customerId: '', configId: '', testCentreId: '', testDate: '', selectedSlot: '', price: '' })
        setShowForm(false)
        setSlots([])
        fetchAll()
      } else {
  ...
  const formComplete = form.customerId && form.configId && form.testCentreId && form.testDate
  ...
  return (
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground">PDA Tests ({tests.length})</h1>
          <button
            onClick={() => { setShowForm(!showForm); setForm({ customerId: '', configId: '', testCentreId: '', testDate: '', selectedSlot: '', price: '' }); setSlots([]) }}
            className="w-full sm:w-auto bg-primary text-foreground px-4 py-2 rounded-lg flex items-center justify-center gap-2 hover:bg-primary/90"
          >
        ...
        {showForm && configs.length > 0 && (
          <div className="bg-card rounded-xl border border-border p-6 mb-6">
            <h2 className="text-lg font-bold text-foreground mb-5">Schedule PDA Test</h2>
            <form onSubmit={handleSchedule} className="space-y-5">

              {/* Row 1: Student + PDA Package */}
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Student <span className="text-destructive">*</span></label>
                  <select
                    required
                    value={form.customerId}
                    onChange={e => setForm(f => ({ ...f, customerId: e.target.value }))}
                    className="w-full px-3 py-2.5 border border-border bg-background text-foreground rounded-lg text-sm focus:ring-2 focus:ring-primary/50 outline-none"
                  >
                  ...
                  {clients.length === 0 && (
                    <p className="text-xs text-muted-foreground/60 mt-1">No students found. Add students from the Clients page.</p>
                  )}
                </div>

                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">PDA Package <span className="text-destructive">*</span></label>
                  <select
                    required
                    value={form.configId}
              ...
              {selectedConfig && (
                <div className="flex flex-wrap gap-2 text-xs">
                  <span className="bg-blue-900/40 text-primary px-2.5 py-1 rounded-full">{selectedConfig.durationMinutes} min</span>
                  ...
                  <span className="bg-secondary/70/60 text-foreground px-2.5 py-1 rounded-full">{selectedConfig.testCentres.length} centre{selectedConfig.testCentres.length !== 1 ? 's' : ''} available</span>
                </div>
              )}
              ...
              {form.configId && availableCentres.length > 0 && (
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Test Centre <span className="text-destructive">*</span></label>
                  <select
                    required
              ...
              {form.testCentreId && (
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Test Date <span className="text-destructive">*</span></label>
                  <input
                    type="date"
                    required
                    min={today}
                    ...
                    className="w-full sm:w-64 px-3 py-2.5 border border-border bg-background text-foreground rounded-lg text-sm focus:ring-2 focus:ring-primary/50 outline-none"
                  />
                </div>
              )}
              ...
              {formComplete && (
                <div>
                  <label className="block text-sm font-medium text-foreground mb-2">
                    Available Slots
                    {slotsLoading && <span className="ml-2 text-muted-foreground text-xs">Loading...</span>}
                  </label>
                  ...
                  {!slotsLoading && slotsError && (
                    <div className="text-destructive text-sm py-2">{slotsError}</div>
                  )}

                  {!slotsLoading && !slotsError && slots.length === 0 && (
                    <div className="bg-secondary rounded-lg px-4 py-3 text-muted-foreground text-sm">
                        ...
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg text-sm font-medium border-2 transition ${
                            form.selectedSlot === slot.time
                              ...
                              : 'border-border bg-secondary text-foreground hover:border-blue-600 hover:text-foreground'
                          }`}
                        >
                          {slot.time}
                        </button>
              ...
              {form.selectedSlot && (
                <div className="sm:w-64">
                  <label className="block text-sm font-medium text-foreground mb-1">
                    Price ($)
                    <span className="ml-1 text-xs text-muted-foreground font-normal">optional override</span>
                  </label>
                  <input
                    type="number"
                    min="0"
                    ...
                    className="w-full px-3 py-2.5 border border-border bg-background text-foreground rounded-lg text-sm focus:ring-2 focus:ring-primary/50 outline-none"
                  />
                  <p className="text-xs text-muted-foreground/60 mt-1">Leave blank to use the package price.</p>
                </div>
              )}
              ...
              <div className="flex gap-3 pt-1 flex-col">
                ...
                <div className="flex gap-3">
                  <button type="button" onClick={() => { setShowForm(false); setSlots([]) }}
                    className="flex-1 py-2.5 border border-border text-foreground rounded-lg text-sm font-medium hover:bg-secondary transition">
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={saving || !form.selectedSlot}
                    className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-primary text-foreground rounded-lg text-sm font-semibold hover:bg-primary/90 transition disabled:opacity-50"
                  >
        ...
        {loading ? (
          ...
          <div className="bg-card rounded-xl border border-border p-12 text-center">
            ...
            <h3 className="text-xl font-semibold mb-2 text-foreground">No PDA tests scheduled</h3>
            <p className="text-muted-foreground">Click &quot;Schedule Test&quot; to add your first test</p>
          </div>
        ) : (
          <div className="bg-card rounded-xl border border-border overflow-hidden">
            <div className="divide-y divide-slate-700">
              {tests.map(test => {
                ...
                return (
                  <div key={test.id} className="hover:bg-secondary transition">
                      ...
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            <h3 className="font-semibold truncate text-foreground">{test.customer.name}</h3>
                            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${resultColor(test.result)}`}>{test.result}</span>
                            {test.price > 0 && <span className="text-xs text-muted-foreground">${test.price.toFixed(0)}</span>}
                          </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {!isEditing && test.result === 'PENDING' && (
                          <button onClick={e => { e.stopPropagation(); setEditingId(test.id); setEditResult(test.result); setExpandedId(test.id) }}
                            className="p-2 hover:bg-secondary/70 rounded-lg text-primary" title="Update result">
                    ...
                    {isExpanded && (
                      <div className="px-4 pb-4 bg-background space-y-4 border-t border-border">
                        {isEditing ? (
                          <div className="space-y-4 pt-4">
                            <p className="text-sm font-medium text-foreground">Update Test Result</p>
                            <div className="flex gap-3">
                              <button onClick={() => setEditResult('PASS')}
                                className={`flex-1 px-4 py-3 rounded-xl border-2 flex items-center justify-center gap-2 font-medium transition ${editResult === 'PASS' ? 'border-green-500 bg-green-900/40 text-emerald-400' : 'border-border hover:border-green-600 text-foreground'}`}>
                                <CheckCircle className="h-5 w-5" /> Pass
                              </button>
                              <button onClick={() => setEditResult('FAIL')}
                                className={`flex-1 px-4 py-3 rounded-xl border-2 flex items-center justify-center gap-2 font-medium transition ${editResult === 'FAIL' ? 'border-red-500 bg-red-900/40 text-destructive' : 'border-border hover:border-red-600 text-foreground'}`}>
                                <XCircle className="h-5 w-5" /> Fail
                              </button>
                            </div>
                            <div className="flex gap-3">
                              <button onClick={() => handleUpdateResult(test.id)} disabled={!editResult || editResult === test.result}
                                className="flex-1 bg-primary text-foreground px-4 py-2.5 rounded-lg hover:bg-primary/90 disabled:opacity-50 flex items-center justify-center gap-2 text-sm font-semibold">
                                <Save className="h-4 w-4" /> Save Result
                              </button>
                              <button onClick={() => { setEditingId(null); setEditResult('') }}
                                className="flex-1 border border-border text-foreground px-4 py-2.5 rounded-lg hover:bg-secondary flex items-center justify-center gap-2 text-sm">
                          ...
                          <div className="grid sm:grid-cols-2 gap-4 text-sm pt-4">
                            <div>
                              <p className="font-medium text-foreground mb-2">Student</p>
                              <div className="space-y-1.5 text-muted-foreground">
                                <div className="flex items-center gap-2"><User className="h-4 w-4 text-muted-foreground/60" />{test.customer.name}</div>
                                <div className="flex items-center gap-2"><Phone className="h-4 w-4 text-muted-foreground/60" />{test.customer.phone}</div>
                                <div className="flex items-center gap-2"><Mail className="h-4 w-4 text-muted-foreground/60" />{test.customer.email}</div>
                              </div>
                            </div>
                            <div>
                              <p className="font-medium text-foreground mb-2">Test Centre</p>
                              <div className="space-y-1.5 text-muted-foreground">
                                <div className="font-medium text-foreground">{test.testCenterName}</div>
                                <div className="flex items-start gap-2"><MapPin className="h-4 w-4 text-muted-foreground/60 mt-0.5 shrink-0" />{test.testCenterAddress}</div>
                                {test.price > 0 && (
                                  <div className="flex items-center gap-2"><DollarSign className="h-4 w-4 text-muted-foreground/60" />${test.price.toFixed(2)}</div>
                                )}
                              </div>
                            </div>
                            {test.result === 'PENDING' && (
                              <div className="sm:col-span-2 pt-2 border-t border-border">
                                <button onClick={() => { setEditingId(test.id); setEditResult(test.result) }}
                                  className="w-full bg-primary text-foreground px-4 py-2.5 rounded-lg hover:bg-primary/90 flex items-center justify-center gap-2 text-sm font-semibold">

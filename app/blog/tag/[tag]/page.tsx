...
function PostCard({ post }: { post: BlogPost }) {
  ...
  const badge = isStudent
    ...
    : { colour: 'bg-pink-500/20 text-pink-300 border border-pink-500/30', label: 'provider' }
  ...
  return (
    <article className="group flex flex-col bg-secondary/50 border border-border rounded-2xl p-6 hover:border-white/20 hover:bg-white/[0.08] hover:-translate-y-1 hover:shadow-xl hover:shadow-black/30 transition-all duration-200">
      ...
      <p className="text-foreground/60 text-sm leading-relaxed mb-4 flex-grow">{post.description}</p>
      <div className="flex items-center justify-between mt-auto pt-4 border-t border-border">
        <time className="text-foreground/40 text-xs" dateTime={post.date}>{formatDate(post.date)}</time>
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="max-w-5xl mx-auto px-4 py-12">
        ...
        <nav className="flex items-center gap-2 text-xs text-foreground/40 mb-8">
          ...
          <span>/</span>
          <span className="text-foreground/60 capitalize">{tagLabel}</span>
        </nav>
        ...
        <div className="mb-10">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-foreground mb-3 capitalize">{tagLabel}</h1>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <aside className="lg:col-span-1">
            <div className="sticky top-24 bg-secondary/50 border border-border rounded-2xl p-6">
                    ...
                    <Link
                      ...
                      className={`text-xs px-2.5 py-1 rounded-full border no-underline transition-colors ${
                        isActive
                          ...
                          : 'bg-secondary/50 text-foreground/50 border-border hover:bg-secondary hover:text-foreground'
                      }`}
                    >
                      {label} <span className="text-foreground/30">({count})</span>
                    </Link>
                  )
                })}
              </div>
      ...
      <footer className="border-t border-border py-10 mt-12 text-center text-foreground/40 text-sm">
        <p>© {new Date().getFullYear()} DriveBook · <Link href="/privacy" className="hover:text-foreground/60 no-underline">Privacy</Link></p>
      </footer>
    </div>
  )
}

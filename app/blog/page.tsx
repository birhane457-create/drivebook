...
function FeaturedCard({ post }: { post: BlogPost }) {
  ...
  return (
    <Link
      href={`/blog/${post.slug}`}
      className="group block rounded-2xl overflow-hidden border border-border bg-gradient-to-br from-white/5 to-white/[0.02] hover:border-purple-500/40 hover:shadow-2xl hover:shadow-purple-500/10 hover:-translate-y-1 transition-all duration-200 no-underline"
    >
      {/* Coloured accent bar */}
      ...
      <div className="p-8 md:p-10">
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-foreground/30 text-xs">Featured</span>
        </div>

        <h2 className="text-2xl md:text-3xl font-bold text-foreground leading-snug mb-4 group-hover:text-purple-200 transition-colors">
          {post.title}
        </h2>
...
function PostCard({ post }: { post: BlogPost }) {
  const badge = categoryBadge(post.category)
  return (
    <article className="group flex flex-col bg-secondary/50 border border-border rounded-2xl p-6 hover:border-white/20 hover:bg-white/[0.08] hover:-translate-y-1 hover:shadow-xl hover:shadow-black/30 transition-all duration-200">
      <div className="flex items-center gap-3 mb-4">
        <span className={`text-xs font-semibold px-3 py-1 rounded-full ${badge.colour}`}>
          {badge.label}
        </span>
      ...
      <h3 className="text-base font-bold text-foreground mb-2 leading-snug group-hover:text-purple-200 transition-colors">
        {post.title}
      </h3>
      ...
      {post.tags && post.tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-4">
          {post.tags.map(tag => (
            <span key={tag} className="text-xs bg-secondary/50 text-foreground/50 px-2 py-0.5 rounded-full border border-border">
              {tag}
            </span>
          ))}
        </div>
      ...
      <div className="flex items-center justify-between mt-auto pt-4 border-t border-border">
        <span className="text-foreground/40 text-xs">{formatDate(post.date)}</span>
        <Link
          href={`/blog/${post.slug}`}
...
function MidCTA({ variant }: { variant: 'students' | 'instructors' }) {
  if (variant === 'students') {
    return (
      <div className="my-10 rounded-2xl bg-gradient-to-r from-violet-900/40 to-indigo-900/40 border border-violet-500/20 p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          <p className="font-bold text-foreground mb-1">Looking for a driving instructor in Perth?</p>
          <p className="text-foreground/60 text-sm">Find verified local instructors and book your lesson instantly.</p>
        </div>
        <Link
          href="/book"
  ...
  return (
    <div className="my-10 rounded-2xl bg-gradient-to-r from-pink-900/40 to-violet-900/40 border border-pink-500/20 p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
      <div>
        <p className="font-bold text-foreground mb-1">Are you a driving instructor?</p>
        <p className="text-foreground/60 text-sm">Online bookings, payments, reminders, and student tracking — all in one place.</p>
      </div>
      <Link
    ...
    <div className="light min-h-screen bg-background text-foreground">
      ...
      <header className="border-b border-border bg-background/80 backdrop-blur-xl sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1 text-sm">
            <Link href="/book" className="text-foreground/70 hover:text-foreground px-3 py-2 rounded-lg hover:bg-secondary transition-colors no-underline">Book a Lesson</Link>
            <Link href="/teach-with-drivebook" className="text-foreground/70 hover:text-foreground px-3 py-2 rounded-lg hover:bg-secondary transition-colors no-underline">For Instructors</Link>
            <Link href="/login" className="text-foreground/70 hover:text-foreground px-3 py-2 rounded-lg hover:bg-secondary transition-colors no-underline">Login</Link>
          </nav>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 py-16">
        ...
        <nav className="flex items-center gap-2 text-xs text-foreground/40 mb-8">
          <Link href="/" className="hover:text-foreground no-underline transition-colors">Home</Link>
          <span>/</span>
        ...
        <div className="mb-10 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-foreground mb-4">DriveBook Blog</h1>
        ...
        {featured && (
          <div className="mb-14">
            <p className="text-xs font-semibold text-foreground/30 uppercase tracking-widest mb-4">Featured Article</p>
        ...
        {studentPosts.length > 0 && (
          <section className="mb-4">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-2xl font-bold text-foreground">For Learner Drivers</h2>
            </div>
        ...
        {instructorPosts.length > 0 && (
          <section className="mb-4">
            <div className="flex items-center gap-3 mb-6">
              <span className="text-2xl">🚗</span>
              <h2 className="text-2xl font-bold text-foreground">For Driving Instructors</h2>
            </div>
            <div className="grid md:grid-cols-2 gap-5">
      ...
      {/* Footer */}
      <footer className="border-t border-border py-10 mt-12 text-center text-foreground/40 text-sm">
        <p>

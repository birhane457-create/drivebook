...
export default function DemoInstructorProfilePage() {
  ...
  return (
    <div className="light min-h-screen bg-gray-50">
      ...
      {/* Demo banner */}
      <div className="bg-primary text-foreground text-center py-2.5 px-4 text-sm font-medium">
      ...
      <nav className="bg-white shadow-sm sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/register"
              className="bg-primary hover:bg-primary/90 text-foreground text-sm font-bold px-4 py-2 rounded-lg transition-colors no-underline">
              Create profile
            </Link>
          </div>
        </div>
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 py-8">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
            ...
            <div className="relative w-28 h-28 sm:w-32 sm:h-32 shrink-0">
              <div className="w-full h-full rounded-full bg-gradient-to-br from-blue-500 to-violet-600 flex items-center justify-center border-4 border-white shadow-xl">
                <span className="text-4xl font-bold text-foreground">D</span>
              </div>
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              <p className="text-muted-foreground/60 text-sm mb-2">AUTO driving lessons · 11+ yrs experience</p>

              <div className="flex flex-wrap justify-center sm:justify-start gap-x-4 gap-y-1.5 text-sm text-muted-foreground/60 mb-3">
                <span className="flex items-center gap-1.5">
                  <MapPin className="w-3.5 h-3.5 text-muted-foreground" /> Maylands, Perth WA
                </span>
                <span className="flex items-center gap-1.5">
                  <Car className="w-3.5 h-3.5 text-muted-foreground" /> Automatic
                </span>
                <span className="flex items-center gap-1.5">
                  <Award className="w-3.5 h-3.5 text-muted-foreground" /> 11 years experience
                </span>
              </div>
            ...
            <div className="text-center sm:text-right shrink-0">
              <p className="text-3xl font-bold text-gray-900">$75<span className="text-lg font-normal text-muted-foreground">/hr</span></p>
              ...
              <a href="#book"
                className="mt-3 inline-block bg-gray-100 text-muted-foreground font-bold px-5 py-2.5 rounded-xl text-sm cursor-default">
                Example only
              </a>
            </div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-3 gap-3 text-center text-xs text-muted-foreground/60 border-t border-gray-100 pt-5">
            <div>
          ...
          <div id="services" className="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            <div className="flex items-center justify-between p-3 border border-gray-200 rounded-xl mb-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Available lesson lengths</p>
                ...
                <div key={pkg.hours}
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {pkg.badge && (
                        <span className="text-xs bg-primary text-foreground px-2 py-0.5 rounded-full font-medium">
                          {pkg.badge}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="text-right">
                    ...
                    <button disabled className="mt-1 text-xs bg-gray-100 text-muted-foreground px-3 py-1.5 rounded-lg font-semibold cursor-not-allowed">
                      Demo only
                    </button>
                  </div>
                </div>
            ...
            <Link href="/register"
              className="mt-4 block text-center bg-primary hover:bg-primary/90 text-foreground font-bold py-3 rounded-xl text-sm transition-colors no-underline">
              Create my instructor profile →
            </Link>

            {/* Vehicle / languages / buffer */}
            <div className="mt-5 pt-4 border-t border-gray-100 grid grid-cols-3 gap-3 text-center text-xs text-muted-foreground/60">
              <div>
            ...
            <div className="space-y-2">
              {availability.map(a => (
                <div key={a.day} className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-muted-foreground/60">{a.hours}</span>
            ...
            <div className="space-y-3">
              {faq.map((item, i) => (
                <details key={i} className="group">
                  ...
                  <p className="text-sm text-muted-foreground/60 mt-1 pl-0 pb-2">{item.a}</p>
                </details>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div id="book" className="bg-white rounded-2xl border border-gray-200 p-6 sticky top-20">
            ...
            <div className="bg-gray-50 rounded-xl p-3 mb-4">
              <p className="text-xs text-muted-foreground mb-2 font-medium uppercase tracking-wide">Next Available</p>
            ...
            <Link
              href="/register"
              className="block text-center py-3 bg-primary hover:bg-primary/90 text-foreground font-bold rounded-xl text-sm transition-colors no-underline">
              Create my instructor profile →
            </Link>
            <p className="text-xs text-muted-foreground text-center mt-2">Free to register &middot; No credit card required</p>
            ...
            <div className="space-y-2.5">
              ...
              <div className="flex items-center gap-2 pt-1">
                ...
                <a
                  ...
                  rel="noopener noreferrer"
                  title="Facebook"
                  className="flex items-center justify-center w-8 h-8 rounded-full bg-primary text-foreground hover:bg-primary/90 transition-colors"
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
      ...
      {/* Footer CTA */}
      <div className="bg-blue-700 text-foreground text-center py-10 px-4 mt-4">
      ...
      {/* Footer */}
      <footer className="bg-white border-t border-gray-200 py-6 text-center text-xs text-muted-foreground">
        ...
        <p className="mt-1 text-foreground">This is an example profile for illustration purposes only.</p>
      </footer>
    </div>
  )
}

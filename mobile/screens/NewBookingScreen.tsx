...
export default function NewBookingScreen({ onBack }: { onBack: () => void }) {
  ...
  const [providerId, setInstructorId] = useState('');
  ...
  useEffect(() => {
    if (date && duration && providerId) {
      fetchTimeSlots();
    }
  }, [date, duration, providerId]);
  ...
  const fetchTimeSlots = async () => {
    if (!providerId || !date || !duration) return;
    
    setLoadingSlots(true);
    setSelectedTime(''); // Reset selected time when fetching new slots
    
    try {
      const response = await availabilityAPI.getSlots(providerId, date, duration);

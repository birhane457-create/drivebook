...
export default function ClientPerformanceScreen({ customerId, onBack }: Props) {
  ...
  const [(performanceScore as any), setPerformanceScore] = useState<string>('');
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setLoading(true);
      try {
        const perfRes = await fetch(`/api/instructor/client-performance?customerId=${customerId}`);
        ...
        const bookRes = await fetch(`/api/instructor/client-lesson-feedback?customerId=${customerId}&limit=50`);
        if (!bookRes.ok) throw new Error('Failed to fetch bookings');
        const bookData = await bookRes.json();
        setBookings(bookData as any).lessons || []);
      } catch (err: any) {
  ...
  const toggleFeedback = (code: number) => {
    setSelectedFeedback((prev) => (prev.includes(code) ? prev.filter((c: any) => c !== code) : [...prev, code]));
  };
    ...
    try {
      const res = await fetch('/api/instructor/lesson-feedback', {
        ...
        body: JSON.stringify({
          ...
          performanceScore: (performanceScore as any) ? parseInt((performanceScore as any), 10) : null,
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 16 }}>
      ...
      <Text style={styles.title}>{performance?.customer?.name || 'Client'}'s Performance</Text>
      ...
      <View style={styles.card}>
        ...
        {performance?.feedbackHistory?.slice(0, 10).map((lesson: any) => (
          <TouchableOpacity key={lesson.id} style={styles.lessonRow} onPress={() => setSelectedBookingId(lesson.id)}>
            ...
            {lesson.(performanceScore as any) ? <Text style={styles.lessonScore}>{lesson.(performanceScore as any)}%</Text> : null}
          </TouchableOpacity>
        ))}
      </View>
      ...
      <View style={styles.card}>
        ...
        <TextInput keyboardType="numeric" value={(performanceScore as any)} onChangeText={setPerformanceScore} style={styles.input} placeholder="e.g., 85" />

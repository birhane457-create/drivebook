...
interface SubscriptionData {
  current: {
    tier: 'BASIC' | 'PRO' | 'STUDIO' | 'PREMIUM';  // subscriptionTier (not accountType)
    status: string;
...
export default function SubscriptionScreen() {
  ...
  const changeTier = async (tier: 'BASIC' | 'PRO' | 'STUDIO' | 'PREMIUM') => {  // subscriptionTier values
    Alert.alert(
      'Change Plan',
      `Switch to ${tier} plan?`,
      [
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.plansSection}>
        ...
        <View style={[
          styles.planCard,
          data.current.tier === 'PREMIUM' && styles.planCardActive
        ]}>
          <View style={styles.planHeader}>
            <Text style={styles.planName}>PREMIUM</Text>
            {data.current.tier === 'PREMIUM' && (
              <View style={styles.currentBadge}>
                <Text style={styles.currentBadgeText}>Current</Text>
              </View>
            )}
          ...
          <Text style={styles.planAmount}>${data.pricing.PREMIUM.monthlyPrice}</Text>
          <Text style={styles.planPeriod}>per month</Text>
          <Text style={styles.planCommission}>
            + {data.pricing.PREMIUM.commissionRate}% per booking
          </Text>
          <Text style={styles.planBonus}>
            (+{data.current.newStudentBonus}% bonus on first booking with new clients)
          </Text>
          ...
          <View style={styles.features}>
            {data.pricing.PREMIUM.features.map((feature, idx) => (
              <View key={idx} style={styles.feature}>
                <Text style={styles.featureCheck}>✓</Text>
                <Text style={styles.featureText}>{feature}</Text>
              </View>
          ...
          {data.current.tier !== 'PREMIUM' && (
            <TouchableOpacity
              onPress={() => changeTier('PREMIUM')}
              disabled={updating}
              style={styles.switchButton}
            >
              <Text style={styles.switchButtonText}>
                {updating ? 'Updating...' : 'Upgrade to PREMIUM'}
              </Text>
            </TouchableOpacity>
          )}
        </View>

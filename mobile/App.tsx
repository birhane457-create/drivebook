...
export default function App() {
  ...
  const [userRole, setUserRole] = useState<'provider' | 'customer' | null>(null);
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const rawRole = (response.data.user?.role || 'provider').toString();
      const role = rawRole.toLowerCase() === 'customer' ? 'customer' : 'provider';
      setUserName(response.data.user.name);
      setUserRole(role as 'provider' | 'customer');
      setIsLoggedIn(true);
      // Set initial screen based on role
      setCurrentScreen(role === 'customer' ? 'clientDashboard' : 'dashboard');
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      ...
      case 'profile':
        return userRole === 'customer' ? <ClientProfileScreen /> : <ProfileScreen />;
      case 'settings':
        return userRole === 'customer' ? <ClientSettingsScreen /> : <SettingsScreen />;
      case 'help':
        return <HelpScreen />;
      default:
        return userRole === 'customer' ? (
      ...
      >
        <View style={styles.drawerHeader}>
          ...
          <Text style={styles.drawerSubtitle}>
            {userName} • {userRole === 'customer' ? '👨‍🎓 Student' : '👨‍🏫 Instructor'}
          </Text>
        </View>
        ...
        <View style={styles.drawerContent}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => navigateTo(userRole === 'customer' ? 'clientDashboard' : 'dashboard')}
          >

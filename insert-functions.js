const fs = require('fs');
const path = 'lib/services/booking-service.ts';
...
export async function approveCancellation(
  ...
  const stripe = (await import('@/lib/services/stripe')).default
  ...
  try {
    stripeRefund = await stripe.refunds.create({
      ...
      metadata: { bookingId, adminId, calculatedRefund: refundAmount.toString(), adminNote: adminNote || '' },
    })
    ...
    await writeAudit('CANCELLATION_APPROVED', adminId, 'ADMIN', bookingId, {
      refundAmount, stripeRefundId: stripeRefund.id, walletDebitId,
      overrideAmount: overrideAmount || null, adminNote: adminNote || null,
    })
    ...
    if (stripeRefund?.id) {
      console.error('[CRITICAL] Stripe refund succeeded but wallet debit failed:', {
        bookingId, stripeRefundId: stripeRefund.id,
      ...
      await writeAudit('CANCELLATION_PARTIAL_FAILURE', adminId, 'ADMIN', bookingId, {
        ...
        stripeRefundId: stripeRefund.id, refundAmount, requiresManualIntervention: true,
      })
    }
    throw error
  }
...
export async function rejectCancellation(
  ...
  await writeAudit('CANCELLATION_REJECTED', adminId, 'ADMIN', bookingId, { reason })
...
console.log('✅ Successfully added 3 new functions to booking-service.ts');
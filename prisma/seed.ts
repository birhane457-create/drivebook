...
async function main() {
  ...
  const instructor1 = await prisma.provider.create({
    data: {
      name: 'John Smith',
      phone: '+61412345678',
      hourlyRate: 65.0,
  ...
  const instructor2 = await prisma.provider.create({
    data: {
      name: 'Sarah Johnson',
      phone: '+61423456789',
      hourlyRate: 70.0,
  ...
  const instructor3 = await prisma.provider.create({
    data: {
      name: 'Michael Chen',
      phone: '+61434567890',
      hourlyRate: 60.0,

...
|------------|------|---------|----------|---------|----------|--------------|---------------|
| **P0-01** | Wallet Operations | ✅ CONFIRMED | CRITICAL | P0 | `app/api/client/wallet-add/route.ts` lines 75-102: No ownership check on PaymentIntent. Any user can credit wallet with any succeeded payment. | Add metadata check: `if (paymentIntent.metadata.userId !== session.user.id) throw new Error('Unauthorized')`. Store userId in metadata during payment intent creation. | Negative test: User A creates payment, User B attempts to credit wallet with User A's paymentIntentId → should fail |

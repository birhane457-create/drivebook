...
- **Fix**: Go to Settings â†’ Billing â†’ Customer Portal â†’ Activate
...
1. âœ… Create 3 products in Stripe (Basic, Pro, Business)
2. âœ… Add 6 price IDs to `.env` (monthly + annual for each)
3. âœ… Enable billing portal in Stripe settings
4. âœ… Restart server

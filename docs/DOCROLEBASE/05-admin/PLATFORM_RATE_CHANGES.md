...
model PlatformRateChange {
  ...
  tier            String    // BASIC | PRO | STUDIO | PREMIUM | ALL
...
2. **Create New Change:**
   - Form with fields:
     - Tier (BASIC | PRO | STUDIO | PREMIUM | ALL)
...
**Setup:**
- Create rate changes for PRO (12% → 11%) and PREMIUM (10% → 9%), same effective date

**Test:**
1. Both changes applied by single cron run
2. PRO instructors notified only (not BASIC)
3. PREMIUM providers notified only (not PRO)

...
### White-Label / Display Name

Receipt functions A, B, C, and E accept an optional `provider?: { name: string; businessName?: string | null }` parameter alongside the `instructorName: string` fallback. When `provider` is supplied, the function calls `getDisplayName(provider)` which returns `businessName` if set, otherwise `name`. This ensures receipts show the business name (e.g. "Perth Drive Academy") rather than the personal name for PREMIUM tier providers.

Call sites that pass a full Provider object should use the `provider` param. Legacy callers that only have a name string continue to work via `instructorName`.

...
Note: The `newStudentBonus` concept was removed in May 2026. Commission is now a flat rate per tier with no first-booking modifier. The `isFirstBooking` flag is still recorded on bookings for analytics but no longer affects the commission calculation.
...
3. Falls back to hardcoded defaults in `lib/config/subscriptions.ts` if no DB record exists
...
Admins can schedule commission rate changes with a future effective date via `/admin/pricing` → Rate Change Scheduler. This gives instructors advance notice before rates change.

- Instructors see a notice on their subscription page showing the upcoming change
- The `apply-rate-changes` cron applies the change on the effective date and notifies all affected instructors
- Only new bookings from the effective date use the new rate

See `docs/DOCROLEBASE/05-admin/SETTINGS.md` for full details.
...
This is separate from commission. Applied at payout time, not at booking time:
...
An ABN on file alone is not sufficient — it must be verified via the ABR API or by an admin before withholding is waived.
...
For advance-notice rate changes, use the Rate Change Scheduler instead of direct updates.

---

## Where Rates Are NOT Stored

`commissionRate` is NOT stored on the `Instructor` model. It is always derived at runtime from `PlatformSettings` (or `lib/config/subscriptions.ts` as fallback).
...
- `docs/DOCROLEBASE/05-admin/SETTINGS.md` — Admin pricing configuration and rate change scheduler
...
`commissionRate` is NOT stored on the `Instructor` model. It is always derived at runtime from `PlatformSettings` (or `lib/config/subscriptions.ts` as fallback). The `newStudentBonus` field was removed from the system in May 2026.

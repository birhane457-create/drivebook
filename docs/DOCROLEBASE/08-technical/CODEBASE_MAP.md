...
**What it does:** Instructor subscription tiers (BASIC/PRO/STUDIO/PREMIUM), Stripe billing, trial periods, scheduled rate changes.
...
| DB model | `Provider` in `prisma/schema.prisma` |
| DB extension | `DrivingProviderProfile` in schema (driving-specific: vehicle, licence, insurance, PDA, voice line) |
...
- `commissionRate` is NEVER stored on `Provider` — always derived from `PlatformSettings`

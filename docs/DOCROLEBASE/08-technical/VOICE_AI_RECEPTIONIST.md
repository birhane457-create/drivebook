...
Returns `voice.confirmation` (pre-assembled) and a full `voice` object for the AI to read from.

**Book Now response — full `voice` object:**
```json
{
  "voice": {
    "instructor":      "Perth Drive Academy",
    "package":         "10 Hour Package",
    "packageHours":    10,
    "scheduledHours":  1.0,
    "scheduledLessons": 1,
    "remainingHours":  9.0,
    "firstLesson":     "Monday 10:00 AM",
    "paymentRequired": true,
    "slotHeldMinutes": 10,
    "pickupVerified":  true,
    "confirmation":    "Your 10 Hour Package with Perth Drive Academy is reserved for 10 minutes..."
  }
}
```

**Book Later response:** Returns `checkoutUrl` (Stripe hosted checkout URL). No `voice` object. No `Booking` row until Stripe `checkout.session.completed` webhook fires after payment. The hybrid server SMS's the `checkoutUrl` to the student's phone.

**Key distinction:**
- `voice.instructor` = `getDisplayName(instructor)` — the business name if set, personal name otherwise
- `voice.voiceName` (from `findInstructors`) = phonetic spelling for TTS — different field, different purpose
...
- Read `voice.voiceName` (from `findInstructors`) for phonetic TTS in search results — never pronounce raw `name` field
- Read `voice.instructor` (from `createBooking`) for booking confirmation — this is `getDisplayName()` output (business name if set)

...
**CLIENT** — A learner driver who books lessons. Has a `User` record with `role: "CLIENT"`, optionally a `ClientWallet`, and one or more `Customer` records (one per provider relationship).

**INSTRUCTOR** — A driving instructor or tradie provider. Has a `User` record with `role: "INSTRUCTOR"` and a `Provider` record with profile, subscription, and branding data. Driving-specific fields (vehicle, licence, insurance, PDA, voice line) are in a linked `DrivingProviderProfile` record.
...
**abnVerified** — Boolean on the `Provider` model. `true` only after an admin or the ABR API confirms the ABN is active and matches the provider's name.

**abnStatus** — The current ABR status of the provider's ABN. Possible values:
...
**Tier** — The instructor's subscription level: `BASIC`, `PRO`, `STUDIO`, or `PREMIUM`. (`BUSINESS` is a future multi-provider tier — shown as Coming Soon.)

**Trial** — A free period before payment is required. BASIC/PRO/STUDIO: 14 days. PREMIUM: 30 days.

**trialEndsAt** — The date the trial expires. Stored on both `Provider` and `Subscription` models.
...
**Subdomain** — A provider's public booking page at `[slug].drivebook.com.au`. Configured via `customSlug` on the `Provider` model.

**Branded Booking Page** — When `showBrandingOnBookingPage: true`, the subdomain page shows the provider's logo, colors, and business name instead of the DriveBook defaults. Requires PRO, STUDIO, or PREMIUM tier.
...
**serviceAreas** — A string field on `Provider` describing the geographic areas they cover.

**serviceRadiusKm** — The radius (in km) from the provider's base address within which they operate.

**stripeConnectStatus** — A **computed runtime field** (not stored in DB) derived from `Provider.stripeAccountId`. Returns `'connected'` if `stripeAccountId` is set, `'not_connected'` otherwise. Do not treat this as a persisted field.

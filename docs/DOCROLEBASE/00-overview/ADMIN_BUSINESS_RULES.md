...
| Enable or configure the PREMIUM tier | [§7 Premium Tier Activation](#7-premium-tier-activation) |
...
- Activates their trial subscription (14 days for BASIC/PRO/STUDIO, 30 days for PREMIUM)
...
- [ ] Stripe PREMIUM monthly and annual products created in Stripe dashboard
...
### Before manually setting a provider to PREMIUM tier:

- [ ] Instructor has set a school name in branding settings (`businessName`)
- [ ] ABN is verified (PREMIUM accounts must have ABN for payouts)
- [ ] Instructor understands multi-instructor features are Coming Soon
- [ ] SUPER_ADMIN approval required

### On PREMIUM activation (webhook handles automatically):
...
- Manually set `subscriptionTier = PREMIUM` via DB without using the admin UI — no audit trail
...
- [ ] Instructor must be on PRO, STUDIO, or PREMIUM tier

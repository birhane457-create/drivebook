...
| PREMIUM commission rate (DB field: businessCommissionRate) | `businessCommissionRate` | 10% | `getCommissionRate()`, all booking routes |
...
| 15-hour package discount | `package15Discount` | 12% | `calculatePackagePriceDynamic` (⚠️ See note below) |
...
### ⚠️ Package Tiers Update (September 2026)

**Package structure (hours, labels) is now DB-driven.** The 3 named discount columns above (`package6Discount`, `package10Discount`, `package15Discount`) are kept for backward compatibility but are now superseded by:

**`PlatformSettings.packageTiers` (JSONB)**
```json
[
...
**What changed:**
- Admin can now **add, remove, or edit tiers** from `/admin/pricing` UI (dynamic table)
- Discount % changes apply instantly (no deploy)
- Tier structure changes (add 20hr tier, drop 6hr tier) need `PREDEFINED_PACKAGES` updated in code for marketing pages

**Migration details:** `docs/PACKAGE_TIERS_MIGRATION.md` and `docs/DOCROLEBASE/06-payments/PRICING_CONFIGURATION_GUIDE.md` section 12.
...
| PREMIUM monthly price | `PREMIUM_MONTHLY_PRICE` | $199 | `lib/config/subscriptions.ts` |
| PREMIUM annual price | `PREMIUM_ANNUAL_PRICE` | $1,990 | `lib/config/subscriptions.ts` |
| Trial days BASIC/PRO/STUDIO | `BASIC_TRIAL_DAYS` etc | 14 | `lib/config/subscriptions.ts` |
| Trial days PREMIUM | `PREMIUM_TRIAL_DAYS` | 30 | `lib/config/subscriptions.ts` |
...
| `STRIPE_PREMIUM_MONTHLY_PRICE_ID` | Stripe price object for PREMIUM monthly ⚠️ not yet created in live mode |
| `STRIPE_PREMIUM_ANNUAL_PRICE_ID` | Stripe price object for PREMIUM annual ⚠️ not yet created in live mode |
...
| Package discounts (%) | `/admin/pricing` → PlatformSettings.packageTiers (or legacy named columns) |
| Package tiers (hours, labels, featured) | `/admin/pricing` → dynamic tier table → PlatformSettings.packageTiers |

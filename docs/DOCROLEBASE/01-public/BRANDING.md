...
This is separate from the DriveBook brand system. Instructors on PRO, STUDIO, and PREMIUM tiers can upload their own logo and brand colours, which appear on their booking page, email confirmations, and student dashboard header.
...
Only applies on `/subdomain/[slug]` and `/book/[instructorId]` pages when instructor has PRO, STUDIO, or PREMIUM subscription.

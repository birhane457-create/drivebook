...
1. Create booking: `POST /api/public/bookings/bulk`
   - **Book Now** returns: `{ success: true, bookingType: 'now', bookingIds: [...], total, voice: { instructor, package, packageHours, scheduledHours, scheduledLessons, remainingHours, firstLesson, paymentRequired, slotHeldMinutes, pickupVerified, confirmation } }`
   - **Book Later** returns: `{ success: true, bookingType: 'later', total, checkoutUrl, paymentIntentId }` — no `voice` object, no Booking row until Stripe `checkout.session.completed` fires

2. **Book Now path:** Create Stripe payment intent: `POST /api/payments/create-intent`
...
> **Note:** The `checkoutUrl` for Book Later is a Stripe-hosted checkout page (`checkout.stripe.com/...`). The hybrid voice server also SMS's this URL to the student's phone. The wallet is credited only after `checkout.session.completed` fires — no wallet transaction is created before payment.
...
**Input:** Package type, hours, scheduled bookings, registration details, `providerId` OR `instructorQuery` (name/phone — used by voice AI)

**Output:**
- Book Now: `{ success, bookingType: 'now', bookingIds, total, voice: { instructor, package, packageHours, scheduledHours, remainingHours, firstLesson, paymentRequired, slotHeldMinutes, pickupVerified, confirmation } }`
- Book Later: `{ success, bookingType: 'later', total, checkoutUrl, paymentIntentId }`

**Creates:**
- New `User` account (if email doesn't exist)
- `Customer` record (linked to provider relationship)
- 1+ `Booking` records (one per scheduled lesson or parent package booking)
- Book Now: `Transaction` record; wallet credited then debited for first lesson
- Book Later: Stripe Checkout Session — wallet credited only after `checkout.session.completed` webhook fires

**Features:**
- `Idempotency-Key` header support — replays stored response on retry (Twilio/voice safety). Keys persisted in `BookingIdempotencyKey` table
- `instructorQuery` fallback — resolves instructor by phone number or name when `providerId` not supplied (voice AI path)
- `paymentMode=DIRECT` guard — returns `503 SERVICE_UNAVAILABLE` with code `PAYMENT_MODE_NOT_IMPLEMENTED` if provider has direct payments configured (phase 2, not yet live)
...
- `POST /api/payments/create-intent` - Create Stripe PaymentIntent (Book Now only)
  - Input: `bookingId`
  ...
  - Note: Book Later skips this entirely — it uses a Stripe Checkout Session created inside `bulk/route.ts`

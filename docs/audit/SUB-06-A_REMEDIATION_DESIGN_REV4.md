# SUB-06-A Remediation Design (REVISION 4 - IMPLEMENTATION READY)

**Finding:** Out-of-order webhook delivery can produce invalid subscription state  
**Severity:** MEDIUM  
**Status:** DESIGN PHASE - REVISION 4 (IMPLEMENTATION READY)  
**Discovery Commit:** `13e7a038`  
**Rev 2 Commit:** `5319cb6b` (REJECTED - 9 issues)  
**Rev 3 Commit:** `4422111f` (CHANGES REQUIRED - 10 issues)  
**Rev 4 Date:** 2026-09-26

---

## Revision History

### Revision 4 (Current - Implementation Ready)
**All database mechanisms explicitly specified. No unresolved placeholders.**

**Addresses ALL 10 Rev 3 issues:**

1. ✅ **Mandatory SELECT FOR UPDATE locking**
2. ✅ **Atomic idempotency with unique constraint**
3. ✅ **Deterministic T7/T8 test orders**
4. ✅ **Complete Stripe event history retrieval**
5. ✅ **Enforceable migration/webhook control**
6. ✅ **Manual sync convergence defined**
7. ✅ **Database-enforced Stripe ID uniqueness**
8. ✅ **Provider pointer consistency invariant**
9. ✅ **SUB-22 integration explicit**
10. ✅ **Separate retention state**

### Revision 3 (Changes Required)
...
-- Performance index (non-unique, for lookups)
...
### 2.1 Mandatory Row Locking (SELECT FOR UPDATE)
 ...
 * MANDATORY for all webhook handlers (Rev 3 Issue #1).
 * 
...
**Alternative (if subscription doesn't exist yet):**
```typescript
// For subscription creation, lock Provider row instead
...
### 2.3 Atomic Idempotency (Unique Constraint Inside Transaction)
  ...
  try {
    ...
    if (error.code === 'P2002' && error.meta?.target?.includes('idempotencyKey')) {
      ...
      if (existing?.processed) {
        return { claimed: false, reason: 'already-processed-successfully' };
      }
      
      // Other request is still in-progress (rare race)
      // Conservative: treat as duplicate (idempotency wins over at-most-once)
      return { claimed: false, reason: 'concurrent-request-in-progress' };
    }
...
async function handleWebhookEvent(
  ...
  await withSerializableRetry(async () => {
    await prisma.$transaction(async (tx) => {
      ...
      // ═══════════════════════════════════════════════════════════════════
      // STEP 2: Lock subscription row (Rev 3 Issue #1)
      // ═══════════════════════════════════════════════════════════════════
      ...
      if (!subscription) {
        ...
        await handleNewSubscriptionCreation(tx, event, idempotencyKey);
        return;
      }
      
      // Load related Provider (already within transaction)
      const provider = await tx.provider.findUnique({
        where: { id: subscription.providerId }
      });
      
      if (!provider) {
        throw new Error(`Provider ${subscription.providerId} not found`);
      }
...
### 3.3 Provider Pointer Consistency (INV-3, Rev 3 Issue #8)
...
**A. Application-Level (on every subscription change):**
```typescript
...
**B. Database-Level Check (assertion):**
```sql
-- Verification query (run in tests or periodic health check)
...
-- Must return 0 rows (no provider with multiple active subscriptions)
```

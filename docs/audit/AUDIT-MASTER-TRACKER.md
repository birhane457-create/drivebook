...
| ID | Title | Risk | Finding | Verification | Fix | Fix-Verified | Status | Evidence | Phase-1-ref |
|---|---|---|---|---|---|---|---|---|---|
| PAY-H-04 | SlotReservation concurrency — no unique constraint | MEDIUM | CONFIRMED | VERIFIED — no `@@unique([providerId, startTime])` | REMEDIATION DESIGN IN PROGRESS — `@@unique([providerId, startTime])` rejected (insufficient); partial exclusion `WHERE (expiresAt > NOW())` REJECTED (NOW() is STABLE not IMMUTABLE); all-rows GIST exclusion constraint is candidate; Q1 btree_gist production query pending; Q5 Prisma error code prototype required; hostile baseline test required before migration | PENDING | ⚠️ OPEN — design not yet authorised | `docs/audit/PAY-H-04_INVESTIGATION.md` | PAY-H-04 |

...
| INT-M-03A | OAuth tokens stored plaintext | HIGH | CONFIRMED | VERIFIED — `googleAccessToken` plaintext in Provider model | `lib/encryption/oauth-tokens.ts` (AES-256-GCM, 12-byte nonce, v1 format); `googleCalendar.ts` encrypt/decrypt integrated | **⏳ REOPENED — Independent audit (2026-08-15) found migration tests simulate logic rather than execute actual migration script. 71/71 tests pass but do not establish: (1) actual migration script execution, (2) migration idempotency via second run, (3) database-wide SQL plaintext scan, (4) migration atomicity/recovery. Core encryption implementation verified sound. Requires: actual `migrate-encrypt-oauth-tokens.mjs` execution tests.** | ⏳ REOPENED — TEST VERIFIED blocked pending actual migration script execution evidence | `docs/audit/INT-M-03A_AUDIT_FINDINGS.md` | INT-M-03A |

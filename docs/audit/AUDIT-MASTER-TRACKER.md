...
| INT-M-03A | OAuth tokens stored plaintext | HIGH | CONFIRMED | VERIFIED — `googleAccessToken` plaintext in Provider model | `lib/encryption/oauth-tokens.ts` (AES-256-GCM, 12-byte nonce, v1 format); `googleCalendar.ts` encrypt/decrypt integrated | 71/71 tests pass (39 unit + 21 integration + 11 migration); test DB verified: plaintext→encrypted, idempotency, fail-closed, DB inspection (no ya29./1//), lifecycle regression | ✅ TEST VERIFIED (test database) — production migration pending | `docs/audit/INT-M-03A_MIGRATION_EXECUTION_LOG.md` | INT-M-03A |

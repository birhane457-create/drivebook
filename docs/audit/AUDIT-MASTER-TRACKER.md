...
| INT-M-PKG-01 | client/packages/mobile IDOR + hardcoded price | MEDIUM | CONFIRMED | VERIFIED — `app/api/client/packages/mobile/route.ts` POST handler (lines 131–216): `packageId` from request body used directly as `providerId` with no ownership/relationship check; `price: 500` hardcoded (line 196); `status: 'CONFIRMED'` immediate (line 197); route is actively used by mobile app (`mobile/services/api.ts` + `WalletScreen.tsx`) | CONTAINMENT — Commit `80c31f97`: Server-side kill switch (fail closed); POST returns 503 unless ENABLE_MOBILE_PACKAGE_PURCHASE=true; GET preserved. Production deployed `caf841df` (2026-08-15) | 10 unit tests (U1-U3), exit 0; 15 HTTP integration tests (C1-C7), exit 0; Production HTTP: 3/3 tests reported passed (local execution), deployed SHA `caf841df` reported via health endpoint; independent GitHub evidence retrieval pending | ⚠️ OPEN — Containment production verification reported (local execution confirmed); independent repository evidence pending/reconciliation; root cause unfixed | `docs/audit/INT-M-PKG-01_PRODUCTION_VERIFIED_EVIDENCE.md` | INT-M-PKG-01 |
...
| MM-12-HTTP | Concurrent same-key race returns HTTP 500 instead of 409 | LOW | CONFIRMED | VERIFIED — production Check 3: Request B → 500 during IDEMPOTENCY_IN_FLIGHT race; financial invariant unaffected (1 tx, correct balance); API contract violated | NOT-STARTED | PENDING | ⚠️ OPEN — API quality defect, not financial security issue | `docs/audit/MM-12-HTTP-500-FOLLOW-UP.md` |

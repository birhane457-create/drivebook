...
| INT-M-PKG-01 | client/packages/mobile IDOR + hardcoded price | MEDIUM | CONFIRMED | VERIFIED — `app/api/client/packages/mobile/route.ts` POST handler (lines 131–216): `packageId` from request body used directly as `providerId` with no ownership/relationship check; `price: 500` hardcoded (line 196); `status: 'CONFIRMED'` immediate (line 197); route is actively used by mobile app (`mobile/services/api.ts` + `WalletScreen.tsx`) | CONTAINMENT — Commit `80c31f97`: Server-side kill switch (fail closed); POST returns 503 unless ENABLE_MOBILE_PACKAGE_PURCHASE=true; GET preserved | 10 unit tests (U1-U3), exit 0 — code-level verification; HTTP integration tests pending; production verification pending | ⚠️ OPEN — CONTAINMENT CODE-VERIFIED; HTTP AND PRODUCTION VERIFICATION PENDING; ROOT CAUSE UNFIXED | `phase2/INT-M-PKG-01-TEST-EXECUTION.md` | INT-M-PKG-01 |

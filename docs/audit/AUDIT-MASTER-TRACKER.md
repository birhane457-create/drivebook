...
| DOC-EXP-01 | Expired provider can still receive bookings | MEDIUM | CONFIRMED | VERIFIED — no document expiry gate in booking creation | New `lib/booking/checkProviderEligible.ts` helper adds full gate (approvalStatus, isActive, subscription, acceptingBookings, DrivingProviderProfile expiry dates). Applied to `public/bookings/route.ts` (also adds missing approvalStatus+isActive) and `public/bookings/bulk/route.ts` (adds expiry gate). Cron extended to flip `documentsVerified=false` on already-expired docs. | Logic review complete — 5 checks verified in source. Public booking paths now enforce doc expiry gate. Authenticated-operator paths (combined, mobile, pda-tests) noted as follow-up. See `docs/audit/DOC-EXP-01-VERIFICATION.txt` | ✅ CLOSED | `docs/audit/DOC-EXP-01-VERIFICATION.txt` | DOC-EXP-01 |

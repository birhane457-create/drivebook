...
const ABC_THRESHOLDS = { A: 0.8, B: 0.95 }; // A: top 80% revenue, B: next 15%
...
export default function InventoryAnalytics() {
  ...
  const analytics = useMemo(() => {
    ...
    // Inventory Turnover (annualized): COGS / Avg Inventory Value
    const turnoverData = products.map(p => {
      ...
      const turnover = avgInventoryValue > 0 ? (cogs / avgInventoryValue) * (365 / 30) : 0; // rough annualization over 30-day window
    ...
    const counts = { A: classified.filter(p => p.abc_class === 'A').length, B: classified.filter(p => p.abc_class === 'B').length, C: classified.filter(p => p.abc_class === 'C').length };
  ...
  return (
    <div>
      <PageHeader title="Inventory Analytics" subtitle="ABC classification, turnover analysis, and inventory health" />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard title="Class A Items" value={analytics.counts.A} icon={Star} trend="Top 80% revenue" trendUp />
        <StatCard title="Class B Items" value={analytics.counts.B} icon={TrendingUp} trend="Next 15% revenue" trendUp />
        <StatCard title="Class C Items" value={analytics.counts.C} icon={AlertTriangle} trend="Bottom 5%" />
        <StatCard title="Avg Turnover" value={`${analytics.avgTurnover}x`} icon={RotateCcw} trend="annualized" trendUp />
      </div>
      ...
      <Tabs defaultValue="abc" className="space-y-6">
                ...
                {analytics.classified.map((p, i) => (
                  <div key={p.id} className="flex items-center gap-3 py-2 border-b last:border-0">
                    ...
                    <div
                      ...
                      style={{ background: ABC_COLORS[p.abc_class] }}
                    >
                      {p.abc_class}
                    </div>
        ...
        <TabsContent value="turnover">
          <Card>
            <CardHeader><CardTitle className="text-base">Inventory Turnover Ratio (Annualized)</CardTitle></CardHeader>
            <CardContent>
              ...
              <div className="space-y-2 max-h-[500px] overflow-y-auto">
                {analytics.turnoverData.filter(p => p.days_on_hand < 999).sort((a, b) => b.days_on_hand - a.days_on_hand).slice(0, 20).map(p => (
                  <div key={p.id} className="flex items-center gap-3 py-2 border-b last:border-0">
                    ...
                    <div className="text-right w-32">
                      <Progress
                        value={Math.min(100, (p.days_on_hand / 120) * 100)}
                        className="h-2"
                      />
                    </div>
                  </div>

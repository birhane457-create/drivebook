...
export default function Reports() {
  const salesQ = useQuery({ queryKey: ['sales'], queryFn: () => base44.entities.Sale.list('-created_date', 500) });
  ...
  const poQ = useQuery({ queryKey: ['purchase-orders'], queryFn: () => base44.entities.PurchaseOrder.list() });
  ...
  const queries = [salesQ, productsQ, stockQ, poQ];
  ...
  const dailySales = {};
  for (let i = 29; i >= 0; i--) { const d = format(subDays(new Date(), i), 'yyyy-MM-dd'); dailySales[d] = 0; }
  sales.forEach(s => { const d = s.created_date?.substring(0, 10); if (d && dailySales[d] !== undefined) dailySales[d] += s.grand_total || 0; });
  const dailyChartData = Object.entries(dailySales).map(([date, total]) => ({ date: format(new Date(date), 'MMM d'), total: Math.round(total * 100) / 100 }));

  const productSales = {};
  sales.filter(s => s.status === 'completed').forEach(s => {
    (s.items || []).forEach(item => {
      if (!productSales[item.product_id]) productSales[item.product_id] = { name: item.product_name, quantity: 0, revenue: 0 };
  ...
  const kpis = [
    ...
    { label: 'Gross Profit', value: `$${grossProfit.toFixed(0)}`, icon: TrendingUp, trend: `${margin}% margin`, trendUp: grossProfit > 0 },
    { label: 'Inventory Value', value: `$${inventoryValue.toFixed(0)}`, icon: Package },
    { label: 'Purchase Orders', value: purchaseOrders.length, icon: BarChart3 },
  ];
  ...
  return (
    ...
    >
      {firstError ? (
        <ErrorState title="Couldn't load report data" message={firstError?.message} onRetry={refetchAll} />
      ) : (

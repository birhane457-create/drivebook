...
export default function BookingSummary({ 
  ...
  if (!instructor) {
    return (
      <div className="bg-gradient-to-br from-white/5 to-white/2 rounded-2xl p-4 sm:p-6 sticky top-4 border border-white/20 backdrop-blur-sm text-foreground">
        <h3 className="text-lg font-semibold mb-4 text-foreground/90">Booking Summary</h3>
        <p className="text-foreground/70 text-sm">Select an instructor to begin</p>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="bg-gradient-to-br from-white/5 to-white/2 rounded-2xl shadow-2xl p-4 sm:p-6 sticky top-4 border border-white/20 backdrop-blur-sm text-foreground">
        <h3 className="text-lg font-semibold mb-4 text-foreground/90">Booking Summary</h3>

        {/* Instructor Info */}
        <div className="flex items-center gap-3 mb-6 pb-6 border-b border-white/6">
          <div className="relative w-16 h-16 rounded-full overflow-hidden bg-secondary/50 flex-shrink-0">
            {instructor.profileImage ? (
              ...
              <div className="w-full h-full flex items-center justify-center text-foreground/70 text-2xl font-bold">
                {instructor.name.charAt(0)}
              </div>
            )}
          </div>
          <div className="flex-1 min-w-0">
            <h4 className="font-semibold text-foreground truncate">{instructor.name}</h4>
            <div className="flex items-center gap-1 text-sm">
              <span className="text-yellow-400">★</span>
              <span className="text-foreground/80">
                {instructor.averageRating?.toFixed(1) || 'New'}
              </span>
              <span className="text-foreground/60">({instructor.totalReviews})</span>
            </div>
          </div>
        </div>

        {/* COMPACT Package Details - 2 LINES */}
        <div className="bg-secondary/50 rounded-lg p-3 mb-6 pb-6 border-b border-white/6">
          <div className="flex items-start justify-between gap-2">
            <div className="flex-1">
              <p className="text-xs font-semibold text-foreground mb-1">{packageInfo.name}</p>
              <p className="text-sm text-foreground/80">{hours}h • ${instructor.hourlyRate}/h</p>
            </div>
            <button
              onClick={() => setShowPackageInfo(true)}
              className="text-foreground/80 hover:text-foreground p-1 flex-shrink-0"
        ...
        {instructor.offersTestPackage && !pdaTestBooking && (
          <div className="bg-secondary/50 rounded-lg p-4 mb-6 border border-white/20">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-semibold text-foreground">Add PDA test pack</p>
                <p className="text-xs text-foreground/70 mt-1">Optional driving test preparation add-on before scheduling and payment.</p>
              </div>
              <button
                type="button"
                onClick={toggleTestPackage}
                className={`px-4 py-2 rounded-lg text-sm font-semibold transition ${includeTestPackage ? 'bg-destructive text-foreground hover:bg-red-500' : 'bg-primary text-foreground hover:bg-blue-500'}`}
              >
                {includeTestPackage ? 'Remove add-on' : 'Add PDA pack'}
              </button>
            </div>
            <div className="mt-3 text-xs text-foreground/70 flex flex-wrap gap-3">
        ...
        <div className="space-y-2 mb-6 pb-6 border-b border-white/6">
          ...
          {pricing.discount > 0 && (
            <div className="flex justify-between text-sm">
              <span className="text-emerald-400">Discount ({pricing.discountPercentage}%)</span>
              <span className="text-emerald-400">-${pricing.discount.toFixed(2)}</span>
            </div>
          )}

          {includeTestPackage && pricing.testPackage > 0 && (
            <div className="flex justify-between text-sm">
              <span className="text-foreground/70">Test Package</span>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <span className="text-2xl font-bold text-foreground">${pricing.total.toFixed(2)}</span>
          </div>
          ...
          {(bookingState.scheduledBookings.length > 0 || bookingState.includeTestPackage) && (
            <div className="space-y-2 mb-6">
              {bookingState.scheduledBookings.length > 0 && (
                <div className="bg-green-600 rounded-lg p-3 border border-green-500">
                  <p className="text-xs text-white font-semibold mb-1">✓ Lessons Scheduled: {bookingState.scheduledBookings.length}</p>
                  {bookingState.remainingHours > 0 && (
                    <p className="text-xs text-white">Extra hours to schedule: {bookingState.remainingHours.toFixed(1)}h</p>
                  )}
                </div>
              )}
              
              {pdaTestBooking && (
                <div className="bg-blue-800/20 rounded-lg p-3 border border-primary/25">
              ...
              {bookingState.includeTestPackage && !pdaTestBooking && (
                <div className="bg-blue-800/20 rounded-lg p-3 border border-primary/25">
                  ...
                  <p className="text-xs text-blue-200">{bookingState.provider?.testPackageDuration ? `${Math.floor((bookingState.provider.testPackageDuration || 165) / 60)}h ${((bookingState.provider.testPackageDuration || 165) % 60)}m` : '2h 45m'} test preparation</p>
                  <p className="text-xs text-primary mt-1">Use the "Schedule PDA Test" tab to book your test</p>
                </div>
              )}
            </div>
          )}
          ...
          {showButtons && (
            <div className="mt-6 pt-6 border-t border-white/6 flex flex-col sm:flex-row gap-3">
              <button
                onClick={onBack}
                disabled={loading}
                className="flex-1 sm:flex-initial px-4 py-2 bg-secondary/50 border border-white/8 text-foreground/90 rounded-lg font-semibold hover:bg-secondary/50 transition-colors disabled:bg-gray-700 disabled:cursor-not-allowed"
              >
                {backText}
              </button>
              <button
                onClick={onContinue}
                disabled={loading}
                className="flex-1 sm:flex-initial px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-foreground rounded-lg font-semibold hover:from-purple-500 hover:to-pink-500 transition-colors disabled:bg-gray-700 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
      ...
      {showPackageInfo && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-4 sm:p-6 animate-in fade-in">
            ...
            <button
              onClick={() => setShowPackageInfo(false)}
              className="w-full mt-6 bg-primary text-foreground font-semibold py-2 rounded-lg hover:bg-primary/90 transition-colors"
            >
              Got it
            </button>
          </div>

      ...
      {message && (
        <div className={`mb-4 p-3 rounded-lg flex items-start gap-2 ${
          message.includes('Error') || message.includes('Failed')
            ...
            : 'bg-green-900/30 text-emerald-400 border border-green-700/50'
        }`}>
          {message.includes('Error') || message.includes('Failed') ? (
        ...
        {connected ? (
          <div className="space-y-4">
            ...
            <div className="bg-background border border-border rounded-lg p-4">
              <h3 className="font-medium text-foreground mb-3">PDA Test Buffer Mode</h3>
              <p className="text-sm text-muted-foreground mb-3">
                Choose how the system handles PDA test blocking times:
              </p>

              <div className="space-y-2">
                <label className={`flex items-start gap-3 p-3 border-2 rounded-lg cursor-pointer transition ${bufferMode === 'auto' ? 'border-sky-500 bg-sky-900/20' : 'border-border hover:border-border'}`}>
                  <input
                    type="radio"
                    name="bufferMode"
                  ...
                  <div className="flex-1">
                    <div className="font-medium text-foreground">Automatic Buffer</div>
                    <div className="text-sm text-muted-foreground">
                      System automatically blocks 2 hours before + 1 hour after PDA tests.
                      <br />
                      <span className="text-xs italic text-muted-foreground/60">Example: Test at 10:00-11:00 → Blocks 8:00-12:00</span>
                    </div>
                  </div>
                </label>

                <label className={`flex items-start gap-3 p-3 border-2 rounded-lg cursor-pointer transition ${bufferMode === 'manual' ? 'border-sky-500 bg-sky-900/20' : 'border-border hover:border-border'}`}>
                  <input
                    type="radio"
                    name="bufferMode"
                  ...
                  />
                  <div className="flex-1">
                    <div className="font-medium text-foreground">Manual Buffer</div>
                    <div className="text-sm text-muted-foreground">
                      You set the exact times in Google Calendar (include buffer yourself).
                      <br />
                      <span className="text-xs italic text-muted-foreground/60">Example: Create event 8:00-12:00 → Blocks 8:00-12:00</span>
                    </div>
                  </div>
                </label>
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <button
                ...
                onClick={handleSync}
                disabled={syncing}
                className="flex-1 bg-primary text-foreground px-4 py-2 rounded-lg hover:bg-primary/90 disabled:opacity-50 flex items-center justify-center gap-2"
              >
              ...
              <button
                ...
                disabled={loading}
                className="flex-1 bg-destructive text-foreground px-4 py-2 rounded-lg hover:bg-destructive/90 disabled:opacity-50 flex items-center justify-center gap-2"
              >
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-primary text-foreground px-4 py-3 rounded-lg hover:bg-primary/90 disabled:opacity-50 flex items-center justify-center gap-2"
            >

...
export default function CompactInstructorCard({ provider: instructor, onSelect }: CompactInstructorCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="flex items-start justify-between gap-2 mb-1">
          ...
          <div className="flex items-center gap-0.5 shrink-0">
            ...
            {instructor.totalReviews > 0 && (
              <span className="text-xs text-muted-foreground ml-0.5">({instructor.totalReviews})</span>
            )}
          </div>
        </div>

        {/* Distance */}
        <p className="text-xs text-muted-foreground mb-2">{instructor.distance.toFixed(1)} km away</p>
        ...
        <button
          onClick={onSelect}
          className="mt-auto w-full px-4 py-2.5 text-sm font-semibold text-foreground bg-primary rounded-lg hover:bg-primary/90 transition-colors"
        >
          Select Instructor
        </button>
      </div>

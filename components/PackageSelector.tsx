...
const INPUT_CLASS = [
  ...
  'hover:border-white hover:bg-background',
...
export default function PackageSelector() {
  ...
  const hourlyRate = bookingState.provider?.hourlyRate || 0;
  ...
  const getDiscountForHours = (hours: number): number => {
    if (tiers && tiers.length > 0) {
      // Find the highest tier whose hours threshold is met
      const sorted = [...tiers].sort((a, b) => b.hours - a.hours);
      const match = sorted.find(t => hours >= t.hours);
  ...
  const predefinedPackages: Array<{ type: PackageType; hours: number; discount: number; label: string; featured: boolean }> = 
    tiers && tiers.length > 0
      ? tiers.map(t => ({
          type: (t.hours === 6 ? 'PACKAGE_6' : t.hours === 10 ? 'PACKAGE_10' : t.hours === 15 ? 'PACKAGE_15' : 'CUSTOM') as PackageType,
          hours:    t.hours,
          discount: t.discount,
          label:    t.label,
          featured: t.featured,
      ...
      : [
          { type: 'PACKAGE_6'  as PackageType, hours: 6,  discount: s.package6Discount,  label: 'Starter',    featured: false },
          { type: 'PACKAGE_10' as PackageType, hours: 10, discount: s.package10Discount, label: 'Popular',    featured: true  },
          { type: 'PACKAGE_15' as PackageType, hours: 15, discount: s.package15Discount, label: 'Best Value', featured: false },
        ];

  return (
    <>
      <div className="space-y-6">
        <h3 className="text-xl font-bold text-foreground tracking-wide">Select Your Package</h3>

        {/* Standard hour packages */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-3 pb-6">
          {predefinedPackages.map(({ type, hours, discount, label, featured }) => {
            ...
            return (
              <button
                ...
                className={[
                  ...
                  isSelected
                    ? 'border-white bg-primary text-foreground shadow-[0_5px_0_0_#0369a1,0_15px_30px_0_rgba(56,189,248,0.5)] translate-y-[4px]'
                    : 'border-slate-400 bg-secondary text-foreground shadow-[0_9px_0_0_#475569,0_20px_30px_0_rgba(0,0,0,0.6)] hover:border-slate-200 hover:translate-y-[-2px] hover:shadow-[0_11px_0_0_#475569,0_25px_35px_0_rgba(0,0,0,0.7)] active:translate-y-[4px] active:shadow-[0_5px_0_0_#475569,0_15px_20px_0_rgba(0,0,0,0.5)]',
                ].join(' ')}
              >
                ...
                {discount > 0 && (
                  <div className={`absolute top-2 right-2 text-[10px] font-black px-2 py-0.5 rounded border-2 shadow-sm uppercase tracking-wider z-10 ${
                    isSelected 
                      ...
                      : type === 'PACKAGE_15'
                      ? 'bg-emerald-500 text-foreground border-white'
                      : 'bg-indigo-500 text-foreground border-white'
                  }`}>
                    -{discount}%
                  </div>
                )}
                ...
                {featured && (
                  <span className={`absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-[11px] font-extrabold border-2 whitespace-nowrap shadow-md uppercase tracking-wider z-10 ${
                    isSelected 
                      ? 'bg-amber-400 text-slate-950 border-white' 
                      : 'bg-sky-500 text-foreground border-sky-400'
                  }`}>
                    ★ Most Popular ★
                  </span>
                )}
                ...
                <div className="w-full flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2.5">
                    <div className={[
                      ...
                      isSelected ? 'border-white bg-sky-800' : 'border-slate-400 bg-card',
                    ].join(' ')}>
                      {isSelected && <div className="w-2 h-2 rounded-full bg-white" />}
                    </div>
                    <div>
                      <p className="font-extrabold text-foreground text-lg tracking-tight">{hours} hrs</p>
                      <p className={`text-xs font-semibold ${isSelected ? 'text-sky-100' : 'text-muted-foreground'}`}>{label}</p>
                    </div>
                  </div>
                  ...
                  <div className="text-right shrink-0">
                    {discount > 0 && (
                      <p className={`text-xs line-through font-medium ${isSelected ? 'text-sky-100/90' : 'text-muted-foreground/60'}`}>
                        ${basePrice.toFixed(0)}
                      </p>
                    )}
                ...
                <div className="w-full mt-4 pt-3 border-t border-white/20 flex justify-between text-sm font-bold">
                  <span className={isSelected ? 'text-sky-100' : 'text-foreground'}>
        ...
        <div className="max-w-sm pt-2">
          <label htmlFor="customHours" className="block text-xs font-black text-muted-foreground mb-1.5 uppercase tracking-wider">
          ...
          <div className="relative shadow-[0_4px_0_0_#334155] rounded-xl">
            ...
            >
              <option value="" className="bg-card font-bold">Choose hours...</option>
              {Array.from({ length: 50 }, (_, i) => i + 1).map((i: any) => {
                const discount = getDiscountForHours(i);
                const price = hourlyRate * i * (1 - discount / 100);
                return (
                  <option key={i} value={i} className="bg-card text-foreground font-bold">
        ...
        {/* Package Benefits */}
        <div className="bg-card rounded-xl p-4 border-2 border-border shadow-[0_4px_0_0_#1e293b]">
          <h4 className="font-extrabold text-primary mb-3 text-sm uppercase tracking-wider">Package Benefits</h4>
          <ul className="space-y-2.5 text-sm font-bold text-foreground">
            ...
            <li className="flex items-center gap-2">
              ...
              Bigger packages unlock better rates (up to {tiers && tiers.length > 0
                ? Math.max(...tiers.map(t => t.discount))
                : s.package15Discount}% off)
            </li>
            <li className="flex items-center gap-2">

...
export default function SlotPicker({
  ...
  const fetchSlots = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/availability/slots?providerId=${providerId}&date=${date}&duration=${duration}`
      );
  ...
  if (loading) {
    return (
      <div className={`flex items-center gap-2 px-4 py-3 rounded-xl border ${
        variant === 'dark'
          ? 'text-primary bg-sky-950/40 border-sky-900 font-bold shadow-[0_4px_0_0_#0c4a6e]'
  ...
  if (error) {
    return (
      <div className={`px-4 py-3 rounded-xl border ${
        ...
        <p className={`text-sm font-bold mb-2 ${variant === 'dark' ? 'text-destructive' : 'text-red-600'}`}>
          ⚠️ {error}
        </p>
        <button
          type="button"
          ...
          className={`flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider transition-colors ${
            variant === 'dark' ? 'text-primary hover:text-foreground' : 'text-sky-600 hover:text-sky-800'
          }`}
        >
  ...
  if (available.length === 0) {
    return (
      <p className={`text-sm px-4 py-3 rounded-xl border ${
        variant === 'dark'
          ? 'text-muted-foreground bg-card/60 border-border'
          : 'text-muted-foreground/60 bg-gray-50 border-gray-200'
      }`}>
          ...
          return (
            <div
              ...
              className={`flex items-center justify-center rounded-xl px-2 py-2.5 text-xs font-semibold select-none ${
                variant === 'dark'
                  ? 'bg-secondary/40 text-slate-600 line-through'
        ...
        return (
          <button
            ...
            className={[
              ...
              variant === 'dark'
                ? isSelected
                  ? 'bg-primary text-foreground shadow-[0_4px_0_0_#0369a1] translate-y-px border-2 border-sky-400'
                  : 'bg-secondary text-foreground border-2 border-border hover:border-sky-400 hover:text-foreground active:translate-y-px active:shadow-none shadow-[0_3px_0_0_#334155]'
                : isSelected
                  ? 'bg-primary text-foreground shadow-sm ring-2 ring-sky-400'

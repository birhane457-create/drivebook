...
function FieldError({ message }: { message: InlineError }) {
  ...
  return (
    <p role="alert" className="mt-1.5 text-sm font-semibold text-destructive bg-red-950/40 border border-red-900 rounded-lg px-3 py-2">
      ⚠️ {message}
    </p>
  );
}
...
function ConfirmDialog({ message, onConfirm, onCancel }: ConfirmDialogProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm px-4">
      <div className="bg-card border-2 border-border rounded-2xl shadow-2xl p-6 max-w-sm w-full">
        <p className="text-foreground font-semibold mb-5 text-center">{message}</p>
        <div className="flex gap-3">
          <button
            onClick={onCancel}
            className="flex-1 px-4 py-2.5 rounded-xl border-2 border-border text-foreground hover:text-foreground hover:border-slate-500 font-bold text-sm transition-colors"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            className="flex-1 px-4 py-2.5 rounded-xl bg-destructive hover:bg-red-500 text-foreground font-bold text-sm transition-colors border-2 border-transparent"
          >
            Remove
          </button>
        </div>
...
export default function BookingDetailsForm() {
  ...
  const bookedHours = scheduledBookings.reduce((sum: any, booking: any) => sum + (booking.duration / 60), 0);
  ...
  const providerIdRef = React.useRef(instructor?.id);
  ...
  React.useEffect(() => { providerIdRef.current = instructor?.id; }, [instructor?.id]);
  ...
  useEffect(() => {
    return () => {
      ...
      const currentInstructorId = providerIdRef.current;
      ...
      currentBookings.forEach((booking: any) => {
        ...
        const params = new URLSearchParams({
          providerId: currentInstructorId,
          date:         booking.date,
          time:         booking.time,
  ...
  const fetchAvailableSlots = async () => {
    ...
    try {
      const response = await fetch(
        `/api/availability/slots?providerId=${instructor.id}&date=${selectedDate}&duration=${selectedDuration}`
      );
      ...
      if (response.ok) {
        ...
        const filteredSlots = ((data as any).slots || []).map((slot: AvailableSlot) => {
  ...
  const fetchPdaConfigs = async () => {
    ...
    try {
      const response = await fetch(`/api/instructor/pda-configs?providerId=${instructor.id}`);
      if (response.ok) {
        const data = await response.json();
        setPdaConfigs((data as any).configs || []);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="bg-card rounded-xl border-2 border-border p-4 sm:p-6 shadow-[0_6px_0_0_#1e293b,0_20px_30px_0_rgba(0,0,0,0.5)]">
        <div className="flex justify-between items-center mb-3">
          <span className="text-xs font-black text-foreground uppercase tracking-wider">Hours Scheduled</span>
          <span className="text-base font-black text-primary bg-background px-2.5 py-0.5 rounded-md border border-border">
            {bookedHours.toFixed(1)} / {hours}
          </span>
        </div>
        
        {/* Progress Track - Opaque and Flat */}
        <div className="w-full bg-background rounded-full h-5 border-2 border-border overflow-hidden shadow-inner">
          <div
        ...
        <div className="flex justify-between items-center mt-3 font-bold text-sm">
          ...
          <p className="text-xs text-muted-foreground font-mono tracking-tight bg-background px-1.5 py-0.5 rounded">
      ...
      {scheduledBookings.length > 0 && (
        <div className="bg-card rounded-xl border-2 border-border p-4 sm:p-6 shadow-[0_6px_0_0_#1e293b,0_20px_30px_0_rgba(0,0,0,0.5)]">
          <h3 className="text-lg font-black text-foreground uppercase tracking-wider mb-4 flex items-center gap-2">
            ...
            <span className="bg-primary text-foreground text-xs font-black px-2.5 py-0.5 rounded-full border border-sky-400">
          ...
          <div className="space-y-4">
            {scheduledBookings.map((booking: any, index: number) => (
              <div
                key={index}
                className="flex items-start justify-between p-4 bg-background border-2 border-emerald-500 rounded-xl shadow-[0_4px_0_0_#064e3b]"
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="space-y-1.5 font-bold text-sm">
                    <p className="text-foreground flex items-center gap-1.5">
                      <span className="text-muted-foreground/60 text-xs uppercase tracking-wider font-black">Duration:</span> 
                      {formatDuration(booking.duration)}
                    </p>
                    <p className="text-foreground flex items-center gap-1.5 truncate">
                      <span className="text-muted-foreground/60 text-xs uppercase tracking-wider font-black">Pickup:</span> 
                ...
                <button
                  ...
                  className="text-destructive hover:text-destructive p-2 ml-2 transition-all duration-100 bg-card hover:bg-red-950/40 border border-border hover:border-red-900 rounded-lg shadow-sm active:scale-95 shrink-0"
      ...
      {(remainingHours > 0 || showPdaPackage) && (
        <div className="bg-card rounded-xl border-2 border-border p-4 sm:p-6 shadow-[0_6px_0_0_#1e293b,0_20px_30px_0_rgba(0,0,0,0.5)]">
          ...
          {showPdaPackage && remainingHours > 0 && (
            <div className="mb-6 grid grid-cols-2 gap-3 p-1.5 bg-background rounded-xl border-2 border-border">
              <button
                ...
                className={`px-4 py-2.5 rounded-lg font-black text-sm uppercase tracking-wider text-center transition-all duration-100 border-2 ${
                  activeTab === 'lessons'
                    ? 'border-white bg-primary text-foreground shadow-[0_3px_0_0_#0369a1]'
          ...
          {showPdaPackage && !remainingHours && (
            <div className="mb-6 text-sm font-bold text-amber-300 bg-background border-2 border-amber-900 rounded-xl p-4 shadow-[0_4px_0_0_#451a03]">
          ...
          <div className="space-y-4">
            ...
            {activeTab === 'lessons' && remainingHours > 0 && (
              <>
                {/* Date Selection */}
                <div>
                  <label htmlFor="date" className="block text-xs font-black text-foreground mb-1.5 uppercase tracking-wider">
                    Date *
                  </label>
                  <div className="relative shadow-[0_4px_0_0_#1e293b] rounded-xl bg-background">
                    <input
                      type="date"
                      id="date"
                      value={selectedDate}
                ...
                {/* Duration Selection */}
                <div>
                  <label htmlFor="duration" className="block text-xs font-black text-foreground mb-1.5 uppercase tracking-wider">
                    Duration *
                  </label>
                  <div className="relative shadow-[0_4px_0_0_#1e293b] rounded-xl bg-background">
                    <select
                      ...
                      aria-required="true"
                      className="w-full px-4 py-3 bg-background border-2 border-slate-400 rounded-xl text-foreground font-bold transition-all duration-100 hover:border-white focus:outline-none focus:border-sky-400 focus:shadow-[0_4px_0_0_#0284c7,0_10px_20px_0_rgba(56,189,248,0.3)]"
                    >
                      {allowedDurations.map((duration: number) => (
                        <option key={duration} value={duration} className="bg-background font-bold text-foreground">
                ...
                {/* Pickup Location */}
                <div>
                  <label htmlFor="pickupLocation" className="block text-xs font-black text-foreground mb-1.5 uppercase tracking-wider">
                    Pickup Location *
                  </label>
                  <PickupLocationInput
                ...
                {/* Notes */}
                <div>
                  <label htmlFor="notes" className="block text-xs font-black text-foreground mb-1.5 uppercase tracking-wider">
                    Notes (Optional)
                  </label>
                  <div className="relative shadow-[0_4px_0_0_#1e293b] rounded-xl bg-background">
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-4 py-3 bg-background border-2 border-slate-400 rounded-xl text-foreground font-bold placeholder-slate-500 transition-all duration-100 hover:border-white focus:outline-none focus:border-sky-400 focus:shadow-[0_4px_0_0_#0284c7,0_10px_20px_0_rgba(56,189,248,0.3)] resize-none"
                    />
                  </div>
                </div>
              </>
            ...
            {activeTab === 'pda-test' && showPdaPackage && (
              <>
                ...
                <div>
                  <label htmlFor="pda-config" className="block text-xs font-black text-foreground mb-1.5 uppercase tracking-wider">
                    PDA Test Package *
                  </label>
                  {isLoadingPda ? (
                    <p className="text-sm font-bold text-primary bg-primary/10 border border-sky-900 rounded-xl px-4 py-3 animate-pulse shadow-[0_4px_0_0_#0c4a6e]">
                      🔄 Loading PDA options...
                    </p>
                  ) : pdaConfigs.length === 0 ? (
                    <p className="text-sm font-bold text-destructive bg-red-950/40 border border-red-900 rounded-xl px-4 py-3 shadow-[0_4px_0_0_#7f1d1d]">
                    ...
                    <div className="relative shadow-[0_4px_0_0_#1e293b] rounded-xl bg-background">
                      <select
                        ...
                        className="w-full px-4 py-3 bg-background border-2 border-slate-400 rounded-xl text-foreground font-bold transition-all duration-100 hover:border-white focus:outline-none focus:border-sky-400 focus:shadow-[0_4px_0_0_#0284c7,0_10px_20px_0_rgba(56,189,248,0.3)]"
                      >
                        <option value="" className="bg-background font-bold text-muted-foreground">Select a test package</option>
                        {pdaConfigs.map((config: any) => (
                          <option key={config.id} value={config.id} className="bg-background font-bold text-foreground">
                ...
                {selectedPdaConfig && getSelectedPdaConfig() && (
                  <div className="bg-background rounded-xl p-4 border-2 border-blue-500 shadow-[0_4px_0_0_#2563eb] mb-6">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                      <div className="flex-1 min-w-0">
                        <p className="text-[10px] text-primary font-black uppercase tracking-wider mb-1">Selected Package:</p>
                        <p className="font-extrabold text-foreground text-base mb-1.5 tracking-tight">{getSelectedPdaConfig().name}</p>
                        <div className="flex items-center gap-3 text-sm font-bold">
                          ...
                          <span className="text-foreground">{formatDurationMinutes(getSelectedPdaConfig().durationMinutes)}</span>
                        </div>
                      ...
                      <button
                        ...
                        className="w-full sm:w-auto px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-wider border-2 border-border bg-card text-foreground hover:text-foreground hover:border-slate-400 shadow-[0_3px_0_0_#334155] transition-all duration-100 active:translate-y-[3px] active:shadow-none whitespace-nowrap text-center shrink-0"
                      >
                        Change Package
                      </button>
                    </div>
                ...
                {selectedPdaConfig && pdaTestCentres.length > 1 && (
                  <div>
                    <label htmlFor="test-centre" className="block text-xs font-black text-foreground mb-1.5 uppercase tracking-wider">
                      Test Centre *
                    </label>
                    {pdaTestCentres.length === 0 ? (
                      ...
                      <div className="relative shadow-[0_4px_0_0_#1e293b] rounded-xl bg-background">
                        <select
                          ...
                          className="w-full px-4 py-3 bg-background border-2 border-slate-400 rounded-xl text-foreground font-bold transition-all duration-100 hover:border-white focus:outline-none focus:border-sky-400 focus:shadow-[0_4px_0_0_#0284c7,0_10px_20px_0_rgba(56,189,248,0.3)]"
                        >
                          <option value="" className="bg-background font-bold text-muted-foreground">Select a test centre</option>
                          {pdaTestCentres.map((centre: any) => (
                            <option key={centre.id} value={centre.id} className="bg-background font-bold text-foreground">
                ...
                {selectedPdaConfig && selectedTestCentre && (
                  <div>
                    <label htmlFor="test-date" className="block text-xs font-black text-foreground mb-1.5 uppercase tracking-wider">
                      Select Date *
                    </label>
                    <div className="relative shadow-[0_4px_0_0_#1e293b] rounded-xl bg-background">
                      <input
                ...
                {selectedPdaConfig && (
                  <div>
                    ...
                    <div className="relative shadow-[0_4px_0_0_#1e293b] rounded-xl bg-background">
                      <div className="w-full px-4 py-3 bg-background border-2 border-border rounded-xl text-foreground font-extrabold text-sm tracking-wide">
                        {formatDurationMinutes(getSelectedPdaConfig()?.durationMinutes || 0)}
                      </div>
                    </div>
                    <p className="text-xs font-semibold text-muted-foreground mt-2">Fixed duration based on your selected package</p>
                  </div>
                )}
                ...
                {selectedPdaConfig && selectedTestDate && (
                  <div>
                    <label htmlFor="test-time" className="block text-xs font-black text-foreground mb-1.5 uppercase tracking-wider">
                      Select Time *
                    </label>
                    <div className="relative shadow-[0_4px_0_0_#1e293b] rounded-xl bg-background">
                      <select
                        ...
                        className="w-full px-4 py-3 bg-background border-2 border-slate-400 rounded-xl text-foreground font-bold transition-all duration-100 hover:border-white focus:outline-none focus:border-sky-400 focus:shadow-[0_4px_0_0_#0284c7,0_10px_20px_0_rgba(56,189,248,0.3)]"
                      >
                        <option value="" className="bg-background font-bold text-muted-foreground">Select a time</option>
                        {Array.from({ length: 17 * 2 - 18 }, (_, i) => {
                          ...
                          <option key={time} value={time} className="bg-background font-bold text-foreground">
                            {time}
                          </option>
                        ))}
                      </select>
                  ...
                  <div>
                    ...
                    {getConfigIncludes().pickup ? (
                      <>
                        ...
                        <div className="space-y-3">
                          {/* Radio Item 1 */}
                          <div className={`p-3.5 rounded-xl border-2 transition-all duration-100 flex items-center gap-3 bg-background ${
                            pickupOption === 'pickup' 
                              ? 'border-white shadow-[0_3px_0_0_#0284c7]' 
                              : 'border-border shadow-[0_3px_0_0_#1e293b]'
                          }`}>
                            <input
                              type="radio"
                              id="pickup-yes"
                              ...
                              className="w-4 h-4 text-sky-500 bg-card border-border focus:ring-0 focus:ring-offset-0"
                            />
                            <label htmlFor="pickup-yes" className="text-sm font-bold text-foreground cursor-pointer select-none">
                          ...
                          {pickupOption === 'pickup' && (
                            <div className="relative shadow-[0_4px_0_0_#1e293b] rounded-xl bg-background">
                              <input
                                ...
                                className="w-full px-4 py-3 bg-background border-2 border-slate-400 rounded-xl text-foreground font-bold placeholder-slate-500 transition-all duration-100 hover:border-white focus:outline-none focus:border-sky-400 focus:shadow-[0_4px_0_0_#0284c7,0_10px_20px_0_rgba(56,189,248,0.3)]"
                              />
                            </div>
                          )}
                          ...
                          <div className={`p-3.5 rounded-xl border-2 transition-all duration-100 flex items-center gap-3 bg-background ${
                            ...
                            <input
                              ...
                              className="w-4 h-4 text-sky-500 bg-card border-border focus:ring-0 focus:ring-offset-0"
                            />
                            <label htmlFor="pickup-no" className="text-sm font-bold text-foreground cursor-pointer select-none">
                      ...
                      <>
                        ...
                        <div className="px-4 py-4 bg-background border-2 border-border rounded-xl shadow-[0_4px_0_0_#1e293b]">
                          <p className="text-sm font-black text-foreground uppercase tracking-wide mb-2 flex items-center gap-1.5">
                            <span className="w-2 h-2 rounded-full bg-slate-400 block" /> Meet at test centre
                          </p>
                          {pdaTestCentres.find(c => c.id === selectedTestCentre) && (
                            <p className="text-sm font-extrabold text-primary bg-card border border-border rounded px-3 py-1.5 flex items-center gap-2">
                              📍 {pdaTestCentres.find(c => c.id === selectedTestCentre)?.name}
                            </p>
                          )}
                          <p className="text-xs font-semibold text-muted-foreground mt-2.5">This package does not include pickup service</p>
                        </div>
                      </>
                    )}
                  </div>
                ...
                {selectedPdaConfig && selectedTestTime && (
                  <div>
                    <label htmlFor="pda-notes" className="block text-xs font-black text-foreground mb-1.5 uppercase tracking-wider">
                      Notes (Optional)
                    </label>
                    <div className="relative shadow-[0_4px_0_0_#1e293b] rounded-xl bg-background">
                      <textarea
                ...
                <button
                  ...
                  className={`w-full px-6 py-3.5 rounded-xl font-black text-sm uppercase tracking-wider text-center transition-all duration-100 border-2 select-none mb-32 lg:mb-0 ${
                    !selectedPdaConfig || !selectedTestCentre || !selectedTestDate || !selectedTestTime
                      ? 'border-border bg-background text-slate-600 cursor-not-allowed shadow-[0_4px_0_0_#0f172a]'
                      : 'border-white bg-primary text-foreground shadow-[0_5px_0_0_#1d4ed8,0_15px_25px_0_rgba(37,99,235,0.4)] hover:bg-blue-500 hover:translate-y-[-2px] hover:shadow-[0_7px_0_0_#1d4ed8,0_20px_30px_0_rgba(37,99,235,0.5)] active:translate-y-[5px] active:shadow-none'
                  }`}
                >
                  + Add This PDA Test
                </button>
            ...
            {activeTab === 'lessons' && showPdaPackage && (
              <div className="bg-background border-2 border-blue-500 rounded-xl p-4 shadow-[0_4px_0_0_#2563eb] mb-2">
                <div className="flex gap-3">
                  <svg className="w-5 h-5 text-primary flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
                  </svg>
                  <div className="text-sm font-bold text-foreground">
                    <p className="text-base font-black text-foreground uppercase tracking-wide mb-1">PDA Test Included</p>
                    <p className="text-foreground">Use the "Schedule PDA Test" tab to schedule your test on a specific date and centre</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'lessons' && remainingHours > 0 && (
              <button
                ...
                className={`w-full px-6 py-3.5 rounded-xl border-2 font-black text-sm uppercase tracking-wider text-center transition-all duration-100 mb-32 lg:mb-0 select-none ${
                  isSubmitting
                    ? 'border-border bg-background text-muted-foreground/60 cursor-not-allowed shadow-[0_4px_0_0_#0f172a]'
                    : 'bg-emerald-600 text-foreground border-white shadow-[0_5px_0_0_#047857,0_15px_25px_0_rgba(16,185,129,0.4)] hover:bg-emerald-500 hover:translate-y-[-2px] hover:shadow-[0_7px_0_0_#047857,0_20px_30px_0_rgba(16,185,129,0.5)] active:translate-y-[5px] active:shadow-none'
                }`}
              >
            ...
            {activeTab === 'lessons' && remainingHours === 0 && (
              <div className="bg-background rounded-xl p-4 border-2 border-emerald-500 text-center shadow-[0_4px_0_0_#064e3b]">
                <p className="text-emerald-400 font-black text-base uppercase tracking-wide">✓ All lesson hours scheduled</p>
                <p className="text-foreground font-bold text-sm mt-1.5">
      ...
      <div className="bg-card rounded-xl p-4 sm:p-5 border-2 border-border shadow-[0_6px_0_0_#1e293b,0_20px_30px_0_rgba(0,0,0,0.5)]">
        <div className="flex gap-3">
          <svg className="w-5 h-5 text-primary flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
          </svg>
          <div className="text-sm font-bold text-foreground w-full">
            <p className="text-base font-black text-foreground uppercase tracking-wide mb-2">Scheduling Tips</p>
            <ul className="space-y-2 list-none pl-0">
              <li className="flex items-start gap-2 bg-background/60 p-2 rounded-lg border border-border">
              ...
              <li className="flex items-start gap-2 bg-background/40 p-2 rounded-lg border border-amber-900/60 text-amber-300 font-extrabold">
...
function PickupLocationInput({ value, onChange, providerId }: PickupLocationInputProps) {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await fetch(
          `/api/public/check-service-area?providerId=${encodeURIComponent(providerId)}&address=${encodeURIComponent(value.trim())}`
        );
  ...
  return (
    <div className="space-y-3">
      ...
      <div ref={containerRef} className="relative">
        <p className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-wider mb-1.5">
        ...
        <div className="relative shadow-[0_4px_0_0_#1e293b] rounded-xl bg-background">
          <input
            ...
            className={`w-full pl-4 pr-10 py-3 bg-background border-2 rounded-xl text-foreground font-bold placeholder-slate-500 transition-all duration-100 hover:border-white focus:outline-none focus:border-sky-400 focus:shadow-[0_4px_0_0_#0284c7,0_10px_20px_0_rgba(56,189,248,0.3)] ${
          ...
          <div className="absolute right-3 top-1/2 -translate-y-1/2 text-sm">
            {suburbPicked
              ? <button type="button" onClick={handleClearSuburb} className="text-muted-foreground/60 hover:text-foreground transition-colors" title="Clear suburb">✕</button>
        ...
        {dropdownOpen && suggestions.length > 0 && (
          <div className="absolute z-50 mt-1 w-full bg-card border-2 border-border rounded-xl shadow-[0_8px_0_0_#1e293b,0_20px_30px_0_rgba(0,0,0,0.7)] overflow-hidden">
            {suggestions.map((entry, i) => (
              <button
                key={`${entry.postcode}-${entry.suburb}-${i}`}
                type="button"
                onMouseDown={(e) => { e.preventDefault(); handleSuburbSelect(entry); }}
                className="w-full text-left px-4 py-2.5 hover:bg-primary transition-colors flex items-center justify-between gap-3 border-b border-border last:border-0"
              >
                <span className="font-bold text-foreground text-sm">{entry.suburb}</span>
        ...
        {dropdownOpen && suburbQuery.length >= 2 && suggestions.length === 0 && (
          <div className="absolute z-50 mt-1 w-full bg-card border-2 border-border rounded-xl px-4 py-3 text-sm text-muted-foreground font-medium">
      ...
      {suburbPicked && (
        <div>
          ...
          {isComplete && (
            <p className="mt-2 text-xs font-bold text-primary bg-primary/10 border border-sky-900 rounded-lg px-3 py-1.5 flex items-center gap-1.5">
      ...
      {checkResult === 'unknown' && value.trim().length >= 5 && (
        <p className="text-xs font-semibold text-muted-foreground/60 px-1">

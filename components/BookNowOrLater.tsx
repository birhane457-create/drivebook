  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-6">
        ...
        <p className="text-foreground text-sm">Choose now for immediate slots or later to schedule from your dashboard.</p>
      </div>
        ...
        <button
          ...
          className={`flex-1 p-4 sm:p-5 rounded-xl border-2 text-left transition-all duration-100 ${
            bookingState.bookingType === 'now' 
              ? 'border-white bg-primary text-foreground shadow-[0_5px_0_0_#1d4ed8,0_15px_30px_0_rgba(37,99,235,0.5)] translate-y-[4px]' 
              : 'border-slate-400 bg-card hover:border-slate-200 text-foreground shadow-[0_9px_0_0_#475569,0_20px_30px_0_rgba(0,0,0,0.6)] hover:translate-y-[-2px] hover:shadow-[0_11px_0_0_#475569,0_25px_35px_0_rgba(0,0,0,0.7)] active:translate-y-[4px] active:shadow-[0_5px_0_0_#475569,0_15px_20px_0_rgba(0,0,0,0.5)]'
          }`}
        >
          <div className="flex items-center justify-between mb-1.5">
            <h3 className="font-extrabold text-foreground text-base tracking-wide">Book Now</h3>
            <div className="relative z-20">
              <button
                ...
                className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black transition ${
                  bookingState.bookingType === 'now'
                    ? 'bg-blue-800 text-foreground hover:bg-blue-900'
                    : 'bg-secondary/70 text-foreground hover:bg-slate-600'
                }`}
              >
                ?
              </button>
              {tooltip === 'now' && (
                <div className="absolute right-0 top-7 w-40 p-2 text-sm bg-background border-2 border-slate-500 rounded-md shadow-2xl text-foreground z-50">
          ...
          <div className="mt-3 pt-2 border-t border-border flex items-center justify-between">
            <span className={`font-bold text-xs uppercase tracking-wider transition ${
              bookingState.bookingType === 'now' ? 'text-blue-100' : 'text-primary'
            }`}>
        ...
        <button
          ...
          className={`flex-1 p-4 sm:p-5 rounded-xl border-2 text-left transition-all duration-100 ${
            bookingState.bookingType === 'later' 
              ? 'border-white bg-cyan-600 text-foreground shadow-[0_5px_0_0_#0891b2,0_15px_30px_0_rgba(6,182,212,0.5)] translate-y-[4px]' 
              : 'border-slate-400 bg-card hover:border-slate-200 text-foreground shadow-[0_9px_0_0_#475569,0_20px_30px_0_rgba(0,0,0,0.6)] hover:translate-y-[-2px] hover:shadow-[0_11px_0_0_#475569,0_25px_35px_0_rgba(0,0,0,0.7)] active:translate-y-[4px] active:shadow-[0_5px_0_0_#475569,0_15px_20px_0_rgba(0,0,0,0.5)]'
          }`}
        >
          <div className="flex items-center justify-between mb-1.5">
            <h3 className="font-extrabold text-foreground text-base tracking-wide">Book Later</h3>
            <div className="relative z-20">
              <button
                ...
                className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black transition ${
                  bookingState.bookingType === 'later'
                    ? 'bg-cyan-800 text-foreground hover:bg-cyan-900'
                    : 'bg-secondary/70 text-foreground hover:bg-slate-600'
                }`}
              >
                ?

...
export default function PDABookingForm({
  ...
  useEffect(() => {
    if (!testCentreId || !testDate || !providerId) return
    ...
    const fetchSlots = async () => {
      ...
      try {
        ...
        const availableTimeSlots = ((data as any).slots || [])
    ...
    fetchSlots()
  }, [testCentreId, testDate, config.id, providerId, config.durationMinutes])
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <select
          ...
          className="w-full px-4 py-2 bg-background border border-border rounded-lg text-foreground focus:border-blue-500 focus:ring-2 focus:ring-blue-500"
        >
        ...
        <div>
          ...
          {slotsLoading ? (
            ...
            <div className="bg-background border border-border rounded-lg p-4 text-center">
              ...
              <p className="text-foreground text-sm">No available times for this date</p>
              <p className="text-muted-foreground text-xs mt-1">Try selecting a different date</p>
            </div>
          ) : (
      ...
      <button
        ...
        className="w-full px-4 py-3 bg-primary text-foreground rounded-lg font-semibold hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
      >

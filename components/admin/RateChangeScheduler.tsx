...
const FIELD_OPTIONS = [
  ...
  { value: 'proCommissionRate',      label: 'Pro — commission rate' },
  { value: 'studioCommissionRate',   label: 'Studio — commission rate' },
  { value: 'businessCommissionRate', label: 'Premium — commission rate' }, // DB column is 'businessCommissionRate' (legacy name) — maps to PREMIUM subscriptionTier
];

const STATUS_STYLE: Record<string, string> = {
  PENDING:   'bg-amber-100 text-amber-700',
  APPLIED:   'bg-green-900/40 text-emerald-400',
    ...
    <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
      <div className="px-6 py-4 border-b border-border flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-foreground">Scheduled Rate Changes</h2>
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 bg-primary text-foreground text-sm rounded-lg hover:bg-primary/90"
        >
      ...
      {/* How it works */}
      <div className="px-6 py-3 bg-blue-900/20 border-b border-blue-100 text-xs text-primary">
      ...
      {showForm && (
        <div className="px-6 py-5 border-b border-border bg-background">
          <h3 className="font-semibold text-foreground mb-4">Schedule a new rate change</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Which rate</label>
                <select value={form.field} onChange={e => setForm(f => ({ ...f, field: e.target.value }))}
                  className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-card">
                  {FIELD_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">New rate (%)</label>
              ...
              <div>
                ...
                <input type="date" required
                  ...
                  className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
                <p className="text-xs text-muted-foreground/60 mt-1">Must be at least 1 day in the future</p>
              </div>
            </div>
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">
                Reason <span className="text-muted-foreground/60">(shown to instructors in the notification)</span>
              </label>
              <textarea required rows={3} minLength={10} maxLength={500}
                ...
                className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none" />
              <p className="text-xs text-muted-foreground/60 mt-1">{form.reason.length}/500 — min 10 characters</p>
            </div>
            ...
            <div className="flex gap-3">
              <button type="button" onClick={() => setShowForm(false)}
                className="flex-1 py-2 border border-border text-muted-foreground text-sm rounded-lg hover:bg-background">
                Cancel
              </button>
            ...
            <div key={change.id} className="px-6 py-4 flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 text-sm">
                  <span className="text-muted-foreground">{change.currentRate}%</span>
                ...
                <p className="text-xs text-muted-foreground mt-1 italic truncate">{change.reason}</p>
                {change.appliedAt && (
              ...
              {change.status === 'PENDING' && (
                <button onClick={() => cancelChange(change.id)}
                  className="p-1.5 text-muted-foreground/60 hover:text-red-500 hover:bg-red-900/20 rounded-lg transition-colors shrink-0">

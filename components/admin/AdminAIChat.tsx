...
function ToolPill({ name }: { name: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs bg-secondary text-muted-foreground/60 border border-border">
  ...
  if (isError) {
    return (
      <div className="flex items-start gap-2">
        <div className="w-7 h-7 rounded-full bg-destructive/20 border border-red-500/30 flex items-center justify-center shrink-0 mt-0.5">
          ...
          <button
            ...
            className={`shrink-0 px-2.5 py-1 rounded-lg text-xs font-medium transition capitalize ${
              activeCategory === cat
                ...
                : 'text-slate-600 hover:text-muted-foreground hover:bg-secondary'
            }`}
          >
            {CATEGORY_LABELS[cat]}
          </button>
      ...
      <div className="flex flex-wrap gap-1.5">
        {questions.map((q) => (
          <button
            ...
            className="text-xs px-3 py-1.5 rounded-lg bg-secondary border border-border text-muted-foreground hover:text-foreground hover:border-border transition text-left"
          >
            {q.question}
          </button>
        ))}
...
export default function AdminAIChat() {
  ...
  return (
    <div className="bg-card rounded-2xl border border-border overflow-hidden flex flex-col" style={{ height: '620px' }}>
      {/* Header */}
      <div className="flex items-center justify-between px-5 py-4 border-b border-border shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm font-semibold text-foreground">Operations Copilot</h2>
        ...
        <button
          onClick={reset}
          className="p-1.5 rounded-lg text-muted-foreground/60 hover:text-foreground hover:bg-secondary transition"
      ...
      {configured === false && (
        <div className="px-5 py-3 bg-violet-900/20 border-b border-violet-700/30 shrink-0">
          <p className="text-xs text-violet-300">
            Add <code className="bg-secondary px-1 rounded">OPENAI_API_KEY</code> or{' '}
            <code className="bg-secondary px-1 rounded">ANTHROPIC_API_KEY</code> to your .env to enable AI chat.
          </p>
        </div>
      )}
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="bg-secondary border border-border rounded-2xl rounded-tl-sm px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">Querying platform data…</span>
              </div>
            </div>
          </div>
        )}
      ...
      {/* Input */}
      <div className="px-4 pb-4 shrink-0 border-t border-border pt-3">
        <div className="flex items-end gap-2 bg-secondary border border-border rounded-xl p-2 focus-within:border-violet-600/60 transition">
          <textarea
            ref={inputRef}
            value={input}
            ...
            className="flex-1 bg-transparent text-sm text-foreground placeholder-slate-600 resize-none outline-none py-1 px-2 max-h-32"
            style={{ minHeight: '36px' }}
          />
          <button
            onClick={() => send()}
            disabled={!input.trim() || loading || configured === false}
            className="p-2 rounded-lg bg-violet-600 hover:bg-violet-500 disabled:opacity-30 disabled:cursor-not-allowed text-foreground transition shrink-0"
          >

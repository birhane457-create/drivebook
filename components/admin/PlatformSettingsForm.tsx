...
type NotifKey =
  ...
  | 'PAYMENT_RECEIVED' | 'LESSON_REMINDER' | 'APPOINTMENT_REMINDER' | 'DOCUMENT_EXPIRING' | 'REVIEW_RECEIVED'
...
const EVENT_LABELS: Record<NotifKey, { label: string; who: string; note?: string }> = {
  BOOKING_REQUEST:     { label: 'Booking request',      who: 'provider',        note: 'When instructor creates a booking' },
  ...
  PAYMENT_RECEIVED:    { label: 'Payment received',     who: 'provider' },
  LESSON_REMINDER:     { label: 'Lesson reminder',      who: 'Client + Instructor', note: '24h before lesson' },
  APPOINTMENT_REMINDER:{ label: 'Appointment reminder', who: 'Client + Provider',   note: '24h before appointment' },
  DOCUMENT_EXPIRING:   { label: 'Document expiring',    who: 'provider',          note: '30 days before expiry' },
  REVIEW_RECEIVED:     { label: 'Review received',      who: 'provider' },
  NEW_MESSAGE:         { label: 'New message',          who: 'Both' },
  PAYOUT_PROCESSED:    { label: 'Payout processed',     who: 'provider' },
...
export default function PlatformSettingsForm() {
  ...
  if (loading || !s) {
    return <div className="bg-card rounded-xl border border-border p-8 text-center text-muted-foreground/60 text-sm">Loading settings...</div>;
  }
  ...
  return (
    <form onSubmit={handleSave} className="space-y-5">
        ...
        <div className="grid sm:grid-cols-2 gap-5">
          <div>
            ...
            <input
              ...
              className="w-full border border-border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500"
            />
            <p className="text-xs text-muted-foreground/60 mt-1">Clients can't book less than this many hours ahead</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-foreground mb-1">
            ...
            <input
              ...
              className="w-full border border-border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500"
            />
            <p className="text-xs text-muted-foreground/60 mt-1">How far ahead a single lesson can be booked</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-foreground mb-1">
          ...
          <div className="flex flex-col justify-center">
            <div className="flex items-start gap-3 p-4 bg-blue-900/20 border border-blue-700/50 rounded-xl">
              <button
                ...
                className={`relative inline-flex h-6 w-11 rounded-full transition-colors shrink-0 mt-0.5 ${s.booking.packageBypassMinAdvance ? 'bg-primary' : 'bg-gray-300'}`}
              >
                <span className={`inline-block h-5 w-5 rounded-full bg-card shadow transform transition-transform mt-0.5 ${s.booking.packageBypassMinAdvance ? 'translate-x-5' : 'translate-x-0.5'}`} />
              </button>
              <div>
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border p-6">
        ...
        <p className="text-xs text-muted-foreground/60 mb-5 flex items-center gap-1">
        ...
        <div className="grid grid-cols-[1fr_auto_auto_auto] gap-x-4 items-center mb-2 px-1">
          ...
          {(['inApp', 'email', 'sms'] as const).map(ch => (
            <div key={ch} className="flex items-center gap-1 text-xs font-medium text-muted-foreground uppercase tracking-wide w-16 justify-center">
            ...
            return (
              <div key={event} className="grid grid-cols-[1fr_auto_auto_auto] gap-x-4 items-center py-3 px-1">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground/60">{who}{note ? ` · ${note}` : ''}</p>
                </div>
                {(['inApp', 'email', 'sms'] as const).map(ch => (
                  <div key={ch} className="w-16 flex justify-center">
                    <button
                      ...
                      className={`w-9 h-5 rounded-full transition-colors relative ${channels[ch] ? 'bg-primary' : 'bg-secondary'}`}
                    >
                      <span className={`absolute top-0.5 h-4 w-4 rounded-full bg-card shadow transition-transform ${channels[ch] ? 'translate-x-4' : 'translate-x-0.5'}`} />
                    </button>
                  </div>
                ))}
              </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <p className="text-xs text-muted-foreground/60">Booking rules apply to new bookings immediately.</p>
      </div>
    </form>
  );
}

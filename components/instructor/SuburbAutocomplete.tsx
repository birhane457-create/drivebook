    ...
    <div ref={containerRef} className="relative">
      <div className="relative">
        ...
        <input
          ...
          className={`w-full bg-background/60 border border-border rounded-lg pl-9 pr-8 py-2 text-sm text-foreground placeholder-slate-500 focus:outline-none focus:border-sky-500/50 transition-colors ${className ?? ''}`}
          autoComplete="off"
          autoCorrect="off"
          spellCheck={false}
        />
        ...
        <div className="absolute z-50 w-full mt-1 bg-card border border-border rounded-xl shadow-2xl overflow-hidden">
          {results.map((entry, i) => (
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-2.5 text-left hover:bg-secondary transition-colors text-sm"
            >
              <MapPin className="w-3.5 h-3.5 text-muted-foreground/60 shrink-0" />
              <span className="flex-1 text-foreground">{entry.suburb}</span>
              <span className="text-muted-foreground/60 text-xs">{entry.state} {entry.postcode}</span>
            </button>
          ))}
        </div>
      )}
      ...
      {open && query.length >= 2 && results.length === 0 && (
        <div className="absolute z-50 w-full mt-1 bg-card border border-border rounded-xl shadow-2xl px-4 py-3 text-sm text-muted-foreground/60">

...
const CAPABILITY_MESSAGES: Record<keyof Capabilities, CapabilityMessage> = {
  ...
  canUseAiReceptionist: {
    ...
    reason:  'AI Receptionist requires a Pro plan or higher',
    cta:     'Upgrade to Pro →',
    ctaHref: '/dashboard/subscription',
  },
  canUseCustomSlug: {
    label:   'Custom booking URL',
    reason:  'Custom booking URL requires a Pro plan or higher',
    cta:     'Upgrade to Pro →',
    ctaHref: '/dashboard/subscription',
  },
  canUseCustomDomain: {
    label:   'Custom domain',
    reason:  'Custom domain requires a Studio or Premium plan',
    cta:     'Upgrade to Studio →',
    ctaHref: '/dashboard/subscription',
  },
  canUseWhiteLabel: {
    label:   'White-label branding',
    reason:  'Full white-label requires a Premium plan',
    cta:     'Upgrade to Premium →',
    ctaHref: '/dashboard/subscription',
  },
  canUseDirectPayments: {
    label:   'Direct payments',
    reason:  'Direct payments (0% commission) is coming soon for Premium providers',
    cta:     'Learn more →',
    ctaHref: '/dashboard/subscription',
  },
};
    ...
    >
      ...
      {tooltipOpen && (
        <div
          ...
          onMouseLeave={() => setTooltipOpen(false)}
        >
          <div className="bg-card border border-border rounded-lg shadow-xl px-3 py-2.5
                          text-xs text-foreground whitespace-nowrap max-w-[240px] mb-1">
            ...
            <Link
              href={msg.ctaHref}
              className="text-primary hover:text-primary underline underline-offset-2 transition-colors block"
            >
              {msg.cta}
            </Link>
          </div>
          {/* Arrow */}
          <div className="w-2 h-2 bg-card border-r border-b border-border rotate-45 -mt-1 shrink-0" />
        </div>
      )}
    </div>

...
async function fetchSlotsForDate(
  ...
  const res = await fetch(
    `/api/availability/slots?providerId=${providerId}&date=${date}&duration=${duration}&bypassDurationCheck=true`
  );
  if (!res.ok) return [];
 ...
 *     30s TTL cache keyed by providerId:date:duration. Different durations on
  ...
  for (let dayOffset = 0; dayOffset <= 6; dayOffset++) {
    ...
    const dayResults = await Promise.all(
      durations.map(async duration => ({
        duration,
        slots: await fetchSlotsForDate(providerId, dateStr, duration),
      }))
    );
...
export default function FindNextSlot({ providerId, onSelect }: Props) {
  ...
  async function handleFind() {
    ...
    setSelectedKey(null);
    try {
      const results = await findSuggestions(providerId, 3);
    ...
    <div className="rounded-2xl border border-sky-500/20 bg-sky-950/20 p-4 mb-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-primary">Find Next Available Slot</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleFind}
            disabled={loading}
            className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-primary hover:bg-sky-500 text-foreground transition-colors disabled:opacity-50"
          >
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between gap-3 px-3 py-2.5 rounded-xl text-left transition-all ${
                      isSelected
                        ? 'bg-primary text-foreground border border-sky-500'
                        : 'bg-secondary/60 hover:bg-secondary border border-white/5 hover:border-sky-500/30'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <div className="text-center min-w-[52px]">
                        <p className={`text-xs font-semibold ${isSelected ? 'text-sky-100' : 'text-primary'}`}>
                          {s.displayDate}
                        </p>
                        <p className={`text-base font-bold leading-tight ${isSelected ? 'text-foreground' : 'text-foreground'}`}>
                          {s.displayTime}
                        </p>
                      </div>
                      <div className={`text-xs ${isSelected ? 'text-sky-100' : 'text-muted-foreground'}`}>
                        {s.duration} min
                        {' · '}
                        <span className={isSelected ? 'text-sky-200' : 'text-muted-foreground/60'}>

      ...
      <div className="flex flex-wrap items-center gap-2 p-2 bg-slate-50 rounded-xl border border-slate-200">
        ...
        <div className="flex gap-1">
          <button
            onClick={() => setTool('pen')}
            title="Pen"
            className={`p-2 rounded-lg transition-colors ${tool === 'pen' ? 'bg-primary text-foreground' : 'text-slate-600 hover:bg-slate-200'}`}
          >
          ...
          <button
            ...
            title="Eraser"
            className={`p-2 rounded-lg transition-colors ${tool === 'eraser' ? 'bg-primary text-foreground' : 'text-slate-600 hover:bg-slate-200'}`}
          >
        ...
        {tool === 'pen' && (
          <div className="flex gap-1 items-center">
            {PEN_SIZES.map(s => (
              ...
              >
                <span
                  className="rounded-full bg-secondary inline-block"
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 bg-primary hover:bg-primary/90 text-foreground rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
        >
          {saving ? (
      ...
      <div className="relative rounded-xl overflow-hidden border-2 border-slate-200 bg-white shadow-sm">
        ...
        {strokes.length === 0 && !initialSketchUrl && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <p className="text-foreground text-sm">Draw a sketch to explain the lesson concept</p>
          </div>
        )}
      </div>

...
export default function OfflineEarningsSection({ 
  ...
  ((data as any).allBookings || []).forEach((booking: any) => {
  ...
  Array.from(dayMap.entries())
    ...
    .forEach(([key, bookings]) => {
      groupedByDay.push({
        ...
        totalAmount: bookings.reduce((sum: any, b: any) => sum + (b.offlineAmountPaid || 0), 0),
      });
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">Cash and direct payments — recorded by you, not verified by DriveBook</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {[
          ...
          <div key={label} className={`bg-background rounded-lg shadow-sm p-4 border ${note ? 'border-border' : 'border-amber-900/50'}`}>
            <div className="flex items-center justify-between mb-1">
              <p className="text-xs text-muted-foreground">{label}</p>
              {icon}
            </div>
            <p className={`text-xl font-bold ${note ? 'text-muted-foreground' : 'text-amber-300'}`}>${value.toFixed(2)}</p>
      ...
      {scheduledBookings.count > 0 && (
        <div className="bg-background rounded-lg shadow-sm border border-amber-900/30 overflow-hidden">
          <button
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-foreground text-sm">Scheduled Offline Lessons</span>
              <span className="text-xs text-muted-foreground">(logged but not yet taught)</span>
            </div>
            <div className="text-right">
              <span className="font-bold text-amber-300 text-sm">${scheduledBookings.total.toFixed(2)}</span>
              <span className="text-xs text-muted-foreground ml-2">{scheduledBookings.count} lessons</span>
            </div>
          </button>
      ...
      {completedBookings.length > 0 && (
        <div className="space-y-3">
          <p className="text-xs text-muted-foreground px-1">📋 Offline lessons recorded</p>
          
          {completedBookings.map(day => (
            <div key={day.dateKey} className="bg-background rounded-lg shadow-sm overflow-hidden border border-border">
              <button
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">{day.bookings.length} lesson{day.bookings.length !== 1 ? 's' : ''}</span>
                </div>
              ...
              {expandedDays.has(day.dateKey) && (
                <div className="divide-y divide-slate-800">
                  {day.bookings.map((b: any) => (
                    <div key={b.id} className="flex items-start justify-between px-6 py-2.5 bg-background hover:bg-card/50">
                      <div>
      ...
      {completedBookings.length === 0 && scheduledBookings.count === 0 && (
        <div className="bg-background rounded-lg shadow-sm p-10 text-center border border-border">
          ...
          <p className="text-muted-foreground text-sm mb-3">No offline lessons logged yet</p>
          <a href="/dashboard/bookings/new?offline=true" className="inline-block px-4 py-2 bg-amber-600 text-foreground rounded-lg hover:bg-amber-700 text-sm font-medium">
      ...
      {/* Info box */}
      <div className="bg-background border border-amber-900/30 rounded-lg p-4">

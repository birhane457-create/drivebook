...
const DEDICATED_LINE_TIERS = ['PRO', 'STUDIO', 'PREMIUM']
    ...
    return (
      <div className="rounded-xl border border-border bg-card/60 p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-semibold text-foreground">AI Receptionist Line</h3>
            <p className="mt-1 text-sm text-muted-foreground">
              A dedicated booking phone number is available on the{' '}
              <strong className="text-foreground">PRO plan</strong> and above. Upgrade to get
              your own number that students can call 24/7 — the AI handles bookings,
              cancellations, and reschedules automatically.
            </p>
            <a
              href="/dashboard/settings?tab=subscription"
              className="mt-3 inline-block rounded-lg bg-primary hover:bg-sky-500 px-4 py-2 text-sm font-semibold text-foreground transition-colors"
            >
              Upgrade to PRO
            </a>
          </div>
    ...
    return (
      <div className="rounded-xl border border-red-500/30 bg-red-950/20 p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-semibold text-foreground">AI Receptionist Line — Suspended</h3>
            <p className="mt-1 text-sm text-foreground">
              Your dedicated number{' '}
              <span className="font-mono font-medium text-foreground">{formatAU(voiceLine)}</span>{' '}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="mt-3 flex items-center gap-3 flex-wrap">
            ...
            <button
              onClick={handleCopy}
              className="flex items-center gap-1.5 rounded-lg border border-border bg-secondary hover:bg-secondary/70 px-3 py-1.5 text-sm text-foreground hover:text-foreground transition-colors"
            >
              {copied ? (
                <>
          ...
          <div className="mt-3 rounded-lg bg-card border border-borderite/8 p-3 text-xs text-muted-foreground/60 space-y-1">
            <p>
              <strong className="text-muted-foreground">Greeting callers hear:</strong>{' '}
              "Hi, you've reached your instructor's booking line. I'm the DriveBook assistant…"
            </p>
            <p>
              <strong className="text-muted-foreground">Want to use your own number?</strong>{' '}
              Set it to forward calls to{' '}
              <span className="font-mono text-foreground">{formatAU(voiceLine)}</span> — students

    ...
    <div className="bg-background rounded-2xl border border-border overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-foreground text-sm">Upcoming Scheduled Lessons</span>
          <span className="text-xs text-muted-foreground">{streamLabel}</span>
        </div>
      ...
      {open && (
        <div className="divide-y divide-slate-800">
          ...
          {platformDays.length > 0 && (
            <>
              {hasOffline && (
                <div className="px-4 py-1.5 bg-card/50">
              ...
              {platformDays.map(day => (
                <div key={day.dateKey}>
                  <div className="flex items-center justify-between px-4 py-2 bg-card">
                    <span className="text-xs font-semibold text-foreground uppercase tracking-wide">{day.label}</span>
                    <span className="text-xs text-muted-foreground/60">
                      ${day.items.reduce((s: any, b: any) => s + b.providerPayout, 0).toFixed(2)} net
                    </span>
                  </div>
                  {day.items.map((b: any) => (
                    <div key={b.id} className="flex items-center justify-between px-4 py-2.5 hover:bg-card">
                      <div>
                      ...
                      <p className="text-sm font-semibold text-primary">${b.providerPayout.toFixed(2)}</p>
                    </div>
                  ))}
                </div>
              ))}
              ...
              {offlineDays.map(day => (
                <div key={day.dateKey}>
                  <div className="flex items-center justify-between px-4 py-2 bg-card">
                    <span className="text-xs font-semibold text-foreground uppercase tracking-wide">{day.label}</span>
                    <span className="text-xs text-muted-foreground/60">
                      ${day.items.reduce((s: any, b: any) => s + b.offlineAmountPaid, 0).toFixed(2)}
                    </span>
                  </div>
                  {day.items.map((b: any) => (
                    <div key={b.id} className="flex items-center justify-between px-4 py-2.5 hover:bg-card">

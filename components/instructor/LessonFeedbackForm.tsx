  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground/60 text-sm mb-4">Select the lesson type first — it changes what you record.</p>
          ...
          {(Object.keys(ASSESSMENT_TYPE_CONFIG) as AssessmentType[]).map(type => {
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground/60 leading-snug">{cfg.description}</p>
              </button>
            )
          })}
        </div>
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-sm font-medium border transition-all ${
                    active
                      ? 'bg-emerald-600 border-emerald-600 text-foreground shadow-sm'
      ...
      {isMock && selectedCodes.length > 0 && (
        <div className="bg-gradient-to-r from-blue-700 to-blue-900 text-foreground p-5 rounded-xl shadow-lg">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="text-center">
              <div className={`text-4xl font-bold ${overallScore >= 80 ? 'text-emerald-400' : overallScore >= 65 ? 'text-yellow-300' : 'text-destructive'}`}>
                {overallScore}
              </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          {!isMock && (
            <span className="text-xs text-muted-foreground">Select issues observed during the lesson</span>
          )}
        </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-xs text-muted-foreground/60">{feedbackItems.length} items</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                        ...
                        >
                          <div className="flex items-start gap-3">
                            <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 mt-0.5 ${
                              ...
                              {isSelected && <svg className="w-3 h-3 text-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
                            </div>
      ...
      <div className="bg-white border-2 border-gray-200 rounded-xl overflow-hidden">
        ...
        >
          ...
          {showWhiteboard ? <ChevronUp className="h-4 w-4 text-muted-foreground" /> : <ChevronDown className="h-4 w-4 text-muted-foreground" />}
        </button>
        {showWhiteboard && (
      ...
      <div className="flex gap-3">
        <button
          ...
          className={`flex-1 py-4 rounded-xl font-bold text-base transition-all shadow-lg ${
            isMock ? 'bg-primary hover:bg-primary/90 text-foreground' : 'bg-emerald-600 hover:bg-emerald-700 text-foreground'

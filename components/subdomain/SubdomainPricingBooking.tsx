  ...
  return (
    <>
      ...
      <div className="py-2 border-b border-gray-50">
        <p className="text-xs text-muted-foreground mb-2 uppercase tracking-wide font-medium">Bulk lesson packages</p>
          ...
          {packages.map(({ packageType, hours, label, discountPct, price }) => (
            ...
            >
              <div>
                ...
                <p className="text-xs text-muted-foreground">{discountPct}% bulk discount</p>
              </div>
              <div className="flex items-center gap-2">
                <p className="font-bold" style={{ color: secondary }}>${price.toFixed(0)}</p>
                <span className="text-xs text-foreground group-hover:text-muted-foreground/60 transition-colors">Book →</span>
              </div>
            </button>
          ))}
        </div>
      ...
      {pdaPackage && (
        ...
        >
          <div>
            ...
            <p className="text-xs text-muted-foreground">Configured by instructor</p>
          </div>
          <div className="flex items-center gap-2">
      ...
      <button
        ...
        className="w-full py-4 rounded-xl text-foreground font-bold text-lg transition-all hover:opacity-90 active:scale-95"
        ...
        <div className="fixed inset-0 z-[100] bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 overflow-y-auto">
          ...
          <div className="sticky top-0 z-10 bg-gradient-to-r from-white/5 to-white/2 border-b border-white/6 backdrop-blur-sm shadow-sm">
            <div className="max-w-4xl mx-auto px-4 h-14 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div
                  className="w-7 h-7 rounded-full flex items-center justify-center text-foreground text-xs font-bold shrink-0"
                  style={{ backgroundColor: primary }}
                >
                  {instructor.name.charAt(0)}
                </div>
                <span className="font-semibold text-foreground/90 text-sm truncate max-w-[200px]">
                  {instructor.name}
                </span>
                {preSelected && (
                  <span className="text-foreground/40 text-xs hidden sm:inline">· {preSelected.label}</span>
                )}
              </div>
              <button

      ...
      >
        <div className="flex items-stretch">
          ...
          {isAcceptingBookings ? (
            <button
              onClick={openBooking}
              className="flex-1 flex flex-col items-center justify-center py-2.5 gap-0.5 text-foreground font-bold"
            ...
            <a
              href="/instructors"
              className="flex-1 flex flex-col items-center justify-center py-2.5 gap-0.5 bg-gray-200 text-muted-foreground/60"
            >
              <CalendarDays className="h-5 w-5" />
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 shrink-0 border-b border-gray-100" style={{ backgroundColor: primary }}>
          <button
            ...
            className="text-foreground p-1.5 rounded-lg hover:bg-white/20 active:bg-white/30"
            aria-label="Back"
          >
            <ChevronLeft className="h-6 w-6" />
          </button>
          <div>
            <p className="text-foreground font-bold text-base leading-tight">Book a Lesson</p>
            {instructorName && <p className="text-foreground/80 text-xs">with {instructorName}</p>}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain">
          <div className="p-4 pb-8">
            {children ?? (
              <p className="text-muted-foreground/60 text-sm text-center mt-8">Loading booking form...</p>
            )}
          </div>
        </div>
      </div>
...
function NavTab({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="flex-1 flex flex-col items-center justify-center py-2.5 gap-0.5 text-muted-foreground/60 active:text-gray-900 active:bg-gray-50"
    >
      {icon}

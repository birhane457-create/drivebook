...
const TIER_ORDER: Record<string, number> = { BASIC: 1, PRO: 2, STUDIO: 3, PREMIUM: 4 };
...
function PlanChangeDialog({
  ...
  const content = {
    'upgrade-trial': {
      ...
      confirmStyle: 'bg-emerald-600 hover:bg-green-700 text-foreground',
    },
    'downgrade-trial': {
      ...
      confirmStyle: 'bg-amber-600 hover:bg-amber-700 text-foreground',
    },
    'active-change': {
      ...
      confirmStyle: 'bg-primary hover:bg-primary/90 text-foreground',
    },
  };
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-3 p-5 border-b border-gray-100">
          ...
          <button onClick={onClose} className="p-1.5 hover:bg-gray-100 rounded-lg text-muted-foreground">
            <X className="w-4 h-4" />
          </button>
        </div>
...
export default function SubscriptionPlans({
  ...
  const activePlans = [
    ...
    { tier: 'PRO',      plan: SUBSCRIPTION_PLANS.PRO,      badge: 'POPULAR',        badgeColor: 'bg-primary',    highlight: true,  comingSoon: false },
    { tier: 'STUDIO',   plan: SUBSCRIPTION_PLANS.STUDIO,   badge: 'CUSTOM DOMAIN',  badgeColor: 'bg-indigo-600', highlight: false, comingSoon: false },
    { tier: 'PREMIUM',  plan: SUBSCRIPTION_PLANS.PREMIUM,  badge: 'WHITE LABEL',    badgeColor: 'bg-purple-600', highlight: false, comingSoon: false },
    { tier: 'BUSINESS', plan: null,                         badge: 'COMING SOON',    badgeColor: 'bg-rose-600',   highlight: false, comingSoon: true  },
  ];
  ...
  return (
    <div>
      ...
      {(isActive || isOnTrial || isPastDue) && (
        <div className="mb-8 flex justify-end">
          <button
            ...
            disabled={loading}
            className="px-5 py-2.5 bg-gray-800 text-foreground rounded-lg hover:bg-gray-900 transition-colors flex items-center gap-2 text-sm font-medium disabled:opacity-50"
          >
      ...
      {/* Plans Grid — 5 cards: 4 active + 1 coming soon */}
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-5">
        {activePlans.map(({ tier, plan, badge, badgeColor, highlight, comingSoon }) => {
          ...
          return (
            ...
            >
              {badge && (
                <div className={`absolute top-0 right-0 ${badgeColor} text-foreground px-3 py-1 text-xs font-semibold rounded-bl-lg`}>
                  {badge}
                </div>
              )}
              ...
              <div className="p-5 flex-1">
                <h3 className="text-xl font-bold text-gray-900">
                  {comingSoon ? 'Business' : plan!.name}
                </h3>
                <p className="mt-1 text-sm text-muted-foreground/60">
                  {tier === 'BASIC'    && 'Individual instructor'}
                  {tier === 'PRO'      && 'Growing your business'}
                  {tier === 'STUDIO'   && 'Your own branded domain'}
                  {tier === 'PREMIUM'  && 'Full white-label, direct payments'}
                  {tier === 'BUSINESS' && 'Multi-provider — schools & agencies'}
                </p>
                ...
                {comingSoon ? (
                  <div className="mt-4 space-y-4">
                    ...
                    <ul className="space-y-2">
                      {[
                        ...
                        'Unlimited providers under one account',
                        'Role-based access (admin, manager, provider)',
                        'School-level branding across all providers',
                        'Centralised payouts & reporting',
                  ...
                  <div className="mt-4">
                    ...
                    <span className="text-muted-foreground/60 text-sm">/{billingCycle === 'monthly' ? 'mo' : 'yr'}</span>
                ...
                {!comingSoon && (
                  <ul className="mt-4 space-y-2">
                    {(plan!.features as unknown as string[]).map((feature, i) => {
                      if (feature === '— Coming soon —') {
                        return (
                          <li key={i} className="flex items-center gap-2 pt-1">
                            <div className="flex-1 h-px bg-gray-200" />
                            <span className="text-xs text-muted-foreground font-medium whitespace-nowrap">Coming soon</span>
                            <div className="flex-1 h-px bg-gray-200" />
                          </li>
                        )
                      }
                      ...
                      return (
                        <li key={i} className={`flex items-start gap-2 text-sm ${isComingSoon ? 'text-muted-foreground' : 'text-gray-700'}`}>
                          {isComingSoon ? (
                            <svg className="h-4 w-4 text-foreground shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                          ) : (
              ...
              <div className="px-5 pb-5">
                {comingSoon ? (
                  <div className="space-y-2">
                    <button disabled className="w-full py-2.5 px-4 rounded-lg text-sm font-medium bg-gray-100 text-muted-foreground cursor-not-allowed">
                      Coming Soon
                    </button>
                    <p className="text-center text-xs text-muted-foreground/60">
                      Get notified when it launches →{' '}
                      <a
                        href="mailto:hello@drivebook.com.au?subject=Business tier waitlist"
                        ...
                        : highlight
                          ...
                          : tier === 'STUDIO'
                            ...
                            : tier === 'PREMIUM'
                              ...
                              : 'bg-gray-800 text-foreground hover:bg-gray-900'
                      }`}
                  >
      ...
      <div className="mt-10 bg-blue-50 rounded-xl p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-5 gap-4">
          <div>
            <div className="text-2xl font-bold text-blue-600">15% → 10%</div>
            <p className="text-sm text-gray-600 mt-1">Lower commission as you grow</p>
          </div>
          <div>
            <div className="text-2xl font-bold text-blue-600">AI receptionist</div>
            <p className="text-sm text-gray-600 mt-1">Pro+: dedicated voice line for bookings</p>
          </div>
          <div>
          ...
          <div>
            <div className="text-2xl font-bold text-blue-600">White label</div>
            <p className="text-sm text-gray-600 mt-1">Premium: 0% commission, direct payments</p>
          </div>
          <div className="opacity-60">
            <div className="text-2xl font-bold text-rose-500">Multi-provider</div>
            <p className="text-sm text-gray-600 mt-1">Business: schools & agencies — coming soon</p>
          </div>
        </div>
      </div>
    </div>

  ...
  const checkServiceArea = async (address: string) => {
    ...
    try {
      const res = await fetch(
        `/api/public/check-service-area?providerId=${encodeURIComponent(providerId)}&address=${encodeURIComponent(address)}`
      );
      const data = await res.json();
      ...
      {stepLabels.map((label, i) => (
        <div key={label} className="flex items-center flex-1">
          <div className="flex flex-col items-center flex-1">
            ...
            <span className={`text-xs mt-1 hidden sm:block ${i === step ? 'font-semibold text-gray-900' : 'text-muted-foreground'}`}>
              {label}
            </span>
          </div>
    ...
    <div className="flex gap-3 pt-4">
      ...
      {step < stepLabels.length - 1 ? (
        <button
          ...
          onClick={handleNext}
          disabled={!canProceed()}
          className="flex-1 flex items-center justify-center gap-1 py-3 rounded-lg text-foreground font-bold disabled:opacity-40 transition-colors"
        ...
        <button
          ...
          disabled={loading}
          className="flex-1 flex items-center justify-center gap-2 py-3 rounded-lg text-foreground font-bold disabled:opacity-40 transition-colors"
          style={{ backgroundColor: primary }}
        >
      ...
      <div className="rounded-lg border p-4 bg-white/90 border-gray-200">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg font-semibold transition ${includeTestPackage ? 'bg-destructive text-foreground hover:bg-red-500' : 'bg-primary text-foreground hover:bg-blue-500'}`}
          >
      ...
      {currentLabel === 'Package' && (
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground/60 mt-1">Packages save you money — the more hours, the bigger the discount.</p>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground/60">Choose exact hours — no discount</p>
                </div>
              </div>
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            {(['PACKAGE_6', 'PACKAGE_10', 'PACKAGE_15'] as PackageType[]).map((pkg) => {
              ...
              return (
                ...
                >
                  {badge && (
                    <div className={`absolute -top-2 right-2 ${badge.cls} text-foreground text-xs px-2 py-0.5 rounded-full`}>
                      {badge.label}
                    </div>
                  )}
                  <div className="text-center">
                    ...
                    <p className="text-xs text-muted-foreground line-through">${orig}</p>
                  </div>
                </div>
              );
            })}
          ...
          <div className="rounded-lg border p-4 bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200">
            ...
            <div className="space-y-1 text-sm">
              ...
              <p className="text-center text-muted-foreground/60 text-xs">or 4 x ${summaryPricing.installments.toFixed(2)}</p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-4 py-2 rounded-lg text-foreground text-sm font-medium disabled:opacity-50 shrink-0"
                style={{ backgroundColor: primary }}
              >
                {areaCheckResult === 'checking' ? 'Checking…' : 'Check'}
              </button>
            </div>
            <p className="text-xs text-muted-foreground">Enter your full street address for the most accurate check</p>
          </div>
      ...
      {currentLabel === 'Your Details' && (
        <div className="space-y-4">
          ...
          <div>
            ...
            {emailStatus === 'checking' && <p className="text-xs text-muted-foreground/60 mt-1">Checking email...</p>}
            {emailStatus === 'exists' && (
              <div className="mt-2 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-900">
                ...
                {existingAccountAction === 'none' && (
                  <div className="flex gap-2 mt-1">
                    <a
                      href="/login"
                      className="flex-1 text-center px-3 py-1.5 rounded-lg bg-primary text-foreground text-xs font-bold hover:bg-primary/90"
                    >
                      Login to my account
                    </a>
                    <button
          ...
          <div>
            ...
            {!hasServiceAreaData && <p className="text-xs text-muted-foreground/60 mt-1">Where should {instructorName} pick you up?</p>}
          </div>
      ...
      {currentLabel === 'Confirm' && (
        <div className="space-y-4">
          ...
          <div className="rounded-xl border-2 p-5 space-y-4" style={{ borderColor: `${primary}40`, backgroundColor: `${primary}08` }}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full flex items-center justify-center text-foreground font-bold text-lg shrink-0" style={{ backgroundColor: primary }}>
                {instructorName.charAt(0)}
              </div>
              <div>
                <p className="font-bold text-gray-900">{instructorName}</p>
                <p className="text-xs text-muted-foreground/60">Driving Instructor</p>
              </div>
            </div>
            ...
            <div className="border-t pt-3 space-y-2 text-sm">
              {selectedSlot?.time ? (
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground/60">First Lesson</span>
              ...
              <div className="flex justify-between">
                <span className="text-muted-foreground/60">Package</span>
                <span className="font-medium text-gray-900">{hours} hours</span>
              </div>
              {formData.address && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground/60">Pickup</span>

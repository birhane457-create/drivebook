...
export default function BookingRecoveryBanner() {
  ...
  const handleContinue = () => {
    if (loadFromLocalStorage() && bookingState.provider) {
      // Navigate to the appropriate step based on booking progress
      router.push(`/book/${bookingState.provider.id}/package`);
    }
  };
  ...
  if (!showRecovery || !loadedBooking?.provider) {
    return null;
  }
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-4 flex-1">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-gray-700 mb-3">
                We found your incomplete booking with <span className="font-medium">{loadedBooking.provider?.name || 'your instructor'}</span>. 
          ...
          <div className="flex flex-col sm:flex-row gap-2 flex-shrink-0">
            <button
              ...
              className="px-4 py-2 bg-primary text-foreground rounded-lg font-semibold hover:bg-primary/90 transition-colors whitespace-nowrap"
            >
              Continue →
            </button>
            <button

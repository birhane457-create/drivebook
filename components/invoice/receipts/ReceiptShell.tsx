...
const STATUS = {
  ...
  CANCELLED: { label: 'CANCELLED', Icon: XCircle,       cls: 'bg-red-500/10     text-destructive     border-red-500/30'     },
  CREDITED:  { label: 'CREDITED',  Icon: CheckCircle,   cls: 'bg-sky-500/10     text-primary     border-sky-500/30'     },
...
export default function ReceiptShell({
  ...
  return (
    <div className="bg-card rounded-2xl border border-border shadow-xl overflow-hidden print-container">
      ...
      >
        <div>
          <p className="text-foreground/60 text-xs uppercase tracking-widest mb-0.5">DriveBook</p>
          <h2 className="text-foreground font-bold text-lg leading-tight">{subtitle}</h2>
        </div>
        <div className="text-right">
          <p className="text-foreground/60 text-xs mb-0.5">Receipt</p>
          <p className="text-foreground font-mono text-sm font-semibold">{receiptNumber}</p>
        </div>
      </div>
      ...
      <div className="px-6 py-5 space-y-5">
        ...
        <button
          ...
          className="no-print w-full flex items-center justify-center gap-2 px-4 py-2 text-xs text-muted-foreground border border-border rounded-lg hover:bg-secondary hover:text-foreground transition-colors mt-2"
        >

...
export default function MobileLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const fetchBranding = async () => {
      ...
      const providerIdMatch = pathname.match(/\/book\/([^\/]+)/);
      const providerId = providerIdMatch?.[1];

      if (providerId) {
        try {
          const response = await fetch(`/api/branding?providerId=${providerId}`);
          if (response.ok) {
  ...
  if (role === 'CLIENT') {
    return (
      <nav className="mobile-nav">
        ...
        <a href="/instructors" className={pathname === '/instructors' ? 'active' : ''}>
          Book
        </a>

 ...
 */
export function sanitizeBookingData(booking: any, viewerRole: 'provider' | 'customer' | 'admin') {
  const sanitized = { ...booking };
  
  if (viewerRole === 'provider' && booking.customer) {
    sanitized.client = sanitizeClientForInstructor(booking.customer);
  }
  
  if (viewerRole === 'customer' && booking.provider) {
    sanitized.provider = sanitizeInstructorForClient(booking.provider);
  }
...
export function canAccessClientData(
  ...
  if (requesterRole === 'CLIENT' && requesterId === customerId) {
    return true;
  }
  
  // Instructor can access their client's data
  if (requesterRole === 'provider' && providerId && requesterId === providerId) {
    return true;
  }
...
export async function logDataAccess(
  ...
  accessorId: string,
  accessorRole: string,
  dataType: 'CLIENT' | 'provider' | 'BOOKING' | 'TRANSACTION',
  dataIds: string[],

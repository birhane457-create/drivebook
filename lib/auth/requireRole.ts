...
export async function requireInstructor(session: Session | null): Promise<
  | { error: NextResponse; user: null; provider: null }
  | { error: null; user: { id: string; role: string; email: string; providerId: string }; provider: { id: string; approvalStatus: string; isActive: boolean | null } }
> {
  const noSession = { error: NextResponse.json({ error: 'Unauthorized' }, { status: 401 }), user: null, provider: null } as const
  ...
  if (!(user as any).provider || user.provider?.approvalStatus !== 'APPROVED') {
    return {
  ...
  return {
    error: null,
    user: { ...user, providerId: user.providerId },

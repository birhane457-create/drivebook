...
export async function generatePackageExpiryAlerts() {
  try {
    console.log('â° Starting package expiry alerts job...');
    ...
    for (const pkg of expiringIn7Days) {
      if (!pkg.customer?.userId) continue;
      ...
      if (!existingNotif) {
        ...
        console.log(`âœ… Created 7-day expiry alert for package ${pkg.id}`);
      }
    }
    ...
    for (const pkg of expiringIn1Day) {
      ...
      if (!existingNotif) {
        await createNotification({
          userId: pkg.customer.userId,
          type: 'PACKAGE_EXPIRY',
          title: 'ðŸš¨ Package Expires Tomorrow',
        ...
        console.log(`âœ… Created 1-day expiry alert for package ${pkg.id}`);
      }
    }
    ...
    for (const pkg of expiringToday) {
      ...
      if (!existingNotif) {
        ...
        console.log(`âœ… Created expiry-today alert for package ${pkg.id}`);
      }
    }
    ...
    for (const pkg of expiredYesterday) {
      ...
      if (!existingNotif) {
        ...
        console.log(`âœ… Created expired notification for package ${pkg.id}`);
      }
    }

    console.log('âœ… Package expiry alerts job completed');
    return { success: true };
  } catch (error) {
    console.error('âŒ Error in package expiry alerts job:', error);
    throw error;
  }
}

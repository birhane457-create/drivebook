...
import { getDisplayName } from '@/lib/utils/account'; // White-label: PREMIUM providers show business name to customers
...
interface BookingWithRelations {
  ...
  provider: {
    name: string;
    businessName: string | null;   // Used by getDisplayName for PREMIUM tier white-label
    accountType: string | null;    // 'INDIVIDUAL' | 'BUSINESS' — NOT the subscriptionTier
    subscriptionTier: string | null; // 'BASIC' | 'PRO' | 'STUDIO' | 'PREMIUM'
    timezone?: string | null;
  };
  customer: {
    userId: string | null;
...
function resolveReminderTimezone(booking: BookingWithRelations) {
  const tzCandidate = booking.timezone || booking.provider?.timezone || process.env.DEFAULT_TIMEZONE || DEFAULT_TIMEZONE;
...
function buildReminderMessage(booking: BookingWithRelations, stageKey: string, startTime: Date, timezone: string) {
  ...
  // getDisplayName returns businessName for PREMIUM tier providers, personal name otherwise
  // NOTE: provider.accountType is the legal entity type (INDIVIDUAL/BUSINESS), not the subscription tier
  const displayName = getDisplayName(booking.provider);

  if (stageKey === 'ONE_HOUR') {
    return `Your lesson with ${displayName} starts at ${formattedStart}.`;
  }

  return `Your lesson with ${displayName} is scheduled for ${formattedStart}.`;
}
...
export async function generateBookingReminders() {
  try {
    ...
    const bookings = await (prisma as any).booking.findMany({
      where: {
        status: { in: ['CONFIRMED'] },
        startTime: { gte: earliestStart, lte: latestStart },
        deletedAt: null,
      },
      include: {
        provider: {
          select: {
            name: true,
            businessName: true,    // White-label: shown to customers when PREMIUM tier
            accountType: true,     // Legal entity type — INDIVIDUAL or BUSINESS
    ...
    for (const booking of bookings) {
      if (!booking.customer?.userId || !booking.startTime) continue;

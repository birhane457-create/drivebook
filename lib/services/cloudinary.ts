...
export async function uploadToCloudinary(
  ...
  try {
    ...
    const result = await cloudinary.uploader.upload(fileToUpload, {
      folder: `${process.env.CLOUDINARY_ROOT_FOLDER ?? process.env.NEXT_PUBLIC_PLATFORM_SLUG ?? 'platform'}/${options.folder}`,
...
export async function uploadInstructorDocument(
  ...
  return uploadToCloudinary(file, {
    folder: `private/instructors/${providerId}/${subfolder}`,
    resourceType: 'auto',
...
export async function uploadInstructorImage(
  ...
  return uploadToCloudinary(file, {
    ...
    publicId: `instructor-${providerId}-${imageType}`,
    resourceType: 'image',

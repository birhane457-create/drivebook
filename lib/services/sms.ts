...
class SMSService {
  ...
  async sendBookingConfirmation(data: {
    ...
    return this.sendSMS({ to: data.customerPhone, message: clientMessage });
  }

  // 24hr appointment reminder — customer
  async sendLessonReminderStudent(data: {
    ...
    timezone?: string;
    bookingLabel?: string;   // e.g. "lesson", "appointment", "consultation" — from terminology
  }) {
    const tz = resolveTimezone(data.timezone ?? DEFAULT_TIMEZONE)
    const providerName = data.provider ? getDisplayName(data.provider) : data.instructorName;
    const timeStr = formatLocalTime(data.startTime, tz, { hour: '2-digit', minute: '2-digit' });
    ...
    const message = `Hi ${data.customerName}! Reminder: your ${data.bookingLabel ?? 'appointment'} with ${providerName} is tomorrow ${dateStr} at ${timeStr}.${pickup}`;
    return this.sendSMS({ to: data.customerPhone, message });
  }
  ...
  async sendLessonReminderInstructor(data: {
    ...
    const dateStr = formatLocalDate(data.startTime, tz, { weekday: 'short', day: 'numeric', month: 'short' } as any);
    const pickup = data.pickupAddress ? ` Pickup: ${data.pickupAddress}.` : '';
    const message = `Hi ${data.instructorName}! Reminder: lesson with ${data.customerName} tomorrow ${dateStr} at ${timeStr}.${pickup}`;

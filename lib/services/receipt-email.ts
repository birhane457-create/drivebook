 ...
 *   A â€” Package purchase (Stripe, isPackageBooking=true)
 *   B â€” Wallet lesson booking (instructor books, wallet debit)
 *   C â€” Single lesson first-time (Stripe, single lesson)
 *   D â€” Wallet top-up (Stripe, wallet credit)
 */
...
const PLATFORM_ABN = process.env.PLATFORM_ABN ?? '';

function gstStatement(total: number): string {
  const gstAmount = (total / 11).toFixed(2);
  return PLATFORM_ABN
    ? `<p style="font-size:12px;color:#6b7280;margin:8px 0 0;">GST included: $${gstAmount} &middot; ABN: ${PLATFORM_ABN}</p>`
    : `<p style="font-size:12px;color:#6b7280;margin:8px 0 0;">GST included: $${gstAmount}</p>`;
}
...
// â”€â”€ A: Package Purchase (Stripe) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
export async function sendPackagePurchaseReceipt(data: {
  ...
  provider?: { name: string; businessName?: string | null };
  ...
  const instructorDisplayName = data.provider ? getDisplayName(data.provider) : data.instructorName;
  ...
  <div class="wrap">
    ...
    <div class="body">
      ...
      <div class="section">
        ...
        <p style="margin:0;font-size:15px;font-weight:600;">${data.packageHours}-Hour Package</p>
        <p style="margin:4px 0 0;font-size:14px;color:#6b7280;">with ${instructorDisplayName}${data.instructorLocation ? ` &middot; ${data.instructorLocation}` : ''}</p>
...
// â”€â”€ B: Wallet Lesson Booking (instructor books, wallet debit) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <div class="body">
      <div class="meta">
        <table>
          ...
          <tr><td>Paid by</td><td>${data.customerName}</td></tr>
          ${data.bookedBy === 'provider' ? `<tr><td>Booked by</td><td>Your instructor (${instructorDisplayName})</td></tr>` : ''}
        </table>
      </div>
      ...
      <div class="section">
        ...
        <p style="margin:4px 0 0;font-size:14px;color:#6b7280;">Duration: ${data.durationHours} hour${data.durationHours !== 1 ? 's' : ''} &middot; Instructor: ${instructorDisplayName}</p>
      </div>
...
// â”€â”€ C: Single Lesson (Stripe payment) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
export async function sendSingleLessonReceipt(data: {
  ...
  await emailService.sendGenericEmail({
    from: 'DriveBook Payments <payments@drivebook.com.au>',
    to: data.customerEmail,
    subject: `Receipt ${rn} &mdash; Booking &middot; ${fmt(data.total)}`,
    html,
  });
}

// â”€â”€ D: Wallet Top-Up (Stripe) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <div class="body">
      <div class="meta">
        <table>
          ...
          <tr><td>ABN</td><td>${PLATFORM_ABN}</td></tr>
          <tr><td>Date</td><td>${fmtDate(data.paidAt, { day: 'numeric', month: 'long', year: 'numeric' })} at ${fmtTime(data.paidAt)}</td></tr>
          <tr><td>Account</td><td>${data.customerName}</td></tr>
          <tr><td>Email</td><td>${data.customerEmail}</td></tr>
        </table>
      </div>
...
// â”€â”€ E: Cancellation Refund â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
export async function sendCancellationReceipt(data: {
  ...
  walletBalanceAfter: number;
  cancelledBy: 'provider' | 'customer' | 'admin';
  ...
  const refundLine = hasRefund
    ...
    : `<tr><td>Refund</td><td style="color:#dc2626;">None â€” ${data.noRefundReason || 'less than 24 hours notice'}</td></tr>`;
    ...
    <div class="body">
      <div class="meta">
        <table>
          ...
          <tr><td>Cancelled by</td><td>${data.cancelledBy === 'provider' ? 'Your instructor' : data.cancelledBy === 'admin' ? 'DriveBook support' : 'You'}</td></tr>
        </table>
      </div>
      ...
      <div class="section">
        ...
        <p style="margin:0;font-size:15px;font-weight:600;">${fmtDate(data.lessonDate, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })} at ${fmtTime(data.lessonDate)}</p>
        <p style="margin:4px 0 0;font-size:14px;color:#6b7280;">Instructor: ${instructorDisplayName}</p>
      </div>
...
// â”€â”€ F: Admin Manual Credit â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ G: Admin Manual Deduction â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
export async function sendAdminDeductionReceipt(data: {
  customerName: string;
  customerEmail: string;
  transactionId: string;  // WalletTransaction.id â€” the DB record, unique, traceable
  deductedAt: Date;

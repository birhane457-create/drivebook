 ...
 * - INSTRUCTOR_PAYABLE:{providerId} - What we owe
 * - INSTRUCTOR_PAID:{providerId} - What we've paid
...
export async function getAccountBalances(accountType: AccountType): Promise<Map<string, number>> {
  ...
  for (const entry of entries) {
    ...
    if (entry.creditAccount.startsWith(`${accountType}:`)) {
      ...
      const currentDec = toDecimal(current);
      const entryDec = toDecimal(entry.amount);
      const newBalance = toNumber(roundAmount(sumAmounts([currentDec, entryDec]), 2));
      balances.set(entry.creditAccount, newBalance);
    }
    ...
    if (entry.debitAccount.startsWith(`${accountType}:`)) {
      ...
      const currentDec = toDecimal(current);
      const entryDec = toDecimal(entry.amount);
      const newBalance = toNumber(roundAmount(subtractAmounts(currentDec, entryDec), 2));
      balances.set(entry.debitAccount, newBalance);
    }
  }
 ...
 */
export async function getInstructorPayable(providerId: string): Promise<number> {
  const account = buildAccount(AccountType.INSTRUCTOR_PAYABLE, providerId);
  return getAccountBalance(account);
}
 ...
 */
export async function getInstructorPaid(providerId: string): Promise<number> {
  const account = buildAccount(AccountType.INSTRUCTOR_PAID, providerId);
  return getAccountBalance(account);
}
...
export async function verifyLedgerIntegrity(): Promise<{
  ...
  const difference = toNumber(roundAmount(subtractAmounts(toDecimal(totalDebits), toDecimal(totalCredits)).abs(), 2));

  return {
    valid: isLessThan(toDecimal(difference), 0.01), // Allow 1 cent rounding
    totalDebits,
    totalCredits,
    difference,
  };
...
export async function getAllInstructorPayables(): Promise<
  Array<{ providerId: string; payable: number; paid: number; balance: number }>
> {
  ...
  const results: Array<{ providerId: string; payable: number; paid: number; balance: number }> = [];
  ...
  for (const providerId of allInstructors) {
    const payableAccount = buildAccount(AccountType.INSTRUCTOR_PAYABLE, providerId);
    const paidAccount = buildAccount(AccountType.INSTRUCTOR_PAID, providerId);
    ...
    results.push({
      ...
      paid: paidBalance,
      balance: toNumber(roundAmount(subtractAmounts(toDecimal(payableBalance), toDecimal(paidBalance)), 2)),
    });
  }

  return results.filter((r: any) => isGreaterThan(toDecimal(r.balance), 0.01)); // Only show positive balances
}
...
export async function getPlatformFinancialSummary() {
  ...
  const payableTotal = Array.from(totalPayables.values()).reduce((sum: any, val: any) => toNumber(sumAmounts([toDecimal(sum), toDecimal(val)])), 0);
  const paidTotal = Array.from(totalPaid.values()).reduce((sum: any, val: any) => toNumber(sumAmounts([toDecimal(sum), toDecimal(val)])), 0);
  ...
  return {
    ...
    outstandingPayables: toNumber(roundAmount(subtractAmounts(toDecimal(payableTotal), toDecimal(paidTotal)), 2)),
  };
}

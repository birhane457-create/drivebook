  ...
  try {
    const booking = await prisma.booking.findUnique({
      ...
      include: {
        ...
        provider: { select: { name: true, businessName: true, accountType: true, timezone: true, state: true } },
      },
    }) as any;

    if (!booking || !booking.customer?.userId) {
    ...
    return createNotification({
      ...
      message: `Your instructor ${getDisplayName(booking.provider)} left feedback on your lesson.`,
...
export async function notifyBookingConfirmed(bookingId: string) {
  try {
    ...
    const tz = booking.provider ? resolveTimezone(booking.provider.timezone) ?? timezoneFromState(booking.provider.state) : DEFAULT_TIMEZONE;
    ...
    return createNotification({
      ...
      message: `Your booking with ${getDisplayName(booking.provider)} on ${bookingDate} has been confirmed.`,
      relatedEntityId: bookingId,
      relatedEntityType: 'BOOKING',
...
export async function notifyBookingCancelled(bookingId: string, cancellationReason?: string) {
  try {
    ...
    return createNotification({
      ...
      message: `Your booking with ${getDisplayName(booking.provider)} on ${bookingDate} has been cancelled.${cancellationReason ? ` Reason: ${cancellationReason}` : ''}`,
      relatedEntityId: bookingId,
      relatedEntityType: 'BOOKING',
      actionUrl: '/client-dashboard/bookings',
...
export async function notifyBookingRescheduled(bookingId: string, newDate: Date) {
  try {
    ...
    return createNotification({
      ...
      message: `Your lesson with ${getDisplayName(booking.provider)} has been rescheduled to ${newDateStr} at ${newTimeStr}.`,
      relatedEntityId: bookingId,
      relatedEntityType: 'BOOKING',
      actionUrl: '/client-dashboard/bookings',
      actionButtonLabel: 'View Booking',

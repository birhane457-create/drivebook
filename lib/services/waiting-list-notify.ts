...
export async function notifyWaitingList(event: SlotOpenedEvent): Promise<void> {
  try {
    ...
    const entry = await (prisma as any).waitingList.findFirst({
      where: { providerId: event.providerId, isActive: true },
    ...
    const message =
      `Hi ${entry.customerName.split(' ')[0]}! A lesson slot has opened up with ${instructorName}${slotInfo}. Book now at drivebook.com.au before it's taken!`

    // Send SMS
    if (entry.customerPhone) {
      const { smsService } = await import('@/lib/services/sms')
      await smsService.sendSMS({ to: entry.customerPhone, message })
    }
    ...
    if (entry.customerEmail) {
      ...
      await emailService.sendGenericEmail({
        ...
        html: `
          <p>Hi ${entry.customerName.split(' ')[0]},</p>
